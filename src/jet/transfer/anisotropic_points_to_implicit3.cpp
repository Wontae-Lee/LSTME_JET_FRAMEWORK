...
void
AnisotropicPointsToImplicit3::convert(const ConstArrayAccessor1<Vector3D>& points, ScalarGrid3* output) const
{
  if (output == nullptr) {
  ...
  temp->fill([&](const Vector3D& x) {
    ...
    meanNeighborSearcher2.forEachNearbyPoint(
      x, r, [&](size_t i, const Vector3D& neighborPosition) { sum += m / d[i] * w(neighborPosition - x, gs[i], gs[i].determinant()); });
