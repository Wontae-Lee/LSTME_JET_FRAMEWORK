...
#include "jet/transform/anisotropic_points_to_implicit2.h"
...
inline Matrix2x2D vvt(const Vector2D& v) { return Matrix2x2D(v.x * v.x, v.x * v.y, v.y * v.x, v.y * v.y); }
...
AnisotropicPointsToImplicit2::AnisotropicPointsToImplicit2(double kernelRadius, double cutOffDensity, double positionSmoothingFactor, size_t minNumNeighbors, bool isOutputSdf)
...
void AnisotropicPointsToImplicit2::convert(const ConstArrayAccessor1<Vector2D>& points, ScalarGrid2* output) const {
    ...
    temp->fill([&](const Vector2D& x) {
        double sum = 0.0;
        meanNeighborSearcher2.forEachNearbyPoint(x, r, [&](size_t i, const Vector2D& neighborPosition) { sum += m / d[i] * w(neighborPosition - x, gs[i], gs[i].determinant()); });
