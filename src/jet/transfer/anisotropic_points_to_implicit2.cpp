...
void
AnisotropicPointsToImplicit2::convert(const ConstArrayAccessor1<Vector2D>& points, ScalarGrid2* output) const
{
  if (output == nullptr) {
  ...
  temp->fill([&](const Vector2D& x) {
    ...
    meanNeighborSearcher2.forEachNearbyPoint(
      x, r, [&](size_t i, const Vector2D& neighborPosition) { sum += m / d[i] * w(neighborPosition - x, gs[i], gs[i].determinant()); });
