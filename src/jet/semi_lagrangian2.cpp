...
#include "jet/solver/lagrangian/semi_lagrangian2.h"
#include <pch.h>
#include <algorithm>
...
void SemiLagrangian2::advect(const ScalarGrid2& input, const VectorField2& flow, double dt, ScalarGrid2* output, const ScalarField2& boundarySdf) {
...
void SemiLagrangian2::advect(const CollocatedVectorGrid2& input, const VectorField2& flow, double dt, CollocatedVectorGrid2* output, const ScalarField2& boundarySdf) {
...
void SemiLagrangian2::advect(const FaceCenteredGrid2& input, const VectorField2& flow, double dt, FaceCenteredGrid2* output, const ScalarField2& boundarySdf) {
    auto inputSamplerFunc = getVectorSamplerFunc(input);
...
Vector2D SemiLagrangian2::backTrace(const VectorField2& flow, double dt, double h, const Vector2D& startPt, const ScalarField2& boundarySdf) {
...
std::function<double(const Vector2D&)> SemiLagrangian2::getScalarSamplerFunc(const ScalarGrid2& input) const { return input.sampler(); }

std::function<Vector2D(const Vector2D&)> SemiLagrangian2::getVectorSamplerFunc(const CollocatedVectorGrid2& input) const { return input.sampler(); }

std::function<Vector2D(const Vector2D&)> SemiLagrangian2::getVectorSamplerFunc(const FaceCenteredGrid2& input) const { return input.sampler(); }
