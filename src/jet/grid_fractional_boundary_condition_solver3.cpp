...
void GridFractionalBoundaryConditionSolver3::constrainVelocity(
  ...
  extrapolateToRegion(velocity->uConstAccessor(), uMarker, extrapolationDepth,
                      u);
  extrapolateToRegion(velocity->vConstAccessor(), vMarker, extrapolationDepth,
                      v);
  extrapolateToRegion(velocity->wConstAccessor(), wMarker, extrapolationDepth,
                      w);
  ...
  velocity->parallelForEachUIndex([&](size_t i, size_t j, size_t k) {
    ...
    if (isInsideSdf(_colliderSdf->sample(pt))) {
      ...
      if (g.lengthSquared() > 0.0) {
        ...
        Vector3D velt =
            projectAndApplyFriction(velr, n, collider()->frictionCoefficient());
