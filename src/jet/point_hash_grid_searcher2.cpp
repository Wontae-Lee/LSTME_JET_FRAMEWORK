...
PointHashGridSearcher2::PointHashGridSearcher2(size_t resolutionX, size_t resolutionY, double gridSpacing) : _gridSpacing(gridSpacing) {
...
void PointHashGridSearcher2::forEachNearbyPoint(const Vector2D& origin, double radius, const ForEachNearbyPointFunc& callback) const {
    if (_buckets.empty()) {
        return;
    }
...
void PointHashGridSearcher2::serialize(std::vector<uint8_t>* buffer) const {
    ...
    for (const auto& bucket : _buckets) {
        ...
        flatbuffers::Offset<fbs::PointHashGridSearcherBucket2> fbsBucket = fbs::CreatePointHashGridSearcherBucket2(builder, builder.CreateVector(bucket64.data(), bucket64.size()));
...
PointHashGridSearcher2 PointHashGridSearcher2::Builder::build() const { return PointHashGridSearcher2(_resolution, _gridSpacing); }

PointHashGridSearcher2Ptr PointHashGridSearcher2::Builder::makeShared() const {
    return std::shared_ptr<PointHashGridSearcher2>(new PointHashGridSearcher2(_resolution, _gridSpacing), [](PointHashGridSearcher2* obj) { delete obj; });
}
