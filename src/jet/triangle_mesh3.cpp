...
struct WindingNumberGatherData {
  ...
  operator+(const WindingNumberGatherData &other) const {
...
WindingNumberGatherData
postOrderTraversal(const Bvh3<size_t> &bvh, size_t nodeIndex,
  ...
  if (bvh.isLeaf(nodeIndex)) {
    ...
    data = data + postOrderTraversal(bvh, children.first, visitorFunc, leafFunc,
                                     initGatherData);
...
TriangleMesh3::TriangleMesh3(const PointArray &points,
                             ...
                             bool isNormalFlipped_)
    : Surface3(transform_, isNormalFlipped_), _points(points),
      _normals(normals), _uvs(uvs), _pointIndices(pointIndices),
      _normalIndices(normalIndices), _uvIndices(uvIndices) {}
...
TriangleMesh3Ptr TriangleMesh3::Builder::makeShared() const {
  return std::shared_ptr<TriangleMesh3>(
      new TriangleMesh3(_points, _normals, _uvs, _pointIndices, _normalIndices,
                        _uvIndices, _transform, _isNormalFlipped),
