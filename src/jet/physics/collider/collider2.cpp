...
void Collider2::setFrictionCoefficient(double newFrictionCoeffient) { _frictionCoeffient = std::max(newFrictionCoeffient, 0.0); }

const Surface2Ptr& Collider2::surface() const { return _surface; }

void Collider2::setSurface(const Surface2Ptr& newSurface) { _surface = newSurface; }
...
void Collider2::setOnBeginUpdateCallback(const OnBeginUpdateCallback& callback) { _onUpdateCallback = callback; }
