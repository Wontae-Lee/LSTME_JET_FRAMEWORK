...
PointParticleEmitter3::PointParticleEmitter3(
    ...
    : _rng(seed), _maxNumberOfNewParticlesPerSecond(maxNumOfNewParticlesPerSec),
      _maxNumberOfParticles(maxNumOfParticles), _origin(origin),
...
void PointParticleEmitter3::emit(Array1<Vector3D> *newPositions,
  ...
  for (size_t i = 0; i < maxNewNumberOfParticles; ++i) {
    Vector3D newDirection = uniformSampleCone(random(), random(), _direction,
