...
size_t
FdmMgLinearSystem3::numberOfLevels() const
{
  return A.levels.size();
}

void
FdmMgLinearSystem3::resizeWithCoarsest(const Size3& coarsestResolution, size_t numberOfLevels)
{
...
void
FdmMgLinearSystem3::resizeWithFinest(const Size3& finestResolution, size_t maxNumberOfLevels)
{
...
void
FdmMgUtils3::correct(const FdmVector3& coarser, FdmVector3* finer)
{
  JET_ASSERT(finer->size().x == 2 * coarser.size().x);
  JET_ASSERT(finer->size().y == 2 * coarser.size().y);
  JET_ASSERT(finer->size().z == 2 * coarser.size().z);
