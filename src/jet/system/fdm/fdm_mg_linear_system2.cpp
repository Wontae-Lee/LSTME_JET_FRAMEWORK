...
size_t
FdmMgLinearSystem2::numberOfLevels() const
{
  return A.levels.size();
}

void
FdmMgLinearSystem2::resizeWithCoarsest(const Size2& coarsestResolution, size_t numberOfLevels)
{
...
void
FdmMgLinearSystem2::resizeWithFinest(const Size2& finestResolution, size_t maxNumberOfLevels)
{
...
void
FdmMgUtils2::correct(const FdmVector2& coarser, FdmVector2* finer)
{
  JET_ASSERT(finer->size().x == 2 * coarser.size().x);
  JET_ASSERT(finer->size().y == 2 * coarser.size().y);
