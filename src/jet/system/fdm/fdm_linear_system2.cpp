...
void FdmBlas2::mvm(const FdmMatrix2& m, const FdmVector2& v, FdmVector2* result) {
    ...
    m.parallelForEachIndex([&](size_t i, size_t j) {
        (*result)(i, j) = m(i, j).center * v(i, j) + ((i > 0) ? m(i - 1, j).right * v(i - 1, j) : 0.0) + ((i + 1 < size.x) ? m(i, j).right * v(i + 1, j) : 0.0) +
                          ((j > 0) ? m(i, j - 1).up * v(i, j - 1) : 0.0) + ((j + 1 < size.y) ? m(i, j).up * v(i, j + 1) : 0.0);
    });
}
...
void FdmBlas2::residual(const FdmMatrix2& a, const FdmVector2& x, const FdmVector2& b, FdmVector2* result) {
    ...
    a.parallelForEachIndex([&](size_t i, size_t j) {
        (*result)(i, j) = b(i, j) - a(i, j).center * x(i, j) - ((i > 0) ? a(i - 1, j).right * x(i - 1, j) : 0.0) - ((i + 1 < size.x) ? a(i, j).right * x(i + 1, j) : 0.0) -
