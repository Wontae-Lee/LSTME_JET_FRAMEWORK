...
ParticleSystemData3::ParticleSystemData3(size_t numberOfParticles) {
    ...
    _neighborSearcher = std::make_shared<PointParallelHashGridSearcher3>(kDefaultHashGridResolution, kDefaultHashGridResolution, kDefaultHashGridResolution, 2.0 * _radius);
...
ConstArrayAccessor1<Vector3D> ParticleSystemData3::positions() const { return vectorDataAt(_positionIdx); }
...
ConstArrayAccessor1<double> ParticleSystemData3::scalarDataAt(size_t idx) const { return _scalarDataList[idx].constAccessor(); }

ArrayAccessor1<double> ParticleSystemData3::scalarDataAt(size_t idx) { return _scalarDataList[idx].accessor(); }

ConstArrayAccessor1<Vector3D> ParticleSystemData3::vectorDataAt(size_t idx) const { return _vectorDataList[idx].constAccessor(); }
...
void ParticleSystemData3::setNeighborSearcher(const PointNeighborSearcher3Ptr& newNeighborSearcher) { _neighborSearcher = newNeighborSearcher; }
...
void ParticleSystemData3::buildNeighborSearcher(double maxSearchRadius) {
    ...
    // Use PointParallelHashGridSearcher3 by default
    _neighborSearcher = std::make_shared<PointParallelHashGridSearcher3>(kDefaultHashGridResolution, kDefaultHashGridResolution, kDefaultHashGridResolution, 2.0 * maxSearchRadius);
...
void ParticleSystemData3::serializeParticleSystemData(flatbuffers::FlatBufferBuilder* builder, flatbuffers::Offset<fbs::ParticleSystemData3>* fbsParticleSystemData) const {
    // Copy data
    ...
    auto fbsNeighborSearcher = fbs::CreatePointNeighborSearcherSerialized3(*builder, neighborSearcherType, builder->CreateVector(neighborSearcherSerialized.data(), neighborSearcherSerialized.size()));
    ...
    for (const auto& neighbors : _neighborLists) {
        ...
        flatbuffers::Offset<fbs::ParticleNeighborList3> fbsNeighborList = fbs::CreateParticleNeighborList3(*builder, builder->CreateVector(neighbors64.data(), neighbors64.size()));
    ...
    *fbsParticleSystemData =
        fbs::CreateParticleSystemData3(*builder, _radius, _mass, _positionIdx, _velocityIdx, _forceIdx, fbsScalarDataList, fbsVectorDataList, fbsNeighborSearcher, fbsNeighborLists);
}
...
void ParticleSystemData3::deserializeParticleSystemData(const fbs::ParticleSystemData3* fbsParticleSystemData) {
    ...
    for (uint32_t i = 0; i < fbsNeighborLists->size(); ++i) {
        ...
        std::transform(fbsNeighborList->data()->begin(), fbsNeighborList->data()->end(), _neighborLists[i].begin(), [](uint64_t val) { return static_cast<size_t>(val); });
    }
}
