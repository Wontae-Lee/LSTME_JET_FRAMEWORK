...
#include "jet/system/data/sph_system_data2.h"
...
void
SphSystemData2::setRadius(double newRadius)
{
...
void
SphSystemData2::setMass(double newMass)
{
...
SphSystemData2::densities() const
{
  return scalarDataAt(_densityIdx);
}

ArrayAccessor1<double>
SphSystemData2::densities()
{
  return scalarDataAt(_densityIdx);
}

ConstArrayAccessor1<double>
SphSystemData2::pressures() const
{
  return scalarDataAt(_pressureIdx);
}

ArrayAccessor1<double>
SphSystemData2::pressures()
{
  return scalarDataAt(_pressureIdx);
}

void
SphSystemData2::updateDensities()
{
  auto p = positions();
  auto d = densities();
  const double m = mass();
...
void
SphSystemData2::setTargetDensity(double targetDensity)
{
...
double
SphSystemData2::targetDensity() const
{
  return _targetDensity;
}

void
SphSystemData2::setTargetSpacing(double spacing)
{
...
double
SphSystemData2::targetSpacing() const
{
  return _targetSpacing;
}

void
SphSystemData2::setRelativeKernelRadius(double relativeRadius)
{
...
double
SphSystemData2::relativeKernelRadius() const
{
  return _kernelRadiusOverTargetSpacing;
}

void
SphSystemData2::setKernelRadius(double kernelRadius)
{
...
double
SphSystemData2::kernelRadius() const
{
  return _kernelRadius;
}

double
SphSystemData2::sumOfKernelNearby(const Vector2D& origin) const
{
  double sum = 0.0;
  SphStdKernel2 kernel(_kernelRadius);
  neighborSearcher()->forEachNearbyPoint(origin, _kernelRadius, [&](size_t, const Vector2D& neighborPosition) {
...
double
SphSystemData2::interpolate(const Vector2D& origin, const ConstArrayAccessor1<double>& values) const
{
  double sum = 0.0;
  auto d = densities();
  SphStdKernel2 kernel(_kernelRadius);
  ...
  neighborSearcher()->forEachNearbyPoint(origin, _kernelRadius, [&](size_t i, const Vector2D& neighborPosition) {
    double dist = origin.distanceTo(neighborPosition);
...
Vector2D
SphSystemData2::interpolate(const Vector2D& origin, const ConstArrayAccessor1<Vector2D>& values) const
{
  Vector2D sum;
  auto d = densities();
  SphStdKernel2 kernel(_kernelRadius);
...
Vector2D
SphSystemData2::gradientAt(size_t i, const ConstArrayAccessor1<double>& values) const
{
  Vector2D sum;
  auto p = positions();
  auto d = densities();
...
double
SphSystemData2::laplacianAt(size_t i, const ConstArrayAccessor1<double>& values) const
{
  double sum = 0.0;
  auto p = positions();
  auto d = densities();
...
Vector2D
SphSystemData2::laplacianAt(size_t i, const ConstArrayAccessor1<Vector2D>& values) const
{
  Vector2D sum;
  auto p = positions();
  auto d = densities();
...
void
SphSystemData2::buildNeighborSearcher()
{
  ParticleSystemData2::buildNeighborSearcher(_kernelRadius);
}

void
SphSystemData2::buildNeighborLists()
{
  ParticleSystemData2::buildNeighborLists(_kernelRadius);
}

void
SphSystemData2::computeMass()
{
  Array1<Vector2D> points;
  TrianglePointGenerator pointsGenerator;
  BoundingBox2D sampleBound(Vector2D(-1.5 * _kernelRadius, -1.5 * _kernelRadius), Vector2D(1.5 * _kernelRadius, 1.5 * _kernelRadius));
...
void
SphSystemData2::serialize(std::vector<uint8_t>* buffer) const
{
  ...
  auto fbsSphSystemData = fbs::CreateSphSystemData2(
    builder, fbsParticleSystemData, _targetDensity, _targetSpacing, _kernelRadiusOverTargetSpacing, _kernelRadius, _pressureIdx, _densityIdx);
...
void
SphSystemData2::deserialize(const std::vector<uint8_t>& buffer)
{
...
void
SphSystemData2::set(const SphSystemData2& other)
{
