...
FaceCenteredGrid2::FaceCenteredGrid2()
    : _dataOriginU(0.0, 0.5), _dataOriginV(0.5, 0.0),
...
double FaceCenteredGrid2::divergence(const Vector2D &x) const {
  ...
  getBarycentric(normalizedX.x, 0, static_cast<ssize_t>(resolution().x) - 1, &i,
                 &fx);
  getBarycentric(normalizedX.y, 0, static_cast<ssize_t>(resolution().y) - 1, &j,
                 &fy);
