...
void Collider3::setFrictionCoefficient(double newFrictionCoeffient) { _frictionCoeffient = std::max(newFrictionCoeffient, 0.0); }

const Surface3Ptr& Collider3::surface() const { return _surface; }

void Collider3::setSurface(const Surface3Ptr& newSurface) { _surface = newSurface; }
...
void Collider3::setOnBeginUpdateCallback(const OnBeginUpdateCallback& callback) { _onUpdateCallback = callback; }
