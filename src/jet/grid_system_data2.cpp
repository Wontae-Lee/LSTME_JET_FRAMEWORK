...
const ScalarGrid2Ptr& GridSystemData2::advectableScalarDataAt(size_t idx) const { return _advectableScalarDataList[idx]; }

const VectorGrid2Ptr& GridSystemData2::advectableVectorDataAt(size_t idx) const { return _advectableVectorDataList[idx]; }
...
void GridSystemData2::serialize(std::vector<uint8_t>* buffer) const {
    ...
    auto gsd = fbs::CreateGridSystemData2(builder, &resolution, &gridSpacing, &origin, _velocityIdx, builder.CreateVector(scalarDataList), builder.CreateVector(vectorDataList),
