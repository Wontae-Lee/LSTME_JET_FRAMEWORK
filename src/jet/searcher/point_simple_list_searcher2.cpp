...
void
PointSimpleListSearcher2::build(const ConstArrayAccessor1<Vector2D>& points)
{
...
void
PointSimpleListSearcher2::forEachNearbyPoint(const Vector2D& origin, double radius, const ForEachNearbyPointFunc& callback) const
{
...
bool
PointSimpleListSearcher2::hasNearbyPoint(const Vector2D& origin, double radius) const
{
  double radiusSquared = radius * radius;
  for (size_t i = 0; i < _points.size(); ++i) {
    Vector2D r = _points[i] - origin;
...
PointSimpleListSearcher2::clone() const
{
...
void
PointSimpleListSearcher2::set(const PointSimpleListSearcher2& other)
{
  _points = other._points;
}

void
PointSimpleListSearcher2::serialize(std::vector<uint8_t>* buffer) const
{
...
void
PointSimpleListSearcher2::deserialize(const std::vector<uint8_t>& buffer)
{
...
{
  return std::shared_ptr<PointSimpleListSearcher2>(new PointSimpleListSearcher2(), [](PointSimpleListSearcher2* obj) { delete obj; });
}
