...
void
PointSimpleListSearcher3::build(const ConstArrayAccessor1<Vector3D>& points)
{
...
void
PointSimpleListSearcher3::forEachNearbyPoint(const Vector3D& origin,
                                             double radius,
...
bool
PointSimpleListSearcher3::hasNearbyPoint(const Vector3D& origin, double radius) const
{
  double radiusSquared = radius * radius;
  for (size_t i = 0; i < _points.size(); ++i) {
    Vector3D r = _points[i] - origin;
...
PointSimpleListSearcher3::clone() const
{
...
void
PointSimpleListSearcher3::set(const PointSimpleListSearcher3& other)
{
  _points = other._points;
}

void
PointSimpleListSearcher3::serialize(std::vector<uint8_t>* buffer) const
{
...
void
PointSimpleListSearcher3::deserialize(const std::vector<uint8_t>& buffer)
{
...
{
  return std::shared_ptr<PointSimpleListSearcher3>(new PointSimpleListSearcher3(), [](PointSimpleListSearcher3* obj) { delete obj; });
}
