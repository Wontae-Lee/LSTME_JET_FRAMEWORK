...
void
PointKdTreeSearcher3::build(const ConstArrayAccessor1<Vector3D>& points)
{
  _tree.build(points);
}

void
PointKdTreeSearcher3::forEachNearbyPoint(const Vector3D& origin, double radius, const ForEachNearbyPointFunc& callback) const
{
  _tree.forEachNearbyPoint(origin, radius, callback);
}

bool
PointKdTreeSearcher3::hasNearbyPoint(const Vector3D& origin, double radius) const
{
  return _tree.hasNearbyPoint(origin, radius);
}

PointNeighborSearcher3Ptr
PointKdTreeSearcher3::clone() const
{
...
void
PointKdTreeSearcher3::set(const PointKdTreeSearcher3& other)
{
  _tree = other._tree;
}

void
PointKdTreeSearcher3::serialize(std::vector<uint8_t>* buffer) const
{
  ...
  // Copy the searcher
  auto fbsSearcher = fbs::CreatePointKdTreeSearcher3(builder, fbsPoints, fbsNodes);
...
void
PointKdTreeSearcher3::deserialize(const std::vector<uint8_t>& buffer)
{
...
PointKdTreeSearcher3
PointKdTreeSearcher3::Builder::build() const
{
  return PointKdTreeSearcher3{};
}

PointKdTreeSearcher3Ptr
PointKdTreeSearcher3::Builder::makeShared() const
{
  return std::shared_ptr<PointKdTreeSearcher3>(new PointKdTreeSearcher3, [](PointKdTreeSearcher3* obj) { delete obj; });
}
