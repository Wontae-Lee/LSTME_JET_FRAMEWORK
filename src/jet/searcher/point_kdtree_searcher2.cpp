...
void PointKdTreeSearcher2::build(const ConstArrayAccessor1<Vector2D>& points) { _tree.build(points); }

void PointKdTreeSearcher2::forEachNearbyPoint(const Vector2D& origin, double radius, const ForEachNearbyPointFunc& callback) const { _tree.forEachNearbyPoint(origin, radius, callback); }

bool PointKdTreeSearcher2::hasNearbyPoint(const Vector2D& origin, double radius) const { return _tree.hasNearbyPoint(origin, radius); }
