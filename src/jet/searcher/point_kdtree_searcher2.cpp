...
void
PointKdTreeSearcher2::build(const ConstArrayAccessor1<Vector2D>& points)
{
  _tree.build(points);
}

void
PointKdTreeSearcher2::forEachNearbyPoint(const Vector2D& origin, double radius, const ForEachNearbyPointFunc& callback) const
{
  _tree.forEachNearbyPoint(origin, radius, callback);
}

bool
PointKdTreeSearcher2::hasNearbyPoint(const Vector2D& origin, double radius) const
{
  return _tree.hasNearbyPoint(origin, radius);
}

PointNeighborSearcher2Ptr
PointKdTreeSearcher2::clone() const
{
...
void
PointKdTreeSearcher2::set(const PointKdTreeSearcher2& other)
{
  _tree = other._tree;
}

void
PointKdTreeSearcher2::serialize(std::vector<uint8_t>* buffer) const
{
  ...
  // Copy the searcher
  auto fbsSearcher = fbs::CreatePointKdTreeSearcher2(builder, fbsPoints, fbsNodes);
...
void
PointKdTreeSearcher2::deserialize(const std::vector<uint8_t>& buffer)
{
...
PointKdTreeSearcher2
PointKdTreeSearcher2::Builder::build() const
{
  return PointKdTreeSearcher2{};
}

PointKdTreeSearcher2Ptr
PointKdTreeSearcher2::Builder::makeShared() const
{
  return std::shared_ptr<PointKdTreeSearcher2>(new PointKdTreeSearcher2, [](PointKdTreeSearcher2* obj) { delete obj; });
}
