...
void
PointHashGridSearcher3::build(const ConstArrayAccessor1<Vector3D>& points)
{
  _buckets.clear();
  _points.clear();
...
void
PointHashGridSearcher3::forEachNearbyPoint(const Vector3D& origin, double radius, const std::function<void(size_t, const Vector3D&)>& callback) const
{
  if (_buckets.empty()) {
    return;
  }
...
bool
PointHashGridSearcher3::hasNearbyPoint(const Vector3D& origin, double radius) const
{
  if (_buckets.empty()) {
    return false;
  }
...
void
PointHashGridSearcher3::add(const Vector3D& point)
{
  if (_buckets.empty()) {
...
Point3I
PointHashGridSearcher3::getBucketIndex(const Vector3D& position) const
{
  Point3I bucketIndex;
...
size_t
PointHashGridSearcher3::getHashKeyFromPosition(const Vector3D& position) const
{
...
size_t
PointHashGridSearcher3::getHashKeyFromBucketIndex(const Point3I& bucketIndex) const
{
...
void
PointHashGridSearcher3::getNearbyKeys(const Vector3D& position, size_t* nearbyKeys) const
{
...
PointHashGridSearcher3::clone() const
{
...
void
PointHashGridSearcher3::set(const PointHashGridSearcher3& other)
{
...
void
PointHashGridSearcher3::serialize(std::vector<uint8_t>* buffer) const
{
  ...
  for (const auto& bucket : _buckets) {
    ...
    flatbuffers::Offset<fbs::PointHashGridSearcherBucket3> fbsBucket =
      fbs::CreatePointHashGridSearcherBucket3(builder, builder.CreateVector(bucket64.data(), bucket64.size()));
  ...
  // Copy the searcher
  auto fbsSearcher = fbs::CreatePointHashGridSearcher3(builder, _gridSpacing, &fbsResolution, fbsPoints, fbsBuckets);
...
void
PointHashGridSearcher3::deserialize(const std::vector<uint8_t>& buffer)
{
  ...
  for (uint32_t i = 0; i < fbsBuckets->size(); ++i) {
    ...
    std::transform(fbsBucket->data()->begin(), fbsBucket->data()->end(), _buckets[i].begin(), [](uint64_t val) { return static_cast<size_t>(val); });
  }
}
