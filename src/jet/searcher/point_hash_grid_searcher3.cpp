...
void PointHashGridSearcher3::forEachNearbyPoint(const Vector3D& origin, double radius, const std::function<void(size_t, const Vector3D&)>& callback) const {
    if (_buckets.empty()) {
        return;
    }
...
void PointHashGridSearcher3::serialize(std::vector<uint8_t>* buffer) const {
    ...
    for (const auto& bucket : _buckets) {
        ...
        flatbuffers::Offset<fbs::PointHashGridSearcherBucket3> fbsBucket = fbs::CreatePointHashGridSearcherBucket3(builder, builder.CreateVector(bucket64.data(), bucket64.size()));
...
PointHashGridSearcher3 PointHashGridSearcher3::Builder::build() const { return PointHashGridSearcher3(_resolution, _gridSpacing); }

PointHashGridSearcher3Ptr PointHashGridSearcher3::Builder::makeShared() const {
    return std::shared_ptr<PointHashGridSearcher3>(new PointHashGridSearcher3(_resolution, _gridSpacing), [](PointHashGridSearcher3* obj) { delete obj; });
}
