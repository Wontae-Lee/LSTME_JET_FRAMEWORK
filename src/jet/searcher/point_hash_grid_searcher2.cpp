...
void
PointHashGridSearcher2::build(const ConstArrayAccessor1<Vector2D>& points)
{
  _buckets.clear();
  _points.clear();
...
void
PointHashGridSearcher2::forEachNearbyPoint(const Vector2D& origin, double radius, const ForEachNearbyPointFunc& callback) const
{
  if (_buckets.empty()) {
    return;
  }
...
bool
PointHashGridSearcher2::hasNearbyPoint(const Vector2D& origin, double radius) const
{
  if (_buckets.empty()) {
    return false;
  }
...
void
PointHashGridSearcher2::add(const Vector2D& point)
{
  if (_buckets.empty()) {
...
Point2I
PointHashGridSearcher2::getBucketIndex(const Vector2D& position) const
{
  Point2I bucketIndex;
...
size_t
PointHashGridSearcher2::getHashKeyFromPosition(const Vector2D& position) const
{
...
size_t
PointHashGridSearcher2::getHashKeyFromBucketIndex(const Point2I& bucketIndex) const
{
...
void
PointHashGridSearcher2::getNearbyKeys(const Vector2D& position, size_t* nearbyKeys) const
{
...
PointHashGridSearcher2::clone() const
{
...
void
PointHashGridSearcher2::set(const PointHashGridSearcher2& other)
{
...
void
PointHashGridSearcher2::serialize(std::vector<uint8_t>* buffer) const
{
  ...
  for (const auto& bucket : _buckets) {
    ...
    flatbuffers::Offset<fbs::PointHashGridSearcherBucket2> fbsBucket =
      fbs::CreatePointHashGridSearcherBucket2(builder, builder.CreateVector(bucket64.data(), bucket64.size()));
  ...
  // Copy the searcher
  auto fbsSearcher = fbs::CreatePointHashGridSearcher2(builder, _gridSpacing, &fbsResolution, fbsPoints, fbsBuckets);
...
void
PointHashGridSearcher2::deserialize(const std::vector<uint8_t>& buffer)
{
