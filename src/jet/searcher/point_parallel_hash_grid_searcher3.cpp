...
void
PointParallelHashGridSearcher3::build(const ConstArrayAccessor1<Vector3D>& points)
{
  _points.clear();
  _keys.clear();
  ...
  parallelSort(
    _sortedIndices.begin(), _sortedIndices.end(), [&tempKeys](size_t indexA, size_t indexB) { return tempKeys[indexA] < tempKeys[indexB]; });
...
void
PointParallelHashGridSearcher3::forEachNearbyPoint(const Vector3D& origin, double radius, const ForEachNearbyPointFunc& callback) const
{
  size_t nearbyKeys[8];
...
bool
PointParallelHashGridSearcher3::hasNearbyPoint(const Vector3D& origin, double radius) const
{
  size_t nearbyKeys[8];
  getNearbyKeys(origin, nearbyKeys);
...
PointParallelHashGridSearcher3::keys() const
{
  return _keys;
}
...
Point3I
PointParallelHashGridSearcher3::getBucketIndex(const Vector3D& position) const
{
  Point3I bucketIndex;
...
size_t
PointParallelHashGridSearcher3::getHashKeyFromPosition(const Vector3D& position) const
{
...
size_t
PointParallelHashGridSearcher3::getHashKeyFromBucketIndex(const Point3I& bucketIndex) const
{
...
void
PointParallelHashGridSearcher3::getNearbyKeys(const Vector3D& position, size_t* nearbyKeys) const
{
...
PointParallelHashGridSearcher3::clone() const
{
...
void
PointParallelHashGridSearcher3::set(const PointParallelHashGridSearcher3& other)
{
...
void
PointParallelHashGridSearcher3::serialize(std::vector<uint8_t>* buffer) const
{
  ...
  auto fbsSearcher = fbs::CreatePointParallelHashGridSearcher3(
    builder, _gridSpacing, &fbsResolution, fbsPoints, fbsKeys, fbsStartIndexTable, fbsEndIndexTable, fbsSortedIndices);
...
void
PointParallelHashGridSearcher3::deserialize(const std::vector<uint8_t>& buffer)
{
...
{
  return std::shared_ptr<PointParallelHashGridSearcher3>(new PointParallelHashGridSearcher3(_resolution, _gridSpacing),
