...
inline fbs::Size2 jetToFbs(const Size2& vec) { return fbs::Size2(vec.x, vec.y); }

inline fbs::Size3 jetToFbs(const Size3& vec) { return fbs::Size3(vec.x, vec.y, vec.z); }

inline fbs::Vector2D jetToFbs(const Vector2D& vec) { return fbs::Vector2D(vec.x, vec.y); }

inline fbs::Vector3D jetToFbs(const Vector3D& vec) { return fbs::Vector3D(vec.x, vec.y, vec.z); }

inline Size2 fbsToJet(const fbs::Size2& vec) { return Size2({vec.x(), vec.y()}); }

inline Size3 fbsToJet(const fbs::Size3& vec) { return Size3({vec.x(), vec.y(), vec.z()}); }

inline Vector2D fbsToJet(const fbs::Vector2D& vec) { return Vector2D(vec.x(), vec.y()); }

inline Vector3D fbsToJet(const fbs::Vector3D& vec) { return Vector3D(vec.x(), vec.y(), vec.z()); }
