...
#include "jet/transfer/triangle_mesh_to_sdf.h"

using namespace jet;

ImplicitTriangleMesh3::ImplicitTriangleMesh3(const TriangleMesh3Ptr& mesh, size_t resolutionX, double margin, const Transform3& transform, bool isNormalFlipped)
    ...
    if (mesh->numberOfTriangles() > 0 && mesh->numberOfPoints() > 0) {
        ...
        _customImplicitSurface = CustomImplicitSurface3::builder()
                                     .withSignedDistanceFunction([&](const Vector3D& pt) -> double { return _grid->sample(pt); })
...
Vector3D ImplicitTriangleMesh3::closestPointLocal(const Vector3D& otherPoint) const { return _customImplicitSurface->closestPoint(otherPoint); }

double ImplicitTriangleMesh3::closestDistanceLocal(const Vector3D& otherPoint) const { return _customImplicitSurface->closestDistance(otherPoint); }
...
Vector3D ImplicitTriangleMesh3::closestNormalLocal(const Vector3D& otherPoint) const { return _customImplicitSurface->closestNormal(otherPoint); }

double ImplicitTriangleMesh3::signedDistanceLocal(const Vector3D& otherPoint) const { return _customImplicitSurface->signedDistance(otherPoint); }

SurfaceRayIntersection3 ImplicitTriangleMesh3::closestIntersectionLocal(const Ray3D& ray) const { return _customImplicitSurface->closestIntersection(ray); }

ImplicitTriangleMesh3::Builder ImplicitTriangleMesh3::builder() { return ImplicitTriangleMesh3::Builder(); }
...
ImplicitTriangleMesh3 ImplicitTriangleMesh3::Builder::build() const { return ImplicitTriangleMesh3(_mesh, _resolutionX, _margin, _transform, _isNormalFlipped); }
