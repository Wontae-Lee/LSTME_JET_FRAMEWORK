...
#include "jet/primitive/surface/implicit_surface_set2.h"
...
ImplicitSurfaceSet2::ImplicitSurfaceSet2(const std::vector<ImplicitSurface2Ptr>& surfaces, const Transform2& transform, bool isNormalFlipped)
...
ImplicitSurfaceSet2::ImplicitSurfaceSet2(const std::vector<Surface2Ptr>& surfaces, const Transform2& transform, bool isNormalFlipped)
  : ImplicitSurface2(transform, isNormalFlipped)
{
...
void
ImplicitSurfaceSet2::updateQueryEngine()
{
  invalidateBvh();
  buildBvh();
}
...
bool
ImplicitSurfaceSet2::isBounded() const
{
...
bool
ImplicitSurfaceSet2::isValidGeometry() const
{
...
size_t
ImplicitSurfaceSet2::numberOfSurfaces() const
{
  return _surfaces.size();
}

const ImplicitSurface2Ptr&
ImplicitSurfaceSet2::surfaceAt(size_t i) const
{
  return _surfaces[i];
}

void
ImplicitSurfaceSet2::addExplicitSurface(const Surface2Ptr& surface)
{
  addSurface(std::make_shared<SurfaceToImplicit2>(surface));
}

void
ImplicitSurfaceSet2::addSurface(const ImplicitSurface2Ptr& surface)
{
...
Vector2D
ImplicitSurfaceSet2::closestPointLocal(const Vector2D& otherPoint) const
{
  buildBvh();

  const auto distanceFunc = [](const Surface2Ptr& surface, const Vector2D& pt) { return surface->closestDistance(pt); };
...
double
ImplicitSurfaceSet2::closestDistanceLocal(const Vector2D& otherPoint) const
{
  buildBvh();
...
Vector2D
ImplicitSurfaceSet2::closestNormalLocal(const Vector2D& otherPoint) const
{
  buildBvh();
...
bool
ImplicitSurfaceSet2::intersectsLocal(const Ray2D& ray) const
{
  buildBvh();
...
ImplicitSurfaceSet2::closestIntersectionLocal(const Ray2D& ray) const
{
  buildBvh();
...
BoundingBox2D
ImplicitSurfaceSet2::boundingBoxLocal() const
{
  buildBvh();
...
bool
ImplicitSurfaceSet2::isInsideLocal(const Vector2D& otherPoint) const
{
  for (auto surface : _surfaces) {
...
double
ImplicitSurfaceSet2::signedDistanceLocal(const Vector2D& otherPoint) const
{
  double sdf = kMaxD;
...
void
ImplicitSurfaceSet2::invalidateBvh()
{
  _bvhInvalidated = true;
}

void
ImplicitSurfaceSet2::buildBvh() const
{
  if (_bvhInvalidated) {
...
ImplicitSurfaceSet2::Builder&
ImplicitSurfaceSet2::Builder::withSurfaces(const std::vector<ImplicitSurface2Ptr>& surfaces)
{
  _surfaces = surfaces;
  return *this;
}
...
ImplicitSurfaceSet2::Builder&
ImplicitSurfaceSet2::Builder::withExplicitSurfaces(const std::vector<Surface2Ptr>& surfaces)
{
  _surfaces.clear();
  for (const auto& surface : surfaces) {
...
ImplicitSurfaceSet2
ImplicitSurfaceSet2::Builder::build() const
{
  return ImplicitSurfaceSet2(_surfaces, _transform, _isNormalFlipped);
}

ImplicitSurfaceSet2Ptr
ImplicitSurfaceSet2::Builder::makeShared() const
{
  return std::shared_ptr<ImplicitSurfaceSet2>(new ImplicitSurfaceSet2(_surfaces, _transform, _isNormalFlipped),
