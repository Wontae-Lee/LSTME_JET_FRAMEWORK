...
SurfaceSet2::SurfaceSet2(const std::vector<Surface2Ptr>& others, const Transform2& transform, bool isNormalFlipped)
...
bool
SurfaceSet2::isValidGeometry() const
{
...
size_t
SurfaceSet2::numberOfSurfaces() const
{
  return _surfaces.size();
}

const Surface2Ptr&
SurfaceSet2::surfaceAt(size_t i) const
{
  return _surfaces[i];
}

void
SurfaceSet2::addSurface(const Surface2Ptr& surface)
{
...
Vector2D
SurfaceSet2::closestPointLocal(const Vector2D& otherPoint) const
{
  buildBvh();
...
Vector2D
SurfaceSet2::closestNormalLocal(const Vector2D& otherPoint) const
{
  buildBvh();
...
double
SurfaceSet2::closestDistanceLocal(const Vector2D& otherPoint) const
{
  buildBvh();
...
bool
SurfaceSet2::intersectsLocal(const Ray2D& ray) const
{
  buildBvh();
...
SurfaceSet2::closestIntersectionLocal(const Ray2D& ray) const
{
  buildBvh();
...
BoundingBox2D
SurfaceSet2::boundingBoxLocal() const
{
  buildBvh();
...
bool
SurfaceSet2::isInsideLocal(const Vector2D& otherPoint) const
{
  for (auto surface : _surfaces) {
...
void
SurfaceSet2::invalidateBvh()
{
  _bvhInvalidated = true;
}

void
SurfaceSet2::buildBvh() const
{
  if (_bvhInvalidated) {
...
SurfaceSet2
SurfaceSet2::Builder::build() const
{
  return SurfaceSet2(_surfaces, _transform, _isNormalFlipped);
}

SurfaceSet2Ptr
SurfaceSet2::Builder::makeShared() const
{
  return std::shared_ptr<SurfaceSet2>(new SurfaceSet2(_surfaces, _transform, _isNormalFlipped), [](SurfaceSet2* obj) { delete obj; });
}
