...
#include "jet/primitive/surface/surface2.h"

#include <algorithm>

using namespace jet;

Surface2::Surface2(const Transform2& transform_, bool isNormalFlipped_) : transform(transform_), isNormalFlipped(isNormalFlipped_) {}

Surface2::Surface2(const Surface2& other) : transform(other.transform), isNormalFlipped(other.isNormalFlipped) {}

Surface2::~Surface2() {}

Vector2D Surface2::closestPoint(const Vector2D& otherPoint) const { return transform.toWorld(closestPointLocal(transform.toLocal(otherPoint))); }
...
double Surface2::closestDistance(const Vector2D& otherPoint) const { return closestDistanceLocal(transform.toLocal(otherPoint)); }
...
bool Surface2::isInside(const Vector2D& otherPoint) const { return isNormalFlipped == !isInsideLocal(transform.toLocal(otherPoint)); }
...
double Surface2::closestDistanceLocal(const Vector2D& otherPointLocal) const { return otherPointLocal.distanceTo(closestPointLocal(otherPointLocal)); }
