...
Vector2D
Surface2::closestNormal(const Vector2D& otherPoint) const
{
...
void
Surface2::updateQueryEngine()
{
  // Do nothing
}

bool
Surface2::isBounded() const
{
  return true;
}

bool
Surface2::isValidGeometry() const
{
  return true;
}

bool
Surface2::isInside(const Vector2D& otherPoint) const
{
  return isNormalFlipped == !isInsideLocal(transform.toLocal(otherPoint));
}

bool
Surface2::intersectsLocal(const Ray2D& rayLocal) const
{
...
double
Surface2::closestDistanceLocal(const Vector2D& otherPointLocal) const
{
  return otherPointLocal.distanceTo(closestPointLocal(otherPointLocal));
}

bool
Surface2::isInsideLocal(const Vector2D& otherPointLocal) const
{
