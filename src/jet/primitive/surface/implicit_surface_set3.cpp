...
ImplicitSurfaceSet3::ImplicitSurfaceSet3(const std::vector<ImplicitSurface3Ptr>& surfaces, const Transform3& transform, bool isNormalFlipped)
...
ImplicitSurfaceSet3::ImplicitSurfaceSet3(const std::vector<Surface3Ptr>& surfaces, const Transform3& transform, bool isNormalFlipped)
  : ImplicitSurface3(transform, isNormalFlipped)
{
...
void
ImplicitSurfaceSet3::updateQueryEngine()
{
  invalidateBvh();
  buildBvh();
}
...
bool
ImplicitSurfaceSet3::isBounded() const
{
...
bool
ImplicitSurfaceSet3::isValidGeometry() const
{
...
size_t
ImplicitSurfaceSet3::numberOfSurfaces() const
{
  return _surfaces.size();
}

const ImplicitSurface3Ptr&
ImplicitSurfaceSet3::surfaceAt(size_t i) const
{
  return _surfaces[i];
}

void
ImplicitSurfaceSet3::addExplicitSurface(const Surface3Ptr& surface)
{
  addSurface(std::make_shared<SurfaceToImplicit3>(surface));
}

void
ImplicitSurfaceSet3::addSurface(const ImplicitSurface3Ptr& surface)
{
...
Vector3D
ImplicitSurfaceSet3::closestPointLocal(const Vector3D& otherPoint) const
{
  buildBvh();

  const auto distanceFunc = [](const Surface3Ptr& surface, const Vector3D& pt) { return surface->closestDistance(pt); };
...
double
ImplicitSurfaceSet3::closestDistanceLocal(const Vector3D& otherPoint) const
{
  buildBvh();
...
Vector3D
ImplicitSurfaceSet3::closestNormalLocal(const Vector3D& otherPoint) const
{
  buildBvh();
...
bool
ImplicitSurfaceSet3::intersectsLocal(const Ray3D& ray) const
{
  buildBvh();
...
ImplicitSurfaceSet3::closestIntersectionLocal(const Ray3D& ray) const
{
  buildBvh();
...
BoundingBox3D
ImplicitSurfaceSet3::boundingBoxLocal() const
{
  buildBvh();
...
bool
ImplicitSurfaceSet3::isInsideLocal(const Vector3D& otherPoint) const
{
  for (auto surface : _surfaces) {
...
double
ImplicitSurfaceSet3::signedDistanceLocal(const Vector3D& otherPoint) const
{
  double sdf = kMaxD;
...
void
ImplicitSurfaceSet3::invalidateBvh()
{
  _bvhInvalidated = true;
}

void
ImplicitSurfaceSet3::buildBvh() const
{
  if (_bvhInvalidated) {
