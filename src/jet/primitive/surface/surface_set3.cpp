...
#include "jet/primitive/surface/surface_set3.h"

using namespace jet;

SurfaceSet3::SurfaceSet3() {}

SurfaceSet3::SurfaceSet3(const std::vector<Surface3Ptr>& others, const Transform3& transform, bool isNormalFlipped)
...
bool
SurfaceSet3::isValidGeometry() const
{
...
size_t
SurfaceSet3::numberOfSurfaces() const
{
  return _surfaces.size();
}

const Surface3Ptr&
SurfaceSet3::surfaceAt(size_t i) const
{
  return _surfaces[i];
}

void
SurfaceSet3::addSurface(const Surface3Ptr& surface)
{
...
Vector3D
SurfaceSet3::closestPointLocal(const Vector3D& otherPoint) const
{
  buildBvh();
...
Vector3D
SurfaceSet3::closestNormalLocal(const Vector3D& otherPoint) const
{
  buildBvh();
...
double
SurfaceSet3::closestDistanceLocal(const Vector3D& otherPoint) const
{
  buildBvh();
...
bool
SurfaceSet3::intersectsLocal(const Ray3D& ray) const
{
  buildBvh();
...
SurfaceSet3::closestIntersectionLocal(const Ray3D& ray) const
{
  buildBvh();
...
BoundingBox3D
SurfaceSet3::boundingBoxLocal() const
{
  buildBvh();
...
bool
SurfaceSet3::isInsideLocal(const Vector3D& otherPoint) const
{
  for (auto surface : _surfaces) {
...
void
SurfaceSet3::invalidateBvh()
{
  _bvhInvalidated = true;
}

void
SurfaceSet3::buildBvh() const
{
  if (_bvhInvalidated) {
...
SurfaceSet3
SurfaceSet3::Builder::build() const
{
  return SurfaceSet3(_surfaces, _transform, _isNormalFlipped);
}

SurfaceSet3Ptr
SurfaceSet3::Builder::makeShared() const
{
  return std::shared_ptr<SurfaceSet3>(new SurfaceSet3(_surfaces, _transform, _isNormalFlipped), [](SurfaceSet3* obj) { delete obj; });
}
