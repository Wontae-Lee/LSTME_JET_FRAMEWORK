...
WindingNumberGatherData postOrderTraversal(const Bvh3<size_t>& bvh, size_t nodeIndex, const GatherFunc& visitorFunc, const LeafGatherFunc& leafFunc, const WindingNumberGatherData& initGatherData) {
...
TriangleMesh3::TriangleMesh3(const Transform3& transform_, bool isNormalFlipped_) : Surface3(transform_, isNormalFlipped_) {}

TriangleMesh3::TriangleMesh3(const PointArray& points, const NormalArray& normals, const UvArray& uvs, const IndexArray& pointIndices, const IndexArray& normalIndices, const IndexArray& uvIndices,
                             const Transform3& transform_, bool isNormalFlipped_)
    : Surface3(transform_, isNormalFlipped_), _points(points), _normals(normals), _uvs(uvs), _pointIndices(pointIndices), _normalIndices(normalIndices), _uvIndices(uvIndices) {}
...
bool TriangleMesh3::isInsideLocal(const Vector3D& otherPoint) const { return fastWindingNumber(otherPoint, kDefaultFastWindingNumberAccuracy) > 0.5; }
...
void TriangleMesh3::addPointUvNormalTriangle(const Point3UI& newPointIndices, const Point3UI& newUvIndices, const Point3UI& newNormalIndices) {
    _pointIndices.append(newPointIndices);
    _normalIndices.append(newNormalIndices);
    _uvIndices.append(newUvIndices);
        ...
        for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); ++f) {
            ...
            if (fv == 3) {
                if (!attrib.vertices.empty()) {
                    addPointTriangle({shape.mesh.indices[idx].vertex_index, shape.mesh.indices[idx + 1].vertex_index, shape.mesh.indices[idx + 2].vertex_index});
                }

                if (!attrib.normals.empty()) {
                    addNormalTriangle({shape.mesh.indices[idx].normal_index, shape.mesh.indices[idx + 1].normal_index, shape.mesh.indices[idx + 2].normal_index});
                }

                if (!attrib.texcoords.empty()) {
                    addUvTriangle({shape.mesh.indices[idx].texcoord_index, shape.mesh.indices[idx + 1].texcoord_index, shape.mesh.indices[idx + 2].texcoord_index});
                }
            }
...
double TriangleMesh3::fastWindingNumber(const Vector3D& q, size_t rootNodeIndex, double accuracy) const {
    // Barill et al., Fast Winding Numbers for Soups and Clouds, ACM SIGGRAPH
    // 2018.
...
TriangleMesh3 TriangleMesh3::Builder::build() const { return TriangleMesh3(_points, _normals, _uvs, _pointIndices, _normalIndices, _uvIndices, _transform, _isNormalFlipped); }

TriangleMesh3Ptr TriangleMesh3::Builder::makeShared() const {
    return std::shared_ptr<TriangleMesh3>(new TriangleMesh3(_points, _normals, _uvs, _pointIndices, _normalIndices, _uvIndices, _transform, _isNormalFlipped), [](TriangleMesh3* obj) { delete obj; });
}
