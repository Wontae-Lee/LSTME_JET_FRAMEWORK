...
postOrderTraversal(const Bvh3<size_t>& bvh,
                   size_t nodeIndex,
...
inline std::ostream&
operator<<(std::ostream& strm, const Vector2D& v)
{
  strm << v.x << ' ' << v.y;
  return strm;
}
...
inline std::ostream&
operator<<(std::ostream& strm, const Vector3D& v)
{
  strm << v.x << ' ' << v.y << ' ' << v.z;
  return strm;
}
...
void
TriangleMesh3::updateQueryEngine()
{
  buildBvh();
  buildWindingNumbers();
}
...
void
TriangleMesh3::updateQueryEngine() const
{
  buildBvh();
  buildWindingNumbers();
}
...
Vector3D
TriangleMesh3::closestPointLocal(const Vector3D& otherPoint) const
{
  buildBvh();
...
Vector3D
TriangleMesh3::closestNormalLocal(const Vector3D& otherPoint) const
{
  buildBvh();
...
TriangleMesh3::closestIntersectionLocal(const Ray3D& ray) const
{
  buildBvh();
...
bool
TriangleMesh3::isInsideLocal(const Vector3D& otherPoint) const
{
  return fastWindingNumber(otherPoint, kDefaultFastWindingNumberAccuracy) > 0.5;
}

BoundingBox3D
TriangleMesh3::boundingBoxLocal() const
{
  buildBvh();
...
bool
TriangleMesh3::intersectsLocal(const Ray3D& ray) const
{
  buildBvh();
...
double
TriangleMesh3::closestDistanceLocal(const Vector3D& otherPoint) const
{
  buildBvh();
...
void
TriangleMesh3::set(const TriangleMesh3& other)
{
...
void
TriangleMesh3::swap(TriangleMesh3& other)
{
...
const Vector3D&
TriangleMesh3::point(size_t i) const
{
  return _points[i];
}

Vector3D&
TriangleMesh3::point(size_t i)
{
  invalidateCache();
  return _points[i];
}
...
const Vector3D&
TriangleMesh3::normal(size_t i) const
{
  return _normals[i];
}

Vector3D&
TriangleMesh3::normal(size_t i)
{
  return _normals[i];
}

const Vector2D&
TriangleMesh3::uv(size_t i) const
{
  return _uvs[i];
}

Vector2D&
TriangleMesh3::uv(size_t i)
{
  return _uvs[i];
}

const Point3UI&
TriangleMesh3::pointIndex(size_t i) const
{
  return _pointIndices[i];
}

Point3UI&
TriangleMesh3::pointIndex(size_t i)
{
  return _pointIndices[i];
}

const Point3UI&
TriangleMesh3::normalIndex(size_t i) const
{
  return _normalIndices[i];
}

Point3UI&
TriangleMesh3::normalIndex(size_t i)
{
  return _normalIndices[i];
}

const Point3UI&
TriangleMesh3::uvIndex(size_t i) const
{
  return _uvIndices[i];
}

Point3UI&
TriangleMesh3::uvIndex(size_t i)
{
  return _uvIndices[i];
}

Triangle3
TriangleMesh3::triangle(size_t i) const
{
  Triangle3 tri;
  for (int j = 0; j < 3; j++) {
...
size_t
TriangleMesh3::numberOfPoints() const
{
  return _points.size();
}

size_t
TriangleMesh3::numberOfNormals() const
{
  return _normals.size();
}

size_t
TriangleMesh3::numberOfUvs() const
{
  return _uvs.size();
}

size_t
TriangleMesh3::numberOfTriangles() const
{
  return _pointIndices.size();
}

bool
TriangleMesh3::hasNormals() const
{
  return _normals.size() > 0;
}

bool
TriangleMesh3::hasUvs() const
{
  return _uvs.size() > 0;
}

void
TriangleMesh3::addPoint(const Vector3D& pt)
{
  _points.append(pt);
}

void
TriangleMesh3::addNormal(const Vector3D& n)
{
  _normals.append(n);
}

void
TriangleMesh3::addUv(const Vector2D& t)
{
  _uvs.append(t);
}

void
TriangleMesh3::addPointTriangle(const Point3UI& newPointIndices)
{
...
void
TriangleMesh3::addNormalTriangle(const Point3UI& newNormalIndices)
{
...
void
TriangleMesh3::addUvTriangle(const Point3UI& newUvIndices)
{
...
void
TriangleMesh3::addPointNormalTriangle(const Point3UI& newPointIndices, const Point3UI& newNormalIndices)
{
  _pointIndices.append(newPointIndices);
  _normalIndices.append(newNormalIndices);
...
void
TriangleMesh3::addPointUvNormalTriangle(const Point3UI& newPointIndices, const Point3UI& newUvIndices, const Point3UI& newNormalIndices)
{
  _pointIndices.append(newPointIndices);
  _normalIndices.append(newNormalIndices);
  _uvIndices.append(newUvIndices);
...
void
TriangleMesh3::addPointUvTriangle(const Point3UI& newPointIndices, const Point3UI& newUvIndices)
{
  _pointIndices.append(newPointIndices);
  _uvIndices.append(newUvIndices);
...
void
TriangleMesh3::addTriangle(const Triangle3& tri)
{
...
void
TriangleMesh3::scale(double factor)
{
...
void
TriangleMesh3::translate(const Vector3D& t)
{
...
void
TriangleMesh3::rotate(const Quaternion<double>& q)
{
...
void
TriangleMesh3::writeObj(std::ostream* strm) const
{
  // vertex
...
bool
TriangleMesh3::writeObj(const std::string& filename) const
{
...
bool
TriangleMesh3::readObj(std::istream* strm)
{
    ...
    for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); ++f) {
      ...
      if (fv == 3) {
        if (!attrib.vertices.empty()) {
          addPointTriangle(
            { shape.mesh.indices[idx].vertex_index, shape.mesh.indices[idx + 1].vertex_index, shape.mesh.indices[idx + 2].vertex_index });
        }

        if (!attrib.normals.empty()) {
          addNormalTriangle(
            { shape.mesh.indices[idx].normal_index, shape.mesh.indices[idx + 1].normal_index, shape.mesh.indices[idx + 2].normal_index });
        }

        if (!attrib.texcoords.empty()) {
          addUvTriangle(
            { shape.mesh.indices[idx].texcoord_index, shape.mesh.indices[idx + 1].texcoord_index, shape.mesh.indices[idx + 2].texcoord_index });
        }
      }
...
bool
TriangleMesh3::readObj(const std::string& filename)
{
...
void
TriangleMesh3::buildWindingNumbers() const
{
  ...
  if (_wnInvalidated) {
    ...
    const auto leafFunc = [&](size_t nodeIndex) -> WindingNumberGatherData {
      ...
      result.areaWeightedPositionSums = area * (tri.points[0] + tri.points[1] + tri.points[2]) / 3.0;
...
double
TriangleMesh3::windingNumber(const Vector3D& queryPoint, size_t triIndex) const
{
...
double
TriangleMesh3::fastWindingNumber(const Vector3D& queryPoint, double accuracy) const
{
  buildWindingNumbers();
...
double
TriangleMesh3::fastWindingNumber(const Vector3D& q, size_t rootNodeIndex, double accuracy) const
{
  // Barill et al., Fast Winding Numbers for Soups and Clouds, ACM SIGGRAPH
  // 2018.
...
TriangleMesh3
TriangleMesh3::Builder::build() const
{
  return TriangleMesh3(_points, _normals, _uvs, _pointIndices, _normalIndices, _uvIndices, _transform, _isNormalFlipped);
}

TriangleMesh3Ptr
TriangleMesh3::Builder::makeShared() const
{
  return std::shared_ptr<TriangleMesh3>(
    new TriangleMesh3(_points, _normals, _uvs, _pointIndices, _normalIndices, _uvIndices, _transform, _isNormalFlipped),
