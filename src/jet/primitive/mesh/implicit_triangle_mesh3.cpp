  ...
  , _mesh(mesh)
{
  if (mesh->numberOfTriangles() > 0 && mesh->numberOfPoints() > 0) {
    ...
    _customImplicitSurface =
      CustomImplicitSurface3::builder().withSignedDistanceFunction([&](const Vector3D&) -> double { return kMaxD; }).makeShared();
  }
}
...
Vector3D
ImplicitTriangleMesh3::closestPointLocal(const Vector3D& otherPoint) const
{
  return _customImplicitSurface->closestPoint(otherPoint);
}

double
ImplicitTriangleMesh3::closestDistanceLocal(const Vector3D& otherPoint) const
{
  return _customImplicitSurface->closestDistance(otherPoint);
}

bool
ImplicitTriangleMesh3::intersectsLocal(const Ray3D& ray) const
{
  return _customImplicitSurface->intersects(ray);
}

BoundingBox3D
ImplicitTriangleMesh3::boundingBoxLocal() const
{
  return _mesh->boundingBox();
}

Vector3D
ImplicitTriangleMesh3::closestNormalLocal(const Vector3D& otherPoint) const
{
  return _customImplicitSurface->closestNormal(otherPoint);
}

double
ImplicitTriangleMesh3::signedDistanceLocal(const Vector3D& otherPoint) const
{
  return _customImplicitSurface->signedDistance(otherPoint);
}

SurfaceRayIntersection3
ImplicitTriangleMesh3::closestIntersectionLocal(const Ray3D& ray) const
{
...
const VertexCenteredScalarGrid3Ptr&
ImplicitTriangleMesh3::grid() const
{
  return _grid;
}
...
{
  return std::shared_ptr<ImplicitTriangleMesh3>(new ImplicitTriangleMesh3(_mesh, _resolutionX, _margin, _transform, _isNormalFlipped),
