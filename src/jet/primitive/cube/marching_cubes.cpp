...
#include "jet/primitive/cube/marching_cubes_table.h"
#include "jet/primitive/square/marching_squares_table.h"

#include "jet/physics/bounding_box/bounding_box2.h"
#include "jet/physics/bounding_box/bounding_box3.h"
#include "jet/transform/marching_cubes.h"
...
inline bool queryVertexId(const MarchingCubeVertexMap& vertexMap, MarchingCubeVertexHashKey vKey, MarchingCubeVertexId* vId) {
...
inline Vector3D grad(const ConstArrayAccessor3<double>& grid, ssize_t i, ssize_t j, ssize_t k, const Vector3D& invGridSize) {
    Vector3D ret;
    ssize_t ip = i + 1;
    ssize_t im = i - 1;
    ssize_t jp = j + 1;
...
inline size_t globalEdgeId(size_t i, size_t j, size_t k, const Size3& dim, size_t localEdgeId) {
    ...
    static const int edgeOffset3D[12][3] = {{1, 0, 0}, {2, 0, 1}, {1, 0, 2}, {0, 0, 1}, {1, 2, 0}, {2, 2, 1}, {1, 2, 2}, {0, 2, 1}, {0, 1, 0}, {2, 1, 0}, {2, 1, 2}, {0, 1, 2}};

    return ((2 * k + edgeOffset3D[localEdgeId][2]) * 2 * dim.y + (2 * j + edgeOffset3D[localEdgeId][1])) * 2 * dim.x + (2 * i + edgeOffset3D[localEdgeId][0]);
}
...
inline size_t globalVertexId(size_t i, size_t j, size_t k, const Size3& dim, size_t localVertexId) {
    ...
    return ((2 * k + vertexOffset3D[localVertexId][2]) * 2 * dim.y + (2 * j + vertexOffset3D[localVertexId][1])) * 2 * dim.x + (2 * i + vertexOffset3D[localVertexId][0]);
}

static void singleSquare(const std::array<double, 4>& data, const std::array<size_t, 8>& vertAndEdgeIds, const Vector3D& normal, const std::array<Vector3D, 4>& corners,
...
static void singleCube(const std::array<double, 8>& data, const std::array<size_t, 12>& edgeIds, const std::array<Vector3D, 8>& normals, const BoundingBox3D& bound, MarchingCubeVertexMap* vertexMap,
...
void marchingCubes(const ConstArrayAccessor3<double>& grid, const Vector3D& gridSize, const Vector3D& origin, TriangleMesh3* mesh, double isoValue, int bndClose, int bndConnectivity) {
        ...
        for (ssize_t j = 0; j < dimy - 1; ++j) {
            for (ssize_t i = 0; i < dimx - 1; ++i) {
                ...
                if (bndClose & kDirectionBack) {
                    ...
                    singleSquare(data, vertexAndEdgeIds, normal, corners, (bndConnectivity & kDirectionBack) ? &vertexMap : &vertexMapBack, mesh, isoValue);
                }
                ...
                if (bndClose & kDirectionFront) {
                    ...
                    singleSquare(data, vertexAndEdgeIds, normal, corners, (bndConnectivity & kDirectionFront) ? &vertexMap : &vertexMapFront, mesh, isoValue);
                }
            }  // i
        }      // j
    }
        ...
        for (ssize_t k = 0; k < dimz - 1; ++k) {
            for (ssize_t j = 0; j < dimy - 1; ++j) {
                ...
                if (bndClose & kDirectionLeft) {
                    ...
                    singleSquare(data, vertexAndEdgeIds, normal, corners, (bndConnectivity & kDirectionLeft) ? &vertexMap : &vertexMapLeft, mesh, isoValue);
                }
                ...
                if (bndClose & kDirectionRight) {
                    ...
                    singleSquare(data, vertexAndEdgeIds, normal, corners, (bndConnectivity & kDirectionRight) ? &vertexMap : &vertexMapRight, mesh, isoValue);
                }
            }  // j
        }      // k
    }
        ...
        for (ssize_t k = 0; k < dimz - 1; ++k) {
            for (ssize_t i = 0; i < dimx - 1; ++i) {
                ...
                if (bndClose & kDirectionDown) {
                    ...
                    singleSquare(data, vertexAndEdgeIds, normal, corners, (bndConnectivity & kDirectionDown) ? &vertexMap : &vertexMapDown, mesh, isoValue);
                }
                ...
                if (bndClose & kDirectionUp) {
                    ...
                    singleSquare(data, vertexAndEdgeIds, normal, corners, (bndConnectivity & kDirectionUp) ? &vertexMap : &vertexMapUp, mesh, isoValue);
                }
            }  // i
        }      // k
    }
