...
inline bool
queryVertexId(const MarchingCubeVertexMap& vertexMap, MarchingCubeVertexHashKey vKey, MarchingCubeVertexId* vId)
{
...
inline Vector3D
grad(const ConstArrayAccessor3<double>& grid, ssize_t i, ssize_t j, ssize_t k, const Vector3D& invGridSize)
{
  Vector3D ret;
  ssize_t ip = i + 1;
  ssize_t im = i - 1;
...
inline Vector3D
safeNormalize(const Vector3D& n)
{
...
//
inline size_t
globalEdgeId(size_t i, size_t j, size_t k, const Size3& dim, size_t localEdgeId)
{
...
//
inline size_t
globalVertexId(size_t i, size_t j, size_t k, const Size3& dim, size_t localVertexId)
{
  // See edgeConnection in marching_cubes_table.h for the edge ordering.
  static const int vertexOffset3D[8][3] = { { 0, 0, 0 }, { 2, 0, 0 }, { 2, 0, 2 }, { 0, 0, 2 }, { 0, 2, 0 }, { 2, 2, 0 }, { 2, 2, 2 }, { 0, 2, 2 } };

  return ((2 * k + vertexOffset3D[localVertexId][2]) * 2 * dim.y + (2 * j + vertexOffset3D[localVertexId][1])) * 2 * dim.x +
         (2 * i + vertexOffset3D[localVertexId][0]);
}

static void
singleSquare(const std::array<double, 4>& data,
...
static void
singleCube(const std::array<double, 8>& data,
