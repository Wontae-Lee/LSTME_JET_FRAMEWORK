...
Cylinder3::Cylinder3(const Cylinder3& other) : Surface3(other), center(other.center), radius(other.radius), height(other.height) {}
...
BoundingBox3D Cylinder3::boundingBoxLocal() const { return BoundingBox3D(center - Vector3D(radius, 0.5 * height, radius), center + Vector3D(radius, 0.5 * height, radius)); }
...
Cylinder3 Cylinder3::Builder::build() const { return Cylinder3(_center, _radius, _height, _transform, _isNormalFlipped); }
