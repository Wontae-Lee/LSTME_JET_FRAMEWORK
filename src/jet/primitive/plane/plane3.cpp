...
Plane3::Plane3(const Vector3D& normal, const Vector3D& point, const Transform3& transform_, bool isNormalFlipped_)
  : Surface3(transform_, isNormalFlipped_)
  , normal(normal)
  , point(point)
{
...
Plane3::Plane3(const Vector3D& point0, const Vector3D& point1, const Vector3D& point2, const Transform3& transform_, bool isNormalFlipped_)
  : Surface3(transform_, isNormalFlipped_)
{
...
bool
Plane3::isBounded() const
{
  return false;
}

Vector3D
Plane3::closestPointLocal(const Vector3D& otherPoint) const
{
...
Vector3D
Plane3::closestNormalLocal(const Vector3D& otherPoint) const
{
...
bool
Plane3::intersectsLocal(const Ray3D& ray) const
{
  return std::fabs(ray.direction.dot(normal)) > 0;
}

SurfaceRayIntersection3
Plane3::closestIntersectionLocal(const Ray3D& ray) const
{
...
Plane3::Builder&
Plane3::Builder::withNormal(const Vector3D& normal)
{
  _normal = normal;
  return *this;
}
...
Plane3::Builder&
Plane3::Builder::withPoint(const Vector3D& point)
{
  _point = point;
  return *this;
}
...
Plane3
Plane3::Builder::build() const
{
  return Plane3(_normal, _point, _transform, _isNormalFlipped);
}

Plane3Ptr
Plane3::Builder::makeShared() const
{
  return std::shared_ptr<Plane3>(new Plane3(_normal, _point, _transform, _isNormalFlipped), [](Plane3* obj) { delete obj; });
}
