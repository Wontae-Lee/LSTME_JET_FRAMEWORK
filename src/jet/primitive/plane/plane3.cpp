...
Plane3::Plane3(const Transform3& transform_, bool isNormalFlipped_) : Surface3(transform_, isNormalFlipped_) {}

Plane3::Plane3(const Vector3D& normal, const Vector3D& point, const Transform3& transform_, bool isNormalFlipped_) : Surface3(transform_, isNormalFlipped_), normal(normal), point(point) {}

Plane3::Plane3(const Vector3D& point0, const Vector3D& point1, const Vector3D& point2, const Transform3& transform_, bool isNormalFlipped_) : Surface3(transform_, isNormalFlipped_) {
    normal = (point1 - point0).cross(point2 - point0).normalized();
    point = point0;
}

Plane3::Plane3(const Plane3& other) : Surface3(other), normal(other.normal), point(other.point) {}
