...
Plane2::Plane2(const Vector2D& normal_, const Vector2D& point_, const Transform2& transform_, bool isNormalFlipped_)
  : Surface2(transform_, isNormalFlipped_)
  , normal(normal_)
  , point(point_)
{
...
bool
Plane2::isBounded() const
{
  return false;
}

Vector2D
Plane2::closestPointLocal(const Vector2D& otherPoint) const
{
...
Vector2D
Plane2::closestNormalLocal(const Vector2D& otherPoint) const
{
...
bool
Plane2::intersectsLocal(const Ray2D& ray) const
{
  return std::fabs(ray.direction.dot(normal)) > 0;
}

SurfaceRayIntersection2
Plane2::closestIntersectionLocal(const Ray2D& ray) const
{
...
Plane2::Builder&
Plane2::Builder::withNormal(const Vector2D& normal)
{
  _normal = normal;
  return *this;
}
...
Plane2::Builder&
Plane2::Builder::withPoint(const Vector2D& point)
{
  _point = point;
  return *this;
}
...
Plane2
Plane2::Builder::build() const
{
  return Plane2(_normal, _point, _transform, _isNormalFlipped);
}

Plane2Ptr
Plane2::Builder::makeShared() const
{
  return std::shared_ptr<Plane2>(new Plane2(_normal, _point, _transform, _isNormalFlipped), [](Plane2* obj) { delete obj; });
}
