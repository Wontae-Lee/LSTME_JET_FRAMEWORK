...
const Vector2D&
CollocatedVectorGrid2::operator()(size_t i, size_t j) const
{
  return _data(i, j);
}

Vector2D&
CollocatedVectorGrid2::operator()(size_t i, size_t j)
{
  return _data(i, j);
}

double
CollocatedVectorGrid2::divergenceAtDataPoint(size_t i, size_t j) const
{
...
double
CollocatedVectorGrid2::curlAtDataPoint(size_t i, size_t j) const
{
  const Size2 ds = _data.size();
  const Vector2D& gs = gridSpacing();
...
Vector2D
CollocatedVectorGrid2::sample(const Vector2D& x) const
{
  return _sampler(x);
}

double
CollocatedVectorGrid2::divergence(const Vector2D& x) const
{
...
double
CollocatedVectorGrid2::curl(const Vector2D& x) const
{
  std::array<Point2UI, 4> indices;
  std::array<double, 4> weights;
  _linearSampler.getCoordinatesAndWeights(x, &indices, &weights);
...
VectorGrid2::VectorDataAccessor
CollocatedVectorGrid2::dataAccessor()
{
...
CollocatedVectorGrid2::dataPosition() const
{
  Vector2D dataOrigin_ = dataOrigin();
  return [this, dataOrigin_](size_t i, size_t j) -> Vector2D { return dataOrigin_ + gridSpacing() * Vector2D({ i, j }); };
}

void
CollocatedVectorGrid2::forEachDataPointIndex(const std::function<void(size_t, size_t)>& func) const
{
  _data.forEachIndex(func);
}

void
CollocatedVectorGrid2::parallelForEachDataPointIndex(const std::function<void(size_t, size_t)>& func) const
{
  _data.parallelForEachIndex(func);
}

void
CollocatedVectorGrid2::swapCollocatedVectorGrid(CollocatedVectorGrid2* other)
{
  swapGrid(other);
...
void
CollocatedVectorGrid2::setCollocatedVectorGrid(const CollocatedVectorGrid2& other)
{
  setGrid(other);
...
void
CollocatedVectorGrid2::onResize(const Size2& resolution, const Vector2D& gridSpacing, const Vector2D& origin, const Vector2D& initialValue)
{
...
void
CollocatedVectorGrid2::resetSampler()
{
...
void
CollocatedVectorGrid2::getData(std::vector<double>* data) const
{
...
void
CollocatedVectorGrid2::setData(const std::vector<double>& data)
{
