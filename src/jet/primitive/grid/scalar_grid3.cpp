...
void
ScalarGrid3::resize(size_t resolutionX,
                    size_t resolutionY,
                    size_t resolutionZ,
                    double gridSpacingX,
                    double gridSpacingY,
...
void
ScalarGrid3::resize(const Size3& resolution, const Vector3D& gridSpacing, const Vector3D& origin, double initialValue)
{
...
void
ScalarGrid3::resize(double gridSpacingX, double gridSpacingY, double gridSpacingZ, double originX, double originY, double originZ)
{
  resize(Vector3D(gridSpacingX, gridSpacingY, gridSpacingZ), Vector3D(originX, originY, originZ));
}

void
ScalarGrid3::resize(const Vector3D& gridSpacing, const Vector3D& origin)
{
  resize(resolution(), gridSpacing, origin);
}

const double&
ScalarGrid3::operator()(size_t i, size_t j, size_t k) const
{
  return _data(i, j, k);
}

double&
ScalarGrid3::operator()(size_t i, size_t j, size_t k)
{
  return _data(i, j, k);
}

Vector3D
ScalarGrid3::gradientAtDataPoint(size_t i, size_t j, size_t k) const
{
  return gradient3(_data.constAccessor(), gridSpacing(), i, j, k);
}

double
ScalarGrid3::laplacianAtDataPoint(size_t i, size_t j, size_t k) const
{
  return laplacian3(_data.constAccessor(), gridSpacing(), i, j, k);
}

double
ScalarGrid3::sample(const Vector3D& x) const
{
  return _sampler(x);
}

std::function<double(const Vector3D&)>
ScalarGrid3::sampler() const
{
  return _sampler;
}

Vector3D
ScalarGrid3::gradient(const Vector3D& x) const
{
...
double
ScalarGrid3::laplacian(const Vector3D& x) const
{
  std::array<Point3UI, 8> indices;
  std::array<double, 8> weights;
  _linearSampler.getCoordinatesAndWeights(x, &indices, &weights);
...
ScalarGrid3::dataAccessor()
{
  return _data.accessor();
}

ScalarGrid3::ConstScalarDataAccessor
ScalarGrid3::constDataAccessor() const
{
  return _data.constAccessor();
}

ScalarGrid3::DataPositionFunc
ScalarGrid3::dataPosition() const
{
  Vector3D o = dataOrigin();
  return [this, o](size_t i, size_t j, size_t k) -> Vector3D { return o + gridSpacing() * Vector3D({ i, j, k }); };
}

void
ScalarGrid3::fill(double value, ExecutionPolicy policy)
{
  parallelFor(
    kZeroSize,
    _data.width(),
...
void
ScalarGrid3::fill(const std::function<double(const Vector3D&)>& func, ExecutionPolicy policy)
{
  ...
  parallelFor(
    ...
    kZeroSize,
    _data.depth(),
    [this, &func, &pos](size_t i, size_t j, size_t k) { _data(i, j, k) = func(pos(i, j, k)); },
    policy);
}

void
ScalarGrid3::forEachDataPointIndex(const std::function<void(size_t, size_t, size_t)>& func) const
{
  _data.forEachIndex(func);
}

void
ScalarGrid3::parallelForEachDataPointIndex(const std::function<void(size_t, size_t, size_t)>& func) const
{
  _data.parallelForEachIndex(func);
}

void
ScalarGrid3::serialize(std::vector<uint8_t>* buffer) const
{
...
void
ScalarGrid3::deserialize(const std::vector<uint8_t>& buffer)
{
...
void
ScalarGrid3::swapScalarGrid(ScalarGrid3* other)
{
  swapGrid(other);
...
void
ScalarGrid3::setScalarGrid(const ScalarGrid3& other)
{
  setGrid(other);
...
void
ScalarGrid3::getData(std::vector<double>* data) const
{
...
void
ScalarGrid3::setData(const std::vector<double>& data)
{
