...
void ScalarGrid3::resize(size_t resolutionX, size_t resolutionY, size_t resolutionZ, double gridSpacingX, double gridSpacingY, double gridSpacingZ, double originX, double originY, double originZ,
                         double initialValue) {
    resize(Size3(resolutionX, resolutionY, resolutionZ), Vector3D(gridSpacingX, gridSpacingY, gridSpacingZ), Vector3D(originX, originY, originZ), initialValue);
}

void ScalarGrid3::resize(const Size3& resolution, const Vector3D& gridSpacing, const Vector3D& origin, double initialValue) {
...
void ScalarGrid3::resize(double gridSpacingX, double gridSpacingY, double gridSpacingZ, double originX, double originY, double originZ) {
    resize(Vector3D(gridSpacingX, gridSpacingY, gridSpacingZ), Vector3D(originX, originY, originZ));
}

void ScalarGrid3::resize(const Vector3D& gridSpacing, const Vector3D& origin) { resize(resolution(), gridSpacing, origin); }
...
Vector3D ScalarGrid3::gradientAtDataPoint(size_t i, size_t j, size_t k) const { return gradient3(_data.constAccessor(), gridSpacing(), i, j, k); }

double ScalarGrid3::laplacianAtDataPoint(size_t i, size_t j, size_t k) const { return laplacian3(_data.constAccessor(), gridSpacing(), i, j, k); }
...
void ScalarGrid3::fill(double value, ExecutionPolicy policy) {
    parallelFor(
        kZeroSize, _data.width(), kZeroSize, _data.height(), kZeroSize, _data.depth(), [this, value](size_t i, size_t j, size_t k) { _data(i, j, k) = value; }, policy);
}
...
void ScalarGrid3::fill(const std::function<double(const Vector3D&)>& func, ExecutionPolicy policy) {
    ...
    parallelFor(
        kZeroSize, _data.width(), kZeroSize, _data.height(), kZeroSize, _data.depth(), [this, &func, &pos](size_t i, size_t j, size_t k) { _data(i, j, k) = func(pos(i, j, k)); }, policy);
}

void ScalarGrid3::forEachDataPointIndex(const std::function<void(size_t, size_t, size_t)>& func) const { _data.forEachIndex(func); }

void ScalarGrid3::parallelForEachDataPointIndex(const std::function<void(size_t, size_t, size_t)>& func) const { _data.parallelForEachIndex(func); }
