...
CustomScalarField3::CustomScalarField3(const std::function<double(const Vector3D&)>& customFunction, double derivativeResolution)
    : _customFunction(customFunction), _resolution(derivativeResolution) {}

CustomScalarField3::CustomScalarField3(const std::function<double(const Vector3D&)>& customFunction, const std::function<Vector3D(const Vector3D&)>& customGradientFunction,
...
double CustomScalarField3::sample(const Vector3D& x) const { return _customFunction(x); }

std::function<double(const Vector3D&)> CustomScalarField3::sampler() const { return _customFunction; }
...
CustomScalarField3Ptr CustomScalarField3::Builder::makeShared() const {
    if (_customLaplacianFunction) {
        return std::shared_ptr<CustomScalarField3>(new CustomScalarField3(_customFunction, _customGradientFunction, _customLaplacianFunction), [](CustomScalarField3* obj) { delete obj; });
    } else {
        return std::shared_ptr<CustomScalarField3>(new CustomScalarField3(_customFunction, _customGradientFunction, _resolution), [](CustomScalarField3* obj) { delete obj; });
    }
}
