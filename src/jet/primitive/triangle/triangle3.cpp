...
inline Vector3D
closestPointOnLine(const Vector3D& v0, const Vector3D& v1, const Vector3D& pt)
{
...
inline Vector3D
closestNormalOnLine(const Vector3D& v0, const Vector3D& v1, const Vector3D& n0, const Vector3D& n1, const Vector3D& pt)
{
  const double lenSquared = (v1 - v0).lengthSquared();
  if (lenSquared < std::numeric_limits<double>::epsilon()) {
    return n0;
...
Vector3D
Triangle3::closestPointLocal(const Vector3D& otherPoint) const
{
...
Vector3D
Triangle3::closestNormalLocal(const Vector3D& otherPoint) const
{
  Vector3D n = faceNormal();
  double nd = n.dot(n);
  double d = n.dot(points[0]);
...
bool
Triangle3::intersectsLocal(const Ray3D& ray) const
{
  Vector3D n = faceNormal();
...
Triangle3::closestIntersectionLocal(const Ray3D& ray) const
{
...
BoundingBox3D
Triangle3::boundingBoxLocal() const
{
...
double
Triangle3::area() const
{
  return 0.5 * (points[1] - points[0]).cross(points[2] - points[0]).length();
}

void
Triangle3::getBarycentricCoords(const Vector3D& pt, double* b0, double* b1, double* b2) const
{
...
void
Triangle3::setNormalsToFaceNormal()
{
...
Triangle3
Triangle3::Builder::build() const
{
  return Triangle3(_points, _normals, _uvs, _transform, _isNormalFlipped);
}

Triangle3Ptr
Triangle3::Builder::makeShared() const
{
  return std::shared_ptr<Triangle3>(new Triangle3(_points, _normals, _uvs, _transform, _isNormalFlipped), [](Triangle3* obj) { delete obj; });
}
