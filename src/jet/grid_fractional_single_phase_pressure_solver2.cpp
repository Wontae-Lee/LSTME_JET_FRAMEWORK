...
void buildSingleSystem(FdmMatrix2* A, FdmVector2* b, const Array2<float>& fluidSdf, const Array2<float>& uWeights, const Array2<float>& vWeights, std::function<Vector2D(const Vector2D&)> boundaryVel,
    ...
    A->parallelForEachIndex([&](size_t i, size_t j) {
        ...
        if (isInsideSdf(centerPhi)) {
            ...
            double boundaryContribution = (1.0 - uWeights(i + 1, j)) * boundaryVel(uPos(i + 1, j)).x * invH.x - (1.0 - uWeights(i, j)) * boundaryVel(uPos(i, j)).x * invH.x +
                                          (1.0 - vWeights(i, j + 1)) * boundaryVel(vPos(i, j + 1)).y * invH.y - (1.0 - vWeights(i, j)) * boundaryVel(vPos(i, j)).y * invH.y;
...
void buildSingleSystem(MatrixCsrD* A, VectorND* x, VectorND* b, const Array2<float>& fluidSdf, const Array2<float>& uWeights, const Array2<float>& vWeights,
                       std::function<Vector2D(const Vector2D&)> boundaryVel, const FaceCenteredGrid2& input) {
    const Size2 size = input.resolution();
    const auto uPos = input.uPosition();
    const auto vPos = input.vPosition();
    ...
    fluidSdf.forEachIndex([&](size_t i, size_t j) {
        ...
        if (isInsideSdf(centerPhi)) {
            ...
            // Accumulate contributions from the moving boundary
            const double boundaryContribution = (1.0 - uWeights(i + 1, j)) * boundaryVel(uPos(i + 1, j)).x * invH.x - (1.0 - uWeights(i, j)) * boundaryVel(uPos(i, j)).x * invH.x +
                                                (1.0 - vWeights(i, j + 1)) * boundaryVel(vPos(i, j + 1)).y * invH.y - (1.0 - vWeights(i, j)) * boundaryVel(vPos(i, j)).y * invH.y;
...
GridFractionalSinglePhasePressureSolver2::GridFractionalSinglePhasePressureSolver2() { _systemSolver = std::make_shared<FdmIccgSolver2>(100, kDefaultTolerance); }

GridFractionalSinglePhasePressureSolver2::~GridFractionalSinglePhasePressureSolver2() {}

void GridFractionalSinglePhasePressureSolver2::solve(const FaceCenteredGrid2& input, double timeIntervalInSeconds, FaceCenteredGrid2* output, const ScalarField2& boundarySdf,
                                                     const VectorField2& boundaryVelocity, const ScalarField2& fluidSdf, bool useCompressed) {
...
GridBoundaryConditionSolver2Ptr GridFractionalSinglePhasePressureSolver2::suggestedBoundaryConditionSolver() const { return std::make_shared<GridFractionalBoundaryConditionSolver2>(); }

const FdmLinearSystemSolver2Ptr& GridFractionalSinglePhasePressureSolver2::linearSystemSolver() const { return _systemSolver; }
...
void GridFractionalSinglePhasePressureSolver2::buildWeights(const FaceCenteredGrid2& input, const ScalarField2& boundarySdf, const VectorField2& boundaryVelocity, const ScalarField2& fluidSdf) {
    ...
    _fluidSdf[0].parallelForEachIndex([&](size_t i, size_t j) { _fluidSdf[0](i, j) = static_cast<float>(fluidSdf.sample(cellPos(i, j))); });
...
void GridFractionalSinglePhasePressureSolver2::buildSystem(const FaceCenteredGrid2& input, bool useCompressed) {
    ...
    if (_mgSystemSolver == nullptr) {
        if (useCompressed) {
            buildSingleSystem(&_compSystem.A, &_compSystem.x, &_compSystem.b, _fluidSdf[0], _uWeights[0], _vWeights[0], _boundaryVel, *finer);
        } else {
            buildSingleSystem(&_system.A, &_system.b, _fluidSdf[0], _uWeights[0], _vWeights[0], _boundaryVel, *finer);
        }
    } else {
        buildSingleSystem(&_mgSystem.A.levels.front(), &_mgSystem.b.levels.front(), _fluidSdf[0], _uWeights[0], _vWeights[0], _boundaryVel, *finer);
    }
    ...
    for (size_t l = 1; l < numLevels; ++l) {
        ...
        buildSingleSystem(&_mgSystem.A.levels[l], &_mgSystem.b.levels[l], _fluidSdf[l], _uWeights[l], _vWeights[l], _boundaryVel, coarser);
...
void GridFractionalSinglePhasePressureSolver2::applyPressureGradient(const FaceCenteredGrid2& input, FaceCenteredGrid2* output) {
    ...
    x.parallelForEachIndex([&](size_t i, size_t j) {
        ...
        if (i + 1 < size.x && _uWeights[0](i + 1, j) > 0.0 && (isInsideSdf(centerPhi) || isInsideSdf(_fluidSdf[0](i + 1, j)))) {
        ...
        if (j + 1 < size.y && _vWeights[0](i, j + 1) > 0.0 && (isInsideSdf(centerPhi) || isInsideSdf(_fluidSdf[0](i, j + 1)))) {
