...
GridBackwardEulerDiffusionSolver3::GridBackwardEulerDiffusionSolver3(BoundaryType boundaryType) : _boundaryType(boundaryType) { _systemSolver = std::make_shared<FdmIccgSolver3>(100, kEpsilonD); }

void GridBackwardEulerDiffusionSolver3::solve(const ScalarGrid3& source, double diffusionCoefficient, double timeIntervalInSeconds, ScalarGrid3* dest, const ScalarField3& boundarySdf,
    ...
    if (_systemSolver != nullptr) {
        ...
        // Assign the solution
        source.parallelForEachDataPointIndex([&](size_t i, size_t j, size_t k) { (*dest)(i, j, k) = _system.x(i, j, k); });
    }
}

void GridBackwardEulerDiffusionSolver3::solve(const CollocatedVectorGrid3& source, double diffusionCoefficient, double timeIntervalInSeconds, CollocatedVectorGrid3* dest,
    ...
    if (_systemSolver != nullptr) {
        ...
        // Assign the solution
        source.parallelForEachDataPointIndex([&](size_t i, size_t j, size_t k) { (*dest)(i, j, k).x = _system.x(i, j, k); });
    }
    ...
    if (_systemSolver != nullptr) {
        ...
        // Assign the solution
        source.parallelForEachDataPointIndex([&](size_t i, size_t j, size_t k) { (*dest)(i, j, k).y = _system.x(i, j, k); });
    }
    ...
    if (_systemSolver != nullptr) {
        ...
        // Assign the solution
        source.parallelForEachDataPointIndex([&](size_t i, size_t j, size_t k) { (*dest)(i, j, k).z = _system.x(i, j, k); });
    }
}

void GridBackwardEulerDiffusionSolver3::solve(const FaceCenteredGrid3& source, double diffusionCoefficient, double timeIntervalInSeconds, FaceCenteredGrid3* dest, const ScalarField3& boundarySdf,
                                              const ScalarField3& fluidSdf) {
    Vector3D h = source.gridSpacing();
    Vector3D c = timeIntervalInSeconds * diffusionCoefficient / (h * h);
    ...
    if (_systemSolver != nullptr) {
        ...
        // Assign the solution
        source.parallelForEachUIndex([&](size_t i, size_t j, size_t k) { dest->u(i, j, k) = _system.x(i, j, k); });
    }
    ...
    if (_systemSolver != nullptr) {
        ...
        // Assign the solution
        source.parallelForEachVIndex([&](size_t i, size_t j, size_t k) { dest->v(i, j, k) = _system.x(i, j, k); });
    }
    ...
    if (_systemSolver != nullptr) {
        ...
        // Assign the solution
        source.parallelForEachWIndex([&](size_t i, size_t j, size_t k) { dest->w(i, j, k) = _system.x(i, j, k); });
    }
}

void GridBackwardEulerDiffusionSolver3::setLinearSystemSolver(const FdmLinearSystemSolver3Ptr& solver) { _systemSolver = solver; }

void GridBackwardEulerDiffusionSolver3::buildMarkers(const Size3& size, const std::function<Vector3D(size_t, size_t, size_t)>& pos, const ScalarField3& boundarySdf, const ScalarField3& fluidSdf) {
...
void GridBackwardEulerDiffusionSolver3::buildMatrix(const Size3& size, const Vector3D& c) {
    ...
    _system.A.parallelForEachIndex([&](size_t i, size_t j, size_t k) {
        ...
        if (_markers(i, j, k) == kFluid) {
            if (i + 1 < size.x) {
                if ((isDirichlet && _markers(i + 1, j, k) != kAir) || _markers(i + 1, j, k) == kFluid) {
                    row.center += c.x;
                }
            ...
            if (i > 0 && ((isDirichlet && _markers(i - 1, j, k) != kAir) || _markers(i - 1, j, k) == kFluid)) {
                row.center += c.x;
            }

            if (j + 1 < size.y) {
                if ((isDirichlet && _markers(i, j + 1, k) != kAir) || _markers(i, j + 1, k) == kFluid) {
                    row.center += c.y;
                }
            ...
            if (j > 0 && ((isDirichlet && _markers(i, j - 1, k) != kAir) || _markers(i, j - 1, k) == kFluid)) {
                row.center += c.y;
            }

            if (k + 1 < size.z) {
                if ((isDirichlet && _markers(i, j, k + 1) != kAir) || _markers(i, j, k + 1) == kFluid) {
                    row.center += c.z;
                }
            ...
            if (k > 0 && ((isDirichlet && _markers(i, j, k - 1) != kAir) || _markers(i, j, k - 1) == kFluid)) {
                row.center += c.z;
            }
        }
    });
