...
void
VolumeGridEmitter2::addSignedDistanceTarget(const ScalarGrid2Ptr& scalarGridTarget)
{
  auto mapper = [](double sdf, const Vector2D&, double oldVal) { return std::min(oldVal, sdf); };
  addTarget(scalarGridTarget, mapper);
}

void
VolumeGridEmitter2::addStepFunctionTarget(const ScalarGrid2Ptr& scalarGridTarget, double minValue, double maxValue)
{
...
void
VolumeGridEmitter2::addTarget(const ScalarGrid2Ptr& scalarGridTarget, const ScalarMapper& customMapper)
{
  _customScalarTargets.emplace_back(scalarGridTarget, customMapper);
}

void
VolumeGridEmitter2::addTarget(const VectorGrid2Ptr& vectorGridTarget, const VectorMapper& customMapper)
{
  _customVectorTargets.emplace_back(vectorGridTarget, customMapper);
}

const ImplicitSurface2Ptr&
VolumeGridEmitter2::sourceRegion() const
{
  return _sourceRegion;
}

bool
VolumeGridEmitter2::isOneShot() const
{
  return _isOneShot;
}

void
VolumeGridEmitter2::onUpdate(double currentTimeInSeconds, double timeIntervalInSeconds)
{
...
VolumeGridEmitter2::Builder&
VolumeGridEmitter2::Builder::withSourceRegion(const Surface2Ptr& sourceRegion)
{
...
VolumeGridEmitter2
VolumeGridEmitter2::Builder::build() const
{
  return VolumeGridEmitter2(_sourceRegion, _isOneShot);
}

VolumeGridEmitter2Ptr
VolumeGridEmitter2::Builder::makeShared() const
{
  return std::shared_ptr<VolumeGridEmitter2>(new VolumeGridEmitter2(_sourceRegion, _isOneShot), [](VolumeGridEmitter2* obj) { delete obj; });
}
