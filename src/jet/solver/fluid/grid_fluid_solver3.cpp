...
#include "jet/solver/grid/grid_fluid_solver3.h"
...
GridFluidSolver3::GridFluidSolver3(const Size3& resolution, const Vector3D& gridSpacing, const Vector3D& gridOrigin)
{
...
const Vector3D&
GridFluidSolver3::gravity() const
{
  return _gravity;
}

void
GridFluidSolver3::setGravity(const Vector3D& newGravity)
{
  _gravity = newGravity;
}

double
GridFluidSolver3::viscosityCoefficient() const
{
  return _viscosityCoefficient;
}

void
GridFluidSolver3::setViscosityCoefficient(double newValue)
{
  _viscosityCoefficient = std::max(newValue, 0.0);
}

double
GridFluidSolver3::cfl(double timeIntervalInSeconds) const
{
...
double
GridFluidSolver3::maxCfl() const
{
  return _maxCfl;
}

void
GridFluidSolver3::setMaxCfl(double newCfl)
{
  _maxCfl = std::max(newCfl, kEpsilonD);
}

bool
GridFluidSolver3::useCompressedLinearSystem() const
{
  return _useCompressedLinearSys;
}

void
GridFluidSolver3::setUseCompressedLinearSystem(bool onoff)
{
  _useCompressedLinearSys = onoff;
}

const AdvectionSolver3Ptr&
GridFluidSolver3::advectionSolver() const
{
  return _advectionSolver;
}

void
GridFluidSolver3::setAdvectionSolver(const AdvectionSolver3Ptr& newSolver)
{
  _advectionSolver = newSolver;
}

const GridDiffusionSolver3Ptr&
GridFluidSolver3::diffusionSolver() const
{
  return _diffusionSolver;
}

void
GridFluidSolver3::setDiffusionSolver(const GridDiffusionSolver3Ptr& newSolver)
{
  _diffusionSolver = newSolver;
}

const GridPressureSolver3Ptr&
GridFluidSolver3::pressureSolver() const
{
  return _pressureSolver;
}

void
GridFluidSolver3::setPressureSolver(const GridPressureSolver3Ptr& newSolver)
{
...
int
GridFluidSolver3::closedDomainBoundaryFlag() const
{
  return _closedDomainBoundaryFlag;
}

void
GridFluidSolver3::setClosedDomainBoundaryFlag(int flag)
{
  _closedDomainBoundaryFlag = flag;
  _boundaryConditionSolver->setClosedDomainBoundaryFlag(_closedDomainBoundaryFlag);
}

const GridSystemData3Ptr&
GridFluidSolver3::gridSystemData() const
{
  return _grids;
}

void
GridFluidSolver3::resizeGrid(const Size3& newSize, const Vector3D& newGridSpacing, const Vector3D& newGridOrigin)
{
  _grids->resize(newSize, newGridSpacing, newGridOrigin);
}

Size3
GridFluidSolver3::resolution() const
{
  return _grids->resolution();
}

Vector3D
GridFluidSolver3::gridSpacing() const
{
  return _grids->gridSpacing();
}

Vector3D
GridFluidSolver3::gridOrigin() const
{
  return _grids->origin();
}

const FaceCenteredGrid3Ptr&
GridFluidSolver3::velocity() const
{
  return _grids->velocity();
}

const Collider3Ptr&
GridFluidSolver3::collider() const
{
  return _collider;
}

void
GridFluidSolver3::setCollider(const Collider3Ptr& newCollider)
{
  _collider = newCollider;
}

const GridEmitter3Ptr&
GridFluidSolver3::emitter() const
{
  return _emitter;
}

void
GridFluidSolver3::setEmitter(const GridEmitter3Ptr& newEmitter)
{
  _emitter = newEmitter;
}
...
void
GridFluidSolver3::onAdvanceTimeStep(double timeIntervalInSeconds)
{
...
unsigned int
GridFluidSolver3::numberOfSubTimeSteps(double timeIntervalInSeconds) const
{
...
void
GridFluidSolver3::onBeginAdvanceTimeStep(double timeIntervalInSeconds)
{
  UNUSED_VARIABLE(timeIntervalInSeconds);
}

void
GridFluidSolver3::onEndAdvanceTimeStep(double timeIntervalInSeconds)
{
  UNUSED_VARIABLE(timeIntervalInSeconds);
}

void
GridFluidSolver3::computeExternalForces(double timeIntervalInSeconds)
{
  computeGravity(timeIntervalInSeconds);
}

void
GridFluidSolver3::computeViscosity(double timeIntervalInSeconds)
{
...
void
GridFluidSolver3::computePressure(double timeIntervalInSeconds)
{
  if (_pressureSolver != nullptr) {
    auto vel = velocity();
    auto vel0 = std::dynamic_pointer_cast<FaceCenteredGrid3>(vel->clone());
...
void
GridFluidSolver3::computeAdvection(double timeIntervalInSeconds)
{
  auto vel = velocity();
...
ScalarField3Ptr
GridFluidSolver3::fluidSdf() const
{
  return std::make_shared<ConstantScalarField3>(-kMaxD);
}

void
GridFluidSolver3::computeGravity(double timeIntervalInSeconds)
{
  if (_gravity.lengthSquared() > kEpsilonD) {
    ...
    if (std::abs(_gravity.x) > kEpsilonD) {
      vel->forEachUIndex([&](size_t i, size_t j, size_t k) { u(i, j, k) += timeIntervalInSeconds * _gravity.x; });
    }
...
void
GridFluidSolver3::applyBoundaryCondition()
{
  auto vel = _grids->velocity();
...
void
GridFluidSolver3::extrapolateIntoCollider(ScalarGrid3* grid)
{
...
void
GridFluidSolver3::extrapolateIntoCollider(CollocatedVectorGrid3* grid)
{
  Array3<char> marker(grid->dataSize());
  auto pos = grid->dataPosition();
  marker.parallelForEachIndex([&](size_t i, size_t j, size_t k) {
...
void
GridFluidSolver3::extrapolateIntoCollider(FaceCenteredGrid3* grid)
{
...
ScalarField3Ptr
GridFluidSolver3::colliderSdf() const
{
  return _boundaryConditionSolver->colliderSdf();
}

VectorField3Ptr
GridFluidSolver3::colliderVelocityField() const
{
  return _boundaryConditionSolver->colliderVelocityField();
}

void
GridFluidSolver3::beginAdvanceTimeStep(double timeIntervalInSeconds)
{
...
void
GridFluidSolver3::endAdvanceTimeStep(double timeIntervalInSeconds)
{
  // Invoke callback
...
void
GridFluidSolver3::updateCollider(double timeIntervalInSeconds)
{
...
void
GridFluidSolver3::updateEmitter(double timeIntervalInSeconds)
{
  if (_emitter != nullptr) {
