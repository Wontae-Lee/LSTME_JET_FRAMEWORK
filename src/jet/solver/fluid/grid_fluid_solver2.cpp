...
#include "jet/solver/advection/cubic_semi_lagrangian2.h"
#include "jet/solver/grid/grid_backward_euler_diffusion_solver2.h"
#include "jet/solver/grid/grid_blocked_boundary_condition_solver2.h"
#include "jet/solver/grid/grid_fluid_solver2.h"
...
GridFluidSolver2::GridFluidSolver2(const Size2& resolution, const Vector2D& gridSpacing, const Vector2D& gridOrigin)
{
...
const Vector2D&
GridFluidSolver2::gravity() const
{
  return _gravity;
}

void
GridFluidSolver2::setGravity(const Vector2D& newGravity)
{
  _gravity = newGravity;
}

double
GridFluidSolver2::viscosityCoefficient() const
{
  return _viscosityCoefficient;
}

void
GridFluidSolver2::setViscosityCoefficient(double newValue)
{
  _viscosityCoefficient = std::max(newValue, 0.0);
}

double
GridFluidSolver2::cfl(double timeIntervalInSeconds) const
{
...
double
GridFluidSolver2::maxCfl() const
{
  return _maxCfl;
}

void
GridFluidSolver2::setMaxCfl(double newCfl)
{
  _maxCfl = std::max(newCfl, kEpsilonD);
}

bool
GridFluidSolver2::useCompressedLinearSystem() const
{
  return _useCompressedLinearSys;
}

void
GridFluidSolver2::setUseCompressedLinearSystem(bool onoff)
{
  _useCompressedLinearSys = onoff;
}

const AdvectionSolver2Ptr&
GridFluidSolver2::advectionSolver() const
{
  return _advectionSolver;
}

void
GridFluidSolver2::setAdvectionSolver(const AdvectionSolver2Ptr& newSolver)
{
  _advectionSolver = newSolver;
}

const GridDiffusionSolver2Ptr&
GridFluidSolver2::diffusionSolver() const
{
  return _diffusionSolver;
}

void
GridFluidSolver2::setDiffusionSolver(const GridDiffusionSolver2Ptr& newSolver)
{
  _diffusionSolver = newSolver;
}

const GridPressureSolver2Ptr&
GridFluidSolver2::pressureSolver() const
{
  return _pressureSolver;
}

void
GridFluidSolver2::setPressureSolver(const GridPressureSolver2Ptr& newSolver)
{
...
int
GridFluidSolver2::closedDomainBoundaryFlag() const
{
  return _closedDomainBoundaryFlag;
}

void
GridFluidSolver2::setClosedDomainBoundaryFlag(int flag)
{
  _closedDomainBoundaryFlag = flag;
  _boundaryConditionSolver->setClosedDomainBoundaryFlag(_closedDomainBoundaryFlag);
}

const GridSystemData2Ptr&
GridFluidSolver2::gridSystemData() const
{
  return _grids;
}

void
GridFluidSolver2::resizeGrid(const Size2& newSize, const Vector2D& newGridSpacing, const Vector2D& newGridOrigin)
{
  _grids->resize(newSize, newGridSpacing, newGridOrigin);
}

Size2
GridFluidSolver2::resolution() const
{
  return _grids->resolution();
}

Vector2D
GridFluidSolver2::gridSpacing() const
{
  return _grids->gridSpacing();
}

Vector2D
GridFluidSolver2::gridOrigin() const
{
  return _grids->origin();
}

const FaceCenteredGrid2Ptr&
GridFluidSolver2::velocity() const
{
  return _grids->velocity();
}

const Collider2Ptr&
GridFluidSolver2::collider() const
{
  return _collider;
}

void
GridFluidSolver2::setCollider(const Collider2Ptr& newCollider)
{
  _collider = newCollider;
}

const GridEmitter2Ptr&
GridFluidSolver2::emitter() const
{
  return _emitter;
}

void
GridFluidSolver2::setEmitter(const GridEmitter2Ptr& newEmitter)
{
  _emitter = newEmitter;
}
...
void
GridFluidSolver2::onAdvanceTimeStep(double timeIntervalInSeconds)
{
...
unsigned int
GridFluidSolver2::numberOfSubTimeSteps(double timeIntervalInSeconds) const
{
...
void
GridFluidSolver2::onBeginAdvanceTimeStep(double timeIntervalInSeconds)
{
  UNUSED_VARIABLE(timeIntervalInSeconds);
}

void
GridFluidSolver2::onEndAdvanceTimeStep(double timeIntervalInSeconds)
{
  UNUSED_VARIABLE(timeIntervalInSeconds);
}

void
GridFluidSolver2::computeExternalForces(double timeIntervalInSeconds)
{
  computeGravity(timeIntervalInSeconds);
}

void
GridFluidSolver2::computeViscosity(double timeIntervalInSeconds)
{
  if (_diffusionSolver != nullptr && _viscosityCoefficient > kEpsilonD) {
    ...
    _diffusionSolver->solve(*vel0, _viscosityCoefficient, timeIntervalInSeconds, vel.get(), *colliderSdf(), *fluidSdf());
    applyBoundaryCondition();
  }
}

void
GridFluidSolver2::computePressure(double timeIntervalInSeconds)
{
  if (_pressureSolver != nullptr) {
    auto vel = velocity();
    auto vel0 = std::dynamic_pointer_cast<FaceCenteredGrid2>(vel->clone());

    _pressureSolver->solve(*vel0, timeIntervalInSeconds, vel.get(), *colliderSdf(), *colliderVelocityField(), *fluidSdf(), _useCompressedLinearSys);
    applyBoundaryCondition();
  }
}

void
GridFluidSolver2::computeAdvection(double timeIntervalInSeconds)
{
  auto vel = velocity();
  if (_advectionSolver != nullptr) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (collocated != nullptr) {
        _advectionSolver->advect(*collocated0, *vel, timeIntervalInSeconds, collocated.get(), *colliderSdf());
      ...
      if (faceCentered != nullptr && faceCentered0 != nullptr) {
        _advectionSolver->advect(*faceCentered0, *vel, timeIntervalInSeconds, faceCentered.get(), *colliderSdf());
...
ScalarField2Ptr
GridFluidSolver2::fluidSdf() const
{
  return std::make_shared<ConstantScalarField2>(-kMaxD);
}

void
GridFluidSolver2::computeGravity(double timeIntervalInSeconds)
{
  if (_gravity.lengthSquared() > kEpsilonD) {
    ...
    if (std::abs(_gravity.x) > kEpsilonD) {
      vel->forEachUIndex([&](size_t i, size_t j) { u(i, j) += timeIntervalInSeconds * _gravity.x; });
    }

    if (std::abs(_gravity.y) > kEpsilonD) {
      vel->forEachVIndex([&](size_t i, size_t j) { v(i, j) += timeIntervalInSeconds * _gravity.y; });
    }
...
void
GridFluidSolver2::applyBoundaryCondition()
{
  auto vel = _grids->velocity();
...
void
GridFluidSolver2::extrapolateIntoCollider(ScalarGrid2* grid)
{
...
void
GridFluidSolver2::extrapolateIntoCollider(CollocatedVectorGrid2* grid)
{
  Array2<char> marker(grid->dataSize());
  auto pos = grid->dataPosition();
  marker.parallelForEachIndex([&](size_t i, size_t j) {
...
void
GridFluidSolver2::extrapolateIntoCollider(FaceCenteredGrid2* grid)
{
...
ScalarField2Ptr
GridFluidSolver2::colliderSdf() const
{
  return _boundaryConditionSolver->colliderSdf();
}

VectorField2Ptr
GridFluidSolver2::colliderVelocityField() const
{
  return _boundaryConditionSolver->colliderVelocityField();
}

void
GridFluidSolver2::beginAdvanceTimeStep(double timeIntervalInSeconds)
{
  ...
  if (_boundaryConditionSolver != nullptr) {
    _boundaryConditionSolver->updateCollider(_collider, _grids->resolution(), _grids->gridSpacing(), _grids->origin());
  }
...
void
GridFluidSolver2::endAdvanceTimeStep(double timeIntervalInSeconds)
{
  // Invoke callback
...
void
GridFluidSolver2::updateCollider(double timeIntervalInSeconds)
{
...
void
GridFluidSolver2::updateEmitter(double timeIntervalInSeconds)
{
  if (_emitter != nullptr) {
