...
void buildSingleSystem(FdmMatrix2* A, FdmVector2* b, const Array2<char>& markers, const FaceCenteredGrid2& input) {
...
void buildSingleSystem(MatrixCsrD* A, VectorND* x, VectorND* b, const Array2<char>& markers, const FaceCenteredGrid2& input) {
    Size2 size = input.resolution();
    Vector2D invH = 1.0 / input.gridSpacing();
    Vector2D invHSqr = invH * invH;
...
GridSinglePhasePressureSolver2::GridSinglePhasePressureSolver2() { _systemSolver = std::make_shared<FdmIccgSolver2>(100, kDefaultTolerance); }

GridSinglePhasePressureSolver2::~GridSinglePhasePressureSolver2() {}

void GridSinglePhasePressureSolver2::solve(const FaceCenteredGrid2& input, double timeIntervalInSeconds, FaceCenteredGrid2* output, const ScalarField2& boundarySdf,
...
GridBoundaryConditionSolver2Ptr GridSinglePhasePressureSolver2::suggestedBoundaryConditionSolver() const { return std::make_shared<GridBlockedBoundaryConditionSolver2>(); }

const FdmLinearSystemSolver2Ptr& GridSinglePhasePressureSolver2::linearSystemSolver() const { return _systemSolver; }
...
void GridSinglePhasePressureSolver2::buildMarkers(const Size2& size, const std::function<Vector2D(size_t, size_t)>& pos, const ScalarField2& boundarySdf, const ScalarField2& fluidSdf) {
    // Build levels
    size_t maxLevels = 1;
    if (_mgSystemSolver != nullptr) {
