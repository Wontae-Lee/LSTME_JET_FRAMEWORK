...
void
GridFractionalBoundaryConditionSolver2::constrainVelocity(FaceCenteredGrid2* velocity, unsigned int extrapolationDepth)
{
...
void
GridFractionalBoundaryConditionSolver2::onColliderUpdated(const Size2& gridSize, const Vector2D& gridSpacing, const Vector2D& gridOrigin)
{
  ...
  if (collider() != nullptr) {
    ...
    _colliderVel =
      CustomVectorField2::builder().withFunction([](const Vector2D&) { return Vector2D(); }).withDerivativeResolution(gridSpacing.x).makeShared();
  }
}
