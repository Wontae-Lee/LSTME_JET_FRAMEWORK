...
void
buildSingleSystem(FdmMatrix2* A,
                  FdmVector2* b,
  ...
  A->parallelForEachIndex([&](size_t i, size_t j) {
    ...
    if (isInsideSdf(centerPhi)) {
      ...
      double boundaryContribution =
        (1.0 - uWeights(i + 1, j)) * boundaryVel(uPos(i + 1, j)).x * invH.x - (1.0 - uWeights(i, j)) * boundaryVel(uPos(i, j)).x * invH.x +
...
void
GridFractionalSinglePhasePressureSolver2::solve(const FaceCenteredGrid2& input,
...
void
GridFractionalSinglePhasePressureSolver2::setLinearSystemSolver(const FdmLinearSystemSolver2Ptr& solver)
{
...
const FdmVector2&
GridFractionalSinglePhasePressureSolver2::pressure() const
{
  if (_mgSystemSolver == nullptr) {
    return _system.x;
  } else {
...
void
GridFractionalSinglePhasePressureSolver2::buildWeights(const FaceCenteredGrid2& input,
                                                       const ScalarField2& boundarySdf,
                                                       const VectorField2& boundaryVelocity,
                                                       const ScalarField2& fluidSdf)
{
...
void
GridFractionalSinglePhasePressureSolver2::decompressSolution()
{
...
void
GridFractionalSinglePhasePressureSolver2::buildSystem(const FaceCenteredGrid2& input, bool useCompressed)
{
...
void
GridFractionalSinglePhasePressureSolver2::applyPressureGradient(const FaceCenteredGrid2& input, FaceCenteredGrid2* output)
{
  Size2 size = input.resolution();
