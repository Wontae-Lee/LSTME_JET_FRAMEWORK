...
GridBackwardEulerDiffusionSolver2::GridBackwardEulerDiffusionSolver2(BoundaryType boundaryType) : _boundaryType(boundaryType) { _systemSolver = std::make_shared<FdmIccgSolver2>(100, kEpsilonD); }

void GridBackwardEulerDiffusionSolver2::solve(const ScalarGrid2& source, double diffusionCoefficient, double timeIntervalInSeconds, ScalarGrid2* dest, const ScalarField2& boundarySdf,
    ...
    if (_systemSolver != nullptr) {
        ...
        // Assign the solution
        source.parallelForEachDataPointIndex([&](size_t i, size_t j) { (*dest)(i, j) = _system.x(i, j); });
    }
}

void GridBackwardEulerDiffusionSolver2::solve(const CollocatedVectorGrid2& source, double diffusionCoefficient, double timeIntervalInSeconds, CollocatedVectorGrid2* dest,
    ...
    if (_systemSolver != nullptr) {
        ...
        // Assign the solution
        source.parallelForEachDataPointIndex([&](size_t i, size_t j) { (*dest)(i, j).x = _system.x(i, j); });
        ...
        // Assign the solution
        source.parallelForEachDataPointIndex([&](size_t i, size_t j) { (*dest)(i, j).y = _system.x(i, j); });
    }
}

void GridBackwardEulerDiffusionSolver2::solve(const FaceCenteredGrid2& source, double diffusionCoefficient, double timeIntervalInSeconds, FaceCenteredGrid2* dest, const ScalarField2& boundarySdf,
                                              const ScalarField2& fluidSdf) {
    if (_systemSolver != nullptr) {
        Vector2D h = source.gridSpacing();
        Vector2D c = timeIntervalInSeconds * diffusionCoefficient / (h * h);
        ...
        // Assign the solution
        source.parallelForEachUIndex([&](size_t i, size_t j) { dest->u(i, j) = _system.x(i, j); });
        ...
        // Assign the solution
        source.parallelForEachVIndex([&](size_t i, size_t j) { dest->v(i, j) = _system.x(i, j); });
    }
}

void GridBackwardEulerDiffusionSolver2::setLinearSystemSolver(const FdmLinearSystemSolver2Ptr& solver) { _systemSolver = solver; }

void GridBackwardEulerDiffusionSolver2::buildMarkers(const Size2& size, const std::function<Vector2D(size_t, size_t)>& pos, const ScalarField2& boundarySdf, const ScalarField2& fluidSdf) {
...
void GridBackwardEulerDiffusionSolver2::buildMatrix(const Size2& size, const Vector2D& c) {
    ...
    _system.A.parallelForEachIndex([&](size_t i, size_t j) {
        ...
        if (_markers(i, j) == kFluid) {
            if (i + 1 < size.x) {
                if ((isDirichlet && _markers(i + 1, j) != kAir) || _markers(i + 1, j) == kFluid) {
                    row.center += c.x;
                }
            ...
            if (i > 0 && ((isDirichlet && _markers(i - 1, j) != kAir) || _markers(i - 1, j) == kFluid)) {
                row.center += c.x;
            }

            if (j + 1 < size.y) {
                if ((isDirichlet && _markers(i, j + 1) != kAir) || _markers(i, j + 1) == kFluid) {
                    row.center += c.y;
                }
            ...
            if (j > 0 && ((isDirichlet && _markers(i, j - 1) != kAir) || _markers(i, j - 1) == kFluid)) {
                row.center += c.y;
            }
        }
    });
