...
void buildSingleSystem(FdmMatrix3* A, FdmVector3* b, const Array3<char>& markers, const FaceCenteredGrid3& input) {
...
void buildSingleSystem(MatrixCsrD* A, VectorND* x, VectorND* b, const Array3<char>& markers, const FaceCenteredGrid3& input) {
    Size3 size = input.resolution();
    Vector3D invH = 1.0 / input.gridSpacing();
    Vector3D invHSqr = invH * invH;
...
GridSinglePhasePressureSolver3::GridSinglePhasePressureSolver3() { _systemSolver = std::make_shared<FdmIccgSolver3>(100, kDefaultTolerance); }

GridSinglePhasePressureSolver3::~GridSinglePhasePressureSolver3() {}

void GridSinglePhasePressureSolver3::solve(const FaceCenteredGrid3& input, double timeIntervalInSeconds, FaceCenteredGrid3* output, const ScalarField3& boundarySdf,
...
GridBoundaryConditionSolver3Ptr GridSinglePhasePressureSolver3::suggestedBoundaryConditionSolver() const { return std::make_shared<GridBlockedBoundaryConditionSolver3>(); }

const FdmLinearSystemSolver3Ptr& GridSinglePhasePressureSolver3::linearSystemSolver() const { return _systemSolver; }
...
void GridSinglePhasePressureSolver3::buildMarkers(const Size3& size, const std::function<Vector3D(size_t, size_t, size_t)>& pos, const ScalarField3& boundarySdf, const ScalarField3& fluidSdf) {
    // Build levels
    size_t maxLevels = 1;
    if (_mgSystemSolver != nullptr) {
...
void GridSinglePhasePressureSolver3::buildSystem(const FaceCenteredGrid3& input, bool useCompressed) {
    ...
    if (_mgSystemSolver == nullptr) {
        ...
        buildSingleSystem(&_mgSystem.A.levels.front(), &_mgSystem.b.levels.front(), _markers[0], *finer);
    }
