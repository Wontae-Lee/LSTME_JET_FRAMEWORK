...
#include "jet/solver/grid/grid_fractional_boundary_condition_solver3.h"
...
void restrict(const Array3<float>& finer, Array3<float>* coarser) {
    ...
    parallelRangeFor(kZeroSize, n.x, kZeroSize, n.y, kZeroSize, n.z, [&](size_t iBegin, size_t iEnd, size_t jBegin, size_t jEnd, size_t kBegin, size_t kEnd) {
...
void buildSingleSystem(FdmMatrix3* A, FdmVector3* b, const Array3<float>& fluidSdf, const Array3<float>& uWeights, const Array3<float>& vWeights, const Array3<float>& wWeights,
                       std::function<Vector3D(const Vector3D&)> boundaryVel, const FaceCenteredGrid3& input) {
    ...
    A->parallelForEachIndex([&](size_t i, size_t j, size_t k) {
        ...
        if (isInsideSdf(centerPhi)) {
            ...
            if (i + 1 < size.x) {
                ...
                (*b)(i, j, k) += uWeights(i + 1, j, k) * input.u(i + 1, j, k) * invH.x;
            } else {
            ...
            if (j + 1 < size.y) {
                ...
                (*b)(i, j, k) += vWeights(i, j + 1, k) * input.v(i, j + 1, k) * invH.y;
            } else {
            ...
            if (k + 1 < size.z) {
                ...
                (*b)(i, j, k) += wWeights(i, j, k + 1) * input.w(i, j, k + 1) * invH.z;
            } else {
            ...
            double boundaryContribution = (1.0 - uWeights(i + 1, j, k)) * boundaryVel(uPos(i + 1, j, k)).x * invH.x - (1.0 - uWeights(i, j, k)) * boundaryVel(uPos(i, j, k)).x * invH.x +
                                          (1.0 - vWeights(i, j + 1, k)) * boundaryVel(vPos(i, j + 1, k)).y * invH.y - (1.0 - vWeights(i, j, k)) * boundaryVel(vPos(i, j, k)).y * invH.y +
                                          (1.0 - wWeights(i, j, k + 1)) * boundaryVel(wPos(i, j, k + 1)).z * invH.z - (1.0 - wWeights(i, j, k)) * boundaryVel(wPos(i, j, k)).z * invH.z;
...
void buildSingleSystem(MatrixCsrD* A, VectorND* x, VectorND* b, const Array3<float>& fluidSdf, const Array3<float>& uWeights, const Array3<float>& vWeights, const Array3<float>& wWeights,
                       std::function<Vector3D(const Vector3D&)> boundaryVel, const FaceCenteredGrid3& input) {
    const Size3 size = input.resolution();
    const auto uPos = input.uPosition();
    const auto vPos = input.vPosition();
...
GridFractionalSinglePhasePressureSolver3::GridFractionalSinglePhasePressureSolver3() { _systemSolver = std::make_shared<FdmIccgSolver3>(100, kDefaultTolerance); }

GridFractionalSinglePhasePressureSolver3::~GridFractionalSinglePhasePressureSolver3() {}

void GridFractionalSinglePhasePressureSolver3::solve(const FaceCenteredGrid3& input, double timeIntervalInSeconds, FaceCenteredGrid3* output, const ScalarField3& boundarySdf,
                                                     const VectorField3& boundaryVelocity, const ScalarField3& fluidSdf, bool useCompressed) {
...
GridBoundaryConditionSolver3Ptr GridFractionalSinglePhasePressureSolver3::suggestedBoundaryConditionSolver() const { return std::make_shared<GridFractionalBoundaryConditionSolver3>(); }

const FdmLinearSystemSolver3Ptr& GridFractionalSinglePhasePressureSolver3::linearSystemSolver() const { return _systemSolver; }
...
void GridFractionalSinglePhasePressureSolver3::buildWeights(const FaceCenteredGrid3& input, const ScalarField3& boundarySdf, const VectorField3& boundaryVelocity, const ScalarField3& fluidSdf) {
    ...
    _fluidSdf[0].parallelForEachIndex([&](size_t i, size_t j, size_t k) { _fluidSdf[0](i, j, k) = static_cast<float>(fluidSdf.sample(cellPos(i, j, k))); });
...
void GridFractionalSinglePhasePressureSolver3::buildSystem(const FaceCenteredGrid3& input, bool useCompressed) {
    ...
    if (_mgSystemSolver == nullptr) {
        if (useCompressed) {
            buildSingleSystem(&_compSystem.A, &_compSystem.x, &_compSystem.b, _fluidSdf[0], _uWeights[0], _vWeights[0], _wWeights[0], _boundaryVel, *finer);
        } else {
            buildSingleSystem(&_system.A, &_system.b, _fluidSdf[0], _uWeights[0], _vWeights[0], _wWeights[0], _boundaryVel, *finer);
        }
    } else {
        buildSingleSystem(&_mgSystem.A.levels.front(), &_mgSystem.b.levels.front(), _fluidSdf[0], _uWeights[0], _vWeights[0], _wWeights[0], _boundaryVel, *finer);
    }
    ...
    for (size_t l = 1; l < numLevels; ++l) {
        ...
        buildSingleSystem(&_mgSystem.A.levels[l], &_mgSystem.b.levels[l], _fluidSdf[l], _uWeights[l], _vWeights[l], _wWeights[l], _boundaryVel, coarser);
...
void GridFractionalSinglePhasePressureSolver3::applyPressureGradient(const FaceCenteredGrid3& input, FaceCenteredGrid3* output) {
    ...
    x.parallelForEachIndex([&](size_t i, size_t j, size_t k) {
        ...
        if (i + 1 < size.x && _uWeights[0](i + 1, j, k) > 0.0 && (isInsideSdf(centerPhi) || isInsideSdf(_fluidSdf[0](i + 1, j, k)))) {
        ...
        if (j + 1 < size.y && _vWeights[0](i, j + 1, k) > 0.0 && (isInsideSdf(centerPhi) || isInsideSdf(_fluidSdf[0](i, j + 1, k)))) {
        ...
        if (k + 1 < size.z && _wWeights[0](i, j, k + 1) > 0.0 && (isInsideSdf(centerPhi) || isInsideSdf(_fluidSdf[0](i, j, k + 1)))) {
