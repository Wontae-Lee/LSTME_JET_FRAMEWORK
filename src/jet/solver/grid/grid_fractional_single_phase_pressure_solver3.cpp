...
void restrict(const Array3<float>& finer, Array3<float>* coarser)
{
  ...
  parallelRangeFor(
    kZeroSize, n.x, kZeroSize, n.y, kZeroSize, n.z, [&](size_t iBegin, size_t iEnd, size_t jBegin, size_t jEnd, size_t kBegin, size_t kEnd) {
...
void
buildSingleSystem(FdmMatrix3* A,
                  FdmVector3* b,
                  ...
                  const FaceCenteredGrid3& input)
{
  ...
  A->parallelForEachIndex([&](size_t i, size_t j, size_t k) {
    ...
    if (isInsideSdf(centerPhi)) {
      ...
      double boundaryContribution = (1.0 - uWeights(i + 1, j, k)) * boundaryVel(uPos(i + 1, j, k)).x * invH.x -
...
void
buildSingleSystem(MatrixCsrD* A,
                  VectorND* x,
                  VectorND* b,
                  const Array3<float>& fluidSdf,
                  const Array3<float>& uWeights,
...
void
GridFractionalSinglePhasePressureSolver3::solve(const FaceCenteredGrid3& input,
...
GridFractionalSinglePhasePressureSolver3::suggestedBoundaryConditionSolver() const
{
...
void
GridFractionalSinglePhasePressureSolver3::setLinearSystemSolver(const FdmLinearSystemSolver3Ptr& solver)
{
...
const FdmVector3&
GridFractionalSinglePhasePressureSolver3::pressure() const
{
  if (_mgSystemSolver == nullptr) {
    return _system.x;
  } else {
...
void
GridFractionalSinglePhasePressureSolver3::buildWeights(const FaceCenteredGrid3& input,
                                                       const ScalarField3& boundarySdf,
                                                       const VectorField3& boundaryVelocity,
                                                       const ScalarField3& fluidSdf)
{
  ...
  _fluidSdf[0].parallelForEachIndex(
    [&](size_t i, size_t j, size_t k) { _fluidSdf[0](i, j, k) = static_cast<float>(fluidSdf.sample(cellPos(i, j, k))); });
...
void
GridFractionalSinglePhasePressureSolver3::decompressSolution()
{
...
void
GridFractionalSinglePhasePressureSolver3::buildSystem(const FaceCenteredGrid3& input, bool useCompressed)
{
  ...
  if (_mgSystemSolver == nullptr) {
    ...
    buildSingleSystem(
      &_mgSystem.A.levels.front(), &_mgSystem.b.levels.front(), _fluidSdf[0], _uWeights[0], _vWeights[0], _wWeights[0], _boundaryVel, *finer);
  }
...
void
GridFractionalSinglePhasePressureSolver3::applyPressureGradient(const FaceCenteredGrid3& input, FaceCenteredGrid3* output)
{
  Size3 size = input.resolution();
