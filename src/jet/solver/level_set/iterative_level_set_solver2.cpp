...
void
IterativeLevelSetSolver2::reinitialize(const ScalarGrid2& inputSdf, double maxDistance, ScalarGrid2* outputSdf)
{
  ...
  for (unsigned int n = 0; n < numberOfIterations; ++n) {
    inputSdf.parallelForEachDataPointIndex([&](size_t i, size_t j) {
      ...
      double val =
        outputAcc(i, j) -
        dtau * std::max(s, 0.0) *
          (std::sqrt(square(std::max(dx[0], 0.0)) + square(std::min(dx[1], 0.0)) + square(std::max(dy[0], 0.0)) + square(std::min(dy[1], 0.0))) -
           1.0) -
        dtau * std::min(s, 0.0) *
          (std::sqrt(square(std::min(dx[0], 0.0)) + square(std::max(dx[1], 0.0)) + square(std::min(dy[0], 0.0)) + square(std::max(dy[1], 0.0))) -
           1.0);
      tempAcc(i, j) = val;
    });
...
void
IterativeLevelSetSolver2::extrapolate(const ScalarGrid2& input, const ScalarField2& sdf, double maxDistance, ScalarGrid2* output)
{
...
void
IterativeLevelSetSolver2::extrapolate(const CollocatedVectorGrid2& input, const ScalarField2& sdf, double maxDistance, CollocatedVectorGrid2* output)
{
  JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
...
void
IterativeLevelSetSolver2::extrapolate(const FaceCenteredGrid2& input, const ScalarField2& sdf, double maxDistance, FaceCenteredGrid2* output)
{
  JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
...
void
IterativeLevelSetSolver2::extrapolate(const ConstArrayAccessor2<double>& input,
...
double
IterativeLevelSetSolver2::maxCfl() const
{
  return _maxCfl;
}

void
IterativeLevelSetSolver2::setMaxCfl(double newMaxCfl)
{
  _maxCfl = std::max(newMaxCfl, 0.0);
}

unsigned int
IterativeLevelSetSolver2::distanceToNumberOfIterations(double distance, double dtau)
{
  return static_cast<unsigned int>(std::ceil(distance / dtau));
}

double
IterativeLevelSetSolver2::sign(const ConstArrayAccessor2<double>& sdf, const Vector2D& gridSpacing, size_t i, size_t j)
{
  double d = sdf(i, j);
...
double
IterativeLevelSetSolver2::pseudoTimeStep(ConstArrayAccessor2<double> sdf, const Vector2D& gridSpacing)
{
