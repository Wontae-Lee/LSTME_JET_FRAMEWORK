...
#include "jet/solver/level_set/level_set_liquid_solver3.h"
...
LevelSetLiquidSolver3::LevelSetLiquidSolver3() : LevelSetLiquidSolver3({1, 1, 1}, {1, 1, 1}, {0, 0, 0}) {}

LevelSetLiquidSolver3::LevelSetLiquidSolver3(const Size3& resolution, const Vector3D& gridSpacing, const Vector3D& gridOrigin) : GridFluidSolver3(resolution, gridSpacing, gridOrigin) {
    auto grids = gridSystemData();
    _signedDistanceFieldId = grids->addAdvectableScalarData(std::make_shared<CellCenteredScalarGrid3::Builder>(), kMaxD);
    _levelSetSolver = std::make_shared<EnoLevelSetSolver3>();
}

LevelSetLiquidSolver3::~LevelSetLiquidSolver3() {}

ScalarGrid3Ptr LevelSetLiquidSolver3::signedDistanceField() const { return gridSystemData()->advectableScalarDataAt(_signedDistanceFieldId); }

LevelSetSolver3Ptr LevelSetLiquidSolver3::levelSetSolver() const { return _levelSetSolver; }

void LevelSetLiquidSolver3::setLevelSetSolver(const LevelSetSolver3Ptr& newSolver) { _levelSetSolver = newSolver; }

void LevelSetLiquidSolver3::setMinReinitializeDistance(double distance) { _minReinitializeDistance = distance; }

void LevelSetLiquidSolver3::setIsGlobalCompensationEnabled(bool isEnabled) { _isGlobalCompensationEnabled = isEnabled; }
...
double LevelSetLiquidSolver3::computeVolume() const {
    ...
    double volume = 0.0;
    sdf->forEachDataPointIndex([&](size_t i, size_t j, size_t k) { volume += 1.0 - smearedHeavisideSdf((*sdf)(i, j, k) / h); });
    volume *= cellVolume;
...
void LevelSetLiquidSolver3::onEndAdvanceTimeStep(double timeIntervalInSeconds) {
    ...
    JET_INFO << "reinitializing level set field took " << timer.durationInSeconds() << " seconds";
...
void LevelSetLiquidSolver3::computeAdvection(double timeIntervalInSeconds) {
    ...
    JET_INFO << "velocity extrapolation took " << timer.durationInSeconds() << " seconds";

    GridFluidSolver3::computeAdvection(timeIntervalInSeconds);
}

ScalarField3Ptr LevelSetLiquidSolver3::fluidSdf() const { return signedDistanceField(); }
...
void LevelSetLiquidSolver3::reinitialize(double currentCfl) {
    if (_levelSetSolver != nullptr) {
        ...
        const Vector3D gridSpacing = sdf->gridSpacing();
        const double h = max3(gridSpacing.x, gridSpacing.y, gridSpacing.z);
        const double maxReinitDist = std::max(2.0 * currentCfl, _minReinitializeDistance) * h;

        _levelSetSolver->reinitialize(*sdf0, maxReinitDist, sdf.get());
...
void LevelSetLiquidSolver3::addVolume(double volDiff) {
    ...
    if (std::abs(dVdh) > 0.0) {
        ...
        sdf->parallelForEachDataPointIndex([&](size_t i, size_t j, size_t k) { (*sdf)(i, j, k) += dist; });
    }
}

LevelSetLiquidSolver3::Builder LevelSetLiquidSolver3::builder() { return Builder(); }

LevelSetLiquidSolver3 LevelSetLiquidSolver3::Builder::build() const { return LevelSetLiquidSolver3(_resolution, getGridSpacing(), _gridOrigin); }

LevelSetLiquidSolver3Ptr LevelSetLiquidSolver3::Builder::makeShared() const {
    return std::shared_ptr<LevelSetLiquidSolver3>(new LevelSetLiquidSolver3(_resolution, getGridSpacing(), _gridOrigin), [](LevelSetLiquidSolver3* obj) { delete obj; });
}
