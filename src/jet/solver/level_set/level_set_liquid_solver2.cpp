...
#include "jet/solver/level_set/level_set_liquid_solver2.h"
#include "jet/helpers/pch.h"
#include "jet/solver/level_set/eno_level_set_solver2.h"
...
LevelSetLiquidSolver2::LevelSetLiquidSolver2() : LevelSetLiquidSolver2({1, 1}, {1, 1}, {0, 0}) {}

LevelSetLiquidSolver2::LevelSetLiquidSolver2(const Size2& resolution, const Vector2D& gridSpacing, const Vector2D& gridOrigin) : GridFluidSolver2(resolution, gridSpacing, gridOrigin) {
    auto grids = gridSystemData();
    _signedDistanceFieldId = grids->addAdvectableScalarData(std::make_shared<CellCenteredScalarGrid2::Builder>(), kMaxD);
    _levelSetSolver = std::make_shared<EnoLevelSetSolver2>();
}

LevelSetLiquidSolver2::~LevelSetLiquidSolver2() {}

ScalarGrid2Ptr LevelSetLiquidSolver2::signedDistanceField() const { return gridSystemData()->advectableScalarDataAt(_signedDistanceFieldId); }

LevelSetSolver2Ptr LevelSetLiquidSolver2::levelSetSolver() const { return _levelSetSolver; }

void LevelSetLiquidSolver2::setLevelSetSolver(const LevelSetSolver2Ptr& newSolver) { _levelSetSolver = newSolver; }

void LevelSetLiquidSolver2::setMinReinitializeDistance(double distance) { _minReinitializeDistance = distance; }

void LevelSetLiquidSolver2::setIsGlobalCompensationEnabled(bool isEnabled) { _isGlobalCompensationEnabled = isEnabled; }
...
double LevelSetLiquidSolver2::computeVolume() const {
    ...
    double volume = 0.0;
    sdf->forEachDataPointIndex([&](size_t i, size_t j) { volume += 1.0 - smearedHeavisideSdf((*sdf)(i, j) / h); });
    volume *= cellVolume;
...
ScalarField2Ptr LevelSetLiquidSolver2::fluidSdf() const { return signedDistanceField(); }
...
void LevelSetLiquidSolver2::addVolume(double volDiff) {
    ...
    if (std::abs(dVdh) > 0.0) {
        ...
        sdf->parallelForEachDataPointIndex([&](size_t i, size_t j) { (*sdf)(i, j) += dist; });
    }
}

LevelSetLiquidSolver2::Builder LevelSetLiquidSolver2::builder() { return Builder(); }

LevelSetLiquidSolver2 LevelSetLiquidSolver2::Builder::build() const { return LevelSetLiquidSolver2(_resolution, getGridSpacing(), _gridOrigin); }

LevelSetLiquidSolver2Ptr LevelSetLiquidSolver2::Builder::makeShared() const {
    return std::shared_ptr<LevelSetLiquidSolver2>(new LevelSetLiquidSolver2(_resolution, getGridSpacing(), _gridOrigin), [](LevelSetLiquidSolver2* obj) { delete obj; });
}
