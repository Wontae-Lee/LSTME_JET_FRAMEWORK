...
LevelSetLiquidSolver2::LevelSetLiquidSolver2(const Size2& resolution, const Vector2D& gridSpacing, const Vector2D& gridOrigin)
...
ScalarGrid2Ptr
LevelSetLiquidSolver2::signedDistanceField() const
{
  return gridSystemData()->advectableScalarDataAt(_signedDistanceFieldId);
}

LevelSetSolver2Ptr
LevelSetLiquidSolver2::levelSetSolver() const
{
  return _levelSetSolver;
}

void
LevelSetLiquidSolver2::setLevelSetSolver(const LevelSetSolver2Ptr& newSolver)
{
  _levelSetSolver = newSolver;
}

void
LevelSetLiquidSolver2::setMinReinitializeDistance(double distance)
{
  _minReinitializeDistance = distance;
}

void
LevelSetLiquidSolver2::setIsGlobalCompensationEnabled(bool isEnabled)
{
  _isGlobalCompensationEnabled = isEnabled;
}

double
LevelSetLiquidSolver2::computeVolume() const
{
...
void
LevelSetLiquidSolver2::onBeginAdvanceTimeStep(double timeIntervalInSeconds)
{
...
void
LevelSetLiquidSolver2::onEndAdvanceTimeStep(double timeIntervalInSeconds)
{
...
void
LevelSetLiquidSolver2::computeAdvection(double timeIntervalInSeconds)
{
  double currentCfl = cfl(timeIntervalInSeconds);
...
ScalarField2Ptr
LevelSetLiquidSolver2::fluidSdf() const
{
  return signedDistanceField();
}

void
LevelSetLiquidSolver2::reinitialize(double currentCfl)
{
...
void
LevelSetLiquidSolver2::extrapolateVelocityToAir(double currentCfl)
{
  auto sdf = signedDistanceField();
...
void
LevelSetLiquidSolver2::addVolume(double volDiff)
{
  auto sdf = signedDistanceField();
  const Vector2D gridSpacing = sdf->gridSpacing();
  const double cellVolume = gridSpacing.x * gridSpacing.y;
...
LevelSetLiquidSolver2::Builder::build() const
{
  return LevelSetLiquidSolver2(_resolution, getGridSpacing(), _gridOrigin);
}

LevelSetLiquidSolver2Ptr
LevelSetLiquidSolver2::Builder::makeShared() const
{
  return std::shared_ptr<LevelSetLiquidSolver2>(new LevelSetLiquidSolver2(_resolution, getGridSpacing(), _gridOrigin),
