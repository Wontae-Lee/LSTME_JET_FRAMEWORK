...
#include "jet/solver/level_set/fmm_level_set_solver3.h"
...
inline double solveQuadNearBoundary(const Array3<char>& markers, ConstArrayAccessor3<double> output, const Vector3D& gridSpacing, const Vector3D& invGridSpacingSqr, double sign, size_t i, size_t j,
                                    size_t k) {
    ...
    double distToBndX = gridSpacing.x * absCenter / (absCenter + std::abs(phiX));

    double distToBndY = gridSpacing.y * absCenter / (absCenter + std::abs(phiY));

    double distToBndZ = gridSpacing.z * absCenter / (absCenter + std::abs(phiZ));
...
inline double solveQuad(const Array3<char>& markers, ArrayAccessor3<double> output, const Vector3D& gridSpacing, const Vector3D& invGridSpacingSqr, size_t i, size_t j, size_t k) {
    Size3 size = output.size();
...
void FmmLevelSetSolver3::reinitialize(const ScalarGrid3& inputSdf, double maxDistance, ScalarGrid3* outputSdf) {
    ...
    markers.parallelForEachIndex([&](size_t i, size_t j, size_t k) { output(i, j, k) = inputSdf(i, j, k); });

    // Solve geometrically near the boundary
    markers.forEachIndex([&](size_t i, size_t j, size_t k) {
        if (isInsideSdf(output(i, j, k)) &&
            ((i > 0 && !isInsideSdf(output(i - 1, j, k))) || (i + 1 < size.x && !isInsideSdf(output(i + 1, j, k))) || (j > 0 && !isInsideSdf(output(i, j - 1, k))) ||
             (j + 1 < size.y && !isInsideSdf(output(i, j + 1, k))) || (k > 0 && !isInsideSdf(output(i, j, k - 1))) || (k + 1 < size.z && !isInsideSdf(output(i, j, k + 1))))) {
            output(i, j, k) = solveQuadNearBoundary(markers, inputSdf.constDataAccessor(), gridSpacing, invGridSpacingSqr, -1.0, i, j, k);
        }
    });
    markers.forEachIndex([&](size_t i, size_t j, size_t k) {
        if (!isInsideSdf(output(i, j, k)) &&
    ...
    for (int sign = 0; sign < 2; ++sign) {
        ...
        auto compare = [&](const Point3UI& a, const Point3UI& b) { return output(a.x, a.y, a.z) > output(b.x, b.y, b.z); };

        // Enqueue initial candidates
        std::priority_queue<Point3UI, std::vector<Point3UI>, decltype(compare)> trial(compare);
        markers.forEachIndex([&](size_t i, size_t j, size_t k) {
            if (markers(i, j, k) != kKnown && ((i > 0 && markers(i - 1, j, k) == kKnown) || (i + 1 < size.x && markers(i + 1, j, k) == kKnown) || (j > 0 && markers(i, j - 1, k) == kKnown) ||
                                               (j + 1 < size.y && markers(i, j + 1, k) == kKnown) || (k > 0 && markers(i, j, k - 1) == kKnown) || (k + 1 < size.z && markers(i, j, k + 1) == kKnown))) {
        ...
        // Flip the sign
        markers.parallelForEachIndex([&](size_t i, size_t j, size_t k) { output(i, j, k) = -output(i, j, k); });
    }
}

void FmmLevelSetSolver3::extrapolate(const ScalarGrid3& input, const ScalarField3& sdf, double maxDistance, ScalarGrid3* output) {
    JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));

    Array3<double> sdfGrid(input.dataSize());
    auto pos = input.dataPosition();
    sdfGrid.parallelForEachIndex([&](size_t i, size_t j, size_t k) { sdfGrid(i, j, k) = sdf.sample(pos(i, j, k)); });

    extrapolate(input.constDataAccessor(), sdfGrid.constAccessor(), input.gridSpacing(), maxDistance, output->dataAccessor());
}

void FmmLevelSetSolver3::extrapolate(const CollocatedVectorGrid3& input, const ScalarField3& sdf, double maxDistance, CollocatedVectorGrid3* output) {
    JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
...
void FmmLevelSetSolver3::extrapolate(const FaceCenteredGrid3& input, const ScalarField3& sdf, double maxDistance, FaceCenteredGrid3* output) {
    JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
    ...
    input.parallelForEachUIndex([&](size_t i, size_t j, size_t k) { sdfAtU(i, j, k) = sdf.sample(uPos(i, j, k)); });
    ...
    input.parallelForEachVIndex([&](size_t i, size_t j, size_t k) { sdfAtV(i, j, k) = sdf.sample(vPos(i, j, k)); });
    ...
    input.parallelForEachWIndex([&](size_t i, size_t j, size_t k) { sdfAtW(i, j, k) = sdf.sample(wPos(i, j, k)); });

    extrapolate(w, sdfAtW, gridSpacing, maxDistance, output->wAccessor());
}

void FmmLevelSetSolver3::extrapolate(const ConstArrayAccessor3<double>& input, const ConstArrayAccessor3<double>& sdf, const Vector3D& gridSpacing, double maxDistance, ArrayAccessor3<double> output) {
    ...
    auto compare = [&](const Point3UI& a, const Point3UI& b) { return sdf(a.x, a.y, a.z) > sdf(b.x, b.y, b.z); };
