...
inline double
solveQuadNearBoundary(const Array3<char>& markers,
...
inline double
solveQuad(const Array3<char>& markers,
...
void
FmmLevelSetSolver3::reinitialize(const ScalarGrid3& inputSdf, double maxDistance, ScalarGrid3* outputSdf)
{
  ...
  markers.forEachIndex([&](size_t i, size_t j, size_t k) {
    if (isInsideSdf(output(i, j, k)) && ((i > 0 && !isInsideSdf(output(i - 1, j, k))) || (i + 1 < size.x && !isInsideSdf(output(i + 1, j, k))) ||
                                         (j > 0 && !isInsideSdf(output(i, j - 1, k))) || (j + 1 < size.y && !isInsideSdf(output(i, j + 1, k))) ||
                                         (k > 0 && !isInsideSdf(output(i, j, k - 1))) || (k + 1 < size.z && !isInsideSdf(output(i, j, k + 1))))) {
  ...
  for (int sign = 0; sign < 2; ++sign) {
    ...
    markers.forEachIndex([&](size_t i, size_t j, size_t k) {
      if (markers(i, j, k) != kKnown && ((i > 0 && markers(i - 1, j, k) == kKnown) || (i + 1 < size.x && markers(i + 1, j, k) == kKnown) ||
                                         (j > 0 && markers(i, j - 1, k) == kKnown) || (j + 1 < size.y && markers(i, j + 1, k) == kKnown) ||
                                         (k > 0 && markers(i, j, k - 1) == kKnown) || (k + 1 < size.z && markers(i, j, k + 1) == kKnown))) {
...
void
FmmLevelSetSolver3::extrapolate(const ScalarGrid3& input, const ScalarField3& sdf, double maxDistance, ScalarGrid3* output)
{
...
void
FmmLevelSetSolver3::extrapolate(const CollocatedVectorGrid3& input, const ScalarField3& sdf, double maxDistance, CollocatedVectorGrid3* output)
{
  JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
...
void
FmmLevelSetSolver3::extrapolate(const FaceCenteredGrid3& input, const ScalarField3& sdf, double maxDistance, FaceCenteredGrid3* output)
{
  JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
...
void
FmmLevelSetSolver3::extrapolate(const ConstArrayAccessor3<double>& input,
