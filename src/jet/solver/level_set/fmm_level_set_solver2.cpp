...
inline double solveQuadNearBoundary(const Array2<char>& markers, ArrayAccessor2<double> output, const Vector2D& gridSpacing, const Vector2D& invGridSpacingSqr, double sign, size_t i, size_t j) {
...
inline double solveQuad(const Array2<char>& markers, ArrayAccessor2<double> output, const Vector2D& gridSpacing, const Vector2D& invGridSpacingSqr, size_t i, size_t j) {
    Size2 size = output.size();
...
void FmmLevelSetSolver2::reinitialize(const ScalarGrid2& inputSdf, double maxDistance, ScalarGrid2* outputSdf) {
    ...
    markers.forEachIndex([&](size_t i, size_t j) {
        if (!isInsideSdf(output(i, j)) && ((i > 0 && isInsideSdf(output(i - 1, j))) || (i + 1 < size.x && isInsideSdf(output(i + 1, j))) || (j > 0 && isInsideSdf(output(i, j - 1))) ||
                                           (j + 1 < size.y && isInsideSdf(output(i, j + 1))))) {
            output(i, j) = solveQuadNearBoundary(markers, output, gridSpacing, invGridSpacingSqr, 1.0, i, j);
        } else if (isInsideSdf(output(i, j)) && ((i > 0 && !isInsideSdf(output(i - 1, j))) || (i + 1 < size.x && !isInsideSdf(output(i + 1, j))) || (j > 0 && !isInsideSdf(output(i, j - 1))) ||
                                                 (j + 1 < size.y && !isInsideSdf(output(i, j + 1))))) {
            output(i, j) = solveQuadNearBoundary(markers, output, gridSpacing, invGridSpacingSqr, -1.0, i, j);
        }
    });
    ...
    for (int sign = 0; sign < 2; ++sign) {
        ...
        auto compare = [&](const Point2UI& a, const Point2UI& b) { return output(a.x, a.y) > output(b.x, b.y); };

        // Enqueue initial candidates
        std::priority_queue<Point2UI, std::vector<Point2UI>, decltype(compare)> trial(compare);
        markers.forEachIndex([&](size_t i, size_t j) {
            if (markers(i, j) != kKnown && ((i > 0 && markers(i - 1, j) == kKnown) || (i + 1 < size.x && markers(i + 1, j) == kKnown) || (j > 0 && markers(i, j - 1) == kKnown) ||
...
void FmmLevelSetSolver2::extrapolate(const ScalarGrid2& input, const ScalarField2& sdf, double maxDistance, ScalarGrid2* output) {
...
void FmmLevelSetSolver2::extrapolate(const CollocatedVectorGrid2& input, const ScalarField2& sdf, double maxDistance, CollocatedVectorGrid2* output) {
    JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
...
void FmmLevelSetSolver2::extrapolate(const FaceCenteredGrid2& input, const ScalarField2& sdf, double maxDistance, FaceCenteredGrid2* output) {
    JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
...
void FmmLevelSetSolver2::extrapolate(const ConstArrayAccessor2<double>& input, const ConstArrayAccessor2<double>& sdf, const Vector2D& gridSpacing, double maxDistance, ArrayAccessor2<double> output) {
    ...
    auto compare = [&](const Point2UI& a, const Point2UI& b) { return sdf(a.x, a.y) > sdf(b.x, b.y); };
