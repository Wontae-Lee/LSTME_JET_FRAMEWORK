...
PicSolver3::PicSolver3(const Size3& resolution, const Vector3D& gridSpacing, const Vector3D& gridOrigin)
  : GridFluidSolver3(resolution, gridSpacing, gridOrigin)
{
  auto grids = gridSystemData();
  _signedDistanceFieldId = grids->addScalarData(std::make_shared<CellCenteredScalarGrid3::Builder>(), kMaxD);
...
ScalarGrid3Ptr
PicSolver3::signedDistanceField() const
{
  return gridSystemData()->scalarDataAt(_signedDistanceFieldId);
}

const ParticleSystemData3Ptr&
PicSolver3::particleSystemData() const
{
  return _particles;
}

const ParticleEmitter3Ptr&
PicSolver3::particleEmitter() const
{
  return _particleEmitter;
}

void
PicSolver3::setParticleEmitter(const ParticleEmitter3Ptr& newEmitter)
{
...
void
PicSolver3::onInitialize()
{
  GridFluidSolver3::onInitialize();

  Timer timer;
  updateParticleEmitter(0.0);
  JET_INFO << "Update particle emitter took " << timer.durationInSeconds() << " seconds";
}

void
PicSolver3::onBeginAdvanceTimeStep(double timeIntervalInSeconds)
{
  UNUSED_VARIABLE(timeIntervalInSeconds);

  JET_INFO << "Number of PIC-type particles: " << _particles->numberOfParticles();
  ...
  JET_INFO << "transferFromParticlesToGrids took " << timer.durationInSeconds() << " seconds";

  timer.reset();
  buildSignedDistanceField();
  JET_INFO << "buildSignedDistanceField took " << timer.durationInSeconds() << " seconds";

  timer.reset();
  extrapolateVelocityToAir();
  JET_INFO << "extrapolateVelocityToAir took " << timer.durationInSeconds() << " seconds";

  applyBoundaryCondition();
}

void
PicSolver3::computeAdvection(double timeIntervalInSeconds)
{
  Timer timer;
  extrapolateVelocityToAir();
  JET_INFO << "extrapolateVelocityToAir took " << timer.durationInSeconds() << " seconds";
  ...
  JET_INFO << "transferFromGridsToParticles took " << timer.durationInSeconds() << " seconds";

  timer.reset();
  moveParticles(timeIntervalInSeconds);
  JET_INFO << "moveParticles took " << timer.durationInSeconds() << " seconds";
}

ScalarField3Ptr
PicSolver3::fluidSdf() const
{
...
PicSolver3::transferFromParticlesToGrids()
{
  ...
  _vMarkers.set(0);
  _wMarkers.set(0);
  LinearArraySampler3<double, double> uSampler(flow->uConstAccessor(), flow->gridSpacing(), flow->uOrigin());
  LinearArraySampler3<double, double> vSampler(flow->vConstAccessor(), flow->gridSpacing(), flow->vOrigin());
  LinearArraySampler3<double, double> wSampler(flow->wConstAccessor(), flow->gridSpacing(), flow->wOrigin());
...
void
PicSolver3::transferFromGridsToParticles()
{
  auto flow = gridSystemData()->velocity();
  auto positions = _particles->positions();
  auto velocities = _particles->velocities();
  ...
  parallelFor(kZeroSize, numberOfParticles, [&](size_t i) { velocities[i] = flow->sample(positions[i]); });
}

void
PicSolver3::moveParticles(double timeIntervalInSeconds)
{
  auto flow = gridSystemData()->velocity();
  auto positions = _particles->positions();
  auto velocities = _particles->velocities();
  ...
  parallelFor(kZeroSize, numberOfParticles, [&](size_t i) {
    ...
    if ((domainBoundaryFlag & kDirectionLeft) && pt1.x <= boundingBox.lowerCorner.x) {
      pt1.x = boundingBox.lowerCorner.x;
      vel.x = 0.0;
    }
    if ((domainBoundaryFlag & kDirectionRight) && pt1.x >= boundingBox.upperCorner.x) {
      pt1.x = boundingBox.upperCorner.x;
      vel.x = 0.0;
    }
    if ((domainBoundaryFlag & kDirectionDown) && pt1.y <= boundingBox.lowerCorner.y) {
      pt1.y = boundingBox.lowerCorner.y;
      vel.y = 0.0;
    }
    if ((domainBoundaryFlag & kDirectionUp) && pt1.y >= boundingBox.upperCorner.y) {
      pt1.y = boundingBox.upperCorner.y;
      vel.y = 0.0;
    }
    if ((domainBoundaryFlag & kDirectionBack) && pt1.z <= boundingBox.lowerCorner.z) {
      pt1.z = boundingBox.lowerCorner.z;
      vel.z = 0.0;
    }
    if ((domainBoundaryFlag & kDirectionFront) && pt1.z >= boundingBox.upperCorner.z) {
  ...
  if (col != nullptr) {
    parallelFor(kZeroSize, numberOfParticles, [&](size_t i) { col->resolveCollision(0.0, 0.0, &positions[i], &velocities[i]); });
  }
}

void
PicSolver3::extrapolateVelocityToAir()
{
...
void
PicSolver3::buildSignedDistanceField()
{
  auto sdf = signedDistanceField();
  auto sdfPos = sdf->dataPosition();
  double maxH = max3(sdf->gridSpacing().x, sdf->gridSpacing().y, sdf->gridSpacing().z);
  ...
  sdf->parallelForEachDataPointIndex([&](size_t i, size_t j, size_t k) {
    ...
    searcher->forEachNearbyPoint(pt, sdfBandRadius, [&](size_t, const Vector3D& x) { minDist = std::min(minDist, pt.distanceTo(x)); });
...
void
PicSolver3::updateParticleEmitter(double timeIntervalInSeconds)
{
...
PicSolver3
PicSolver3::Builder::build() const
{
  return PicSolver3(_resolution, getGridSpacing(), _gridOrigin);
}

PicSolver3Ptr
PicSolver3::Builder::makeShared() const
{
  return std::shared_ptr<PicSolver3>(new PicSolver3(_resolution, getGridSpacing(), _gridOrigin), [](PicSolver3* obj) { delete obj; });
}
