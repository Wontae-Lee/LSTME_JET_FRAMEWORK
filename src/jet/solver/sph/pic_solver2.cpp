...
PicSolver2::PicSolver2(const Size2& resolution, const Vector2D& gridSpacing, const Vector2D& gridOrigin)
  : GridFluidSolver2(resolution, gridSpacing, gridOrigin)
{
  auto grids = gridSystemData();
  _signedDistanceFieldId = grids->addScalarData(std::make_shared<CellCenteredScalarGrid2::Builder>(), kMaxD);
...
ScalarGrid2Ptr
PicSolver2::signedDistanceField() const
{
  return gridSystemData()->scalarDataAt(_signedDistanceFieldId);
}

const ParticleSystemData2Ptr&
PicSolver2::particleSystemData() const
{
  return _particles;
}

const ParticleEmitter2Ptr&
PicSolver2::particleEmitter() const
{
  return _particleEmitter;
}

void
PicSolver2::setParticleEmitter(const ParticleEmitter2Ptr& newEmitter)
{
...
void
PicSolver2::onBeginAdvanceTimeStep(double timeIntervalInSeconds)
{
...
void
PicSolver2::computeAdvection(double timeIntervalInSeconds)
{
  Timer timer;
  extrapolateVelocityToAir();
  JET_INFO << "extrapolateVelocityToAir took " << timer.durationInSeconds() << " seconds";
...
PicSolver2::transferFromParticlesToGrids()
{
  ...
  _uMarkers.set(0);
  _vMarkers.set(0);
  LinearArraySampler2<double, double> uSampler(flow->uConstAccessor(), flow->gridSpacing(), flow->uOrigin());
  LinearArraySampler2<double, double> vSampler(flow->vConstAccessor(), flow->gridSpacing(), flow->vOrigin());
...
void
PicSolver2::transferFromGridsToParticles()
{
  auto flow = gridSystemData()->velocity();
  auto positions = _particles->positions();
  auto velocities = _particles->velocities();
...
void
PicSolver2::moveParticles(double timeIntervalInSeconds)
{
  auto flow = gridSystemData()->velocity();
  auto positions = _particles->positions();
  auto velocities = _particles->velocities();
...
void
PicSolver2::extrapolateVelocityToAir()
{
...
void
PicSolver2::buildSignedDistanceField()
{
  ...
  sdf->parallelForEachDataPointIndex([&](size_t i, size_t j) {
    ...
    searcher->forEachNearbyPoint(pt, 2.0 * radius, [&](size_t, const Vector2D& x) { minDist = std::min(minDist, pt.distanceTo(x)); });
...
void
PicSolver2::updateParticleEmitter(double timeIntervalInSeconds)
{
...
PicSolver2
PicSolver2::Builder::build() const
{
  return PicSolver2(_resolution, getGridSpacing(), _gridOrigin);
}

PicSolver2Ptr
PicSolver2::Builder::makeShared() const
{
  return std::shared_ptr<PicSolver2>(new PicSolver2(_resolution, getGridSpacing(), _gridOrigin), [](PicSolver2* obj) { delete obj; });
}
