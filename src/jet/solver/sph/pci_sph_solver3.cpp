...
PciSphSolver3::accumulatePressureForce(double timeIntervalInSeconds)
{
  ...
  for (unsigned int k = 0; k < _maxNumberOfIterations; ++k) {
    ...
    SphSolver3::accumulatePressureForce(static_cast<ConstArrayAccessor<Vector3D ,1>>(x), ds.constAccessor(), static_cast<ConstArrayAccessor<double ,1>>(p), _pressureForces.accessor());
...
{
  return std::shared_ptr<PciSphSolver3>(new PciSphSolver3(_targetDensity, _targetSpacing, _relativeKernelRadius),
