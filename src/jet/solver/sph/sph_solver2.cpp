...
#include "jet/solver/sph/sph_solver2.h"
#include "jet/helpers/pch.h"
#include "jet/helpers/physics_helpers.h"
...
double
SphSolver2::eosExponent() const
{
  return _eosExponent;
}

void
SphSolver2::setEosExponent(double newEosExponent)
{
  _eosExponent = std::max(newEosExponent, 1.0);
}

double
SphSolver2::negativePressureScale() const
{
  return _negativePressureScale;
}

void
SphSolver2::setNegativePressureScale(double newNegativePressureScale)
{
  _negativePressureScale = clamp(newNegativePressureScale, 0.0, 1.0);
}

double
SphSolver2::viscosityCoefficient() const
{
  return _viscosityCoefficient;
}

void
SphSolver2::setViscosityCoefficient(double newViscosityCoefficient)
{
  _viscosityCoefficient = std::max(newViscosityCoefficient, 0.0);
}

double
SphSolver2::pseudoViscosityCoefficient() const
{
  return _pseudoViscosityCoefficient;
}

void
SphSolver2::setPseudoViscosityCoefficient(double newPseudoViscosityCoefficient)
{
  _pseudoViscosityCoefficient = std::max(newPseudoViscosityCoefficient, 0.0);
}

double
SphSolver2::speedOfSound() const
{
  return _speedOfSound;
}

void
SphSolver2::setSpeedOfSound(double newSpeedOfSound)
{
  _speedOfSound = std::max(newSpeedOfSound, kEpsilonD);
}

double
SphSolver2::timeStepLimitScale() const
{
  return _timeStepLimitScale;
}

void
SphSolver2::setTimeStepLimitScale(double newScale)
{
  _timeStepLimitScale = std::max(newScale, 0.0);
}

SphSystemData2Ptr
SphSolver2::sphSystemData() const
{
  return std::dynamic_pointer_cast<SphSystemData2>(particleSystemData());
}

unsigned int
SphSolver2::numberOfSubTimeSteps(double timeIntervalInSeconds) const
{
...
void
SphSolver2::accumulateForces(double timeStepInSeconds)
{
...
void
SphSolver2::onBeginAdvanceTimeStep(double timeStepInSeconds)
{
  ...
  JET_INFO << "Building neighbor lists and updating densities took " << timer.durationInSeconds() << " seconds";
}

void
SphSolver2::onEndAdvanceTimeStep(double timeStepInSeconds)
{
...
void
SphSolver2::accumulateNonPressureForces(double timeStepInSeconds)
{
...
void
SphSolver2::accumulatePressureForce(double timeStepInSeconds)
{
  UNUSED_VARIABLE(timeStepInSeconds);
  ...
  accumulatePressureForce(
    static_cast<ConstArrayAccessor<Vector2D, 1>>(x), static_cast<ConstArrayAccessor<double, 1>>(d), static_cast<ConstArrayAccessor<double, 1>>(p), f);
}

void
SphSolver2::computePressure()
{
  auto particles = sphSystemData();
  size_t numberOfParticles = particles->numberOfParticles();
  auto d = particles->densities();
...
void
SphSolver2::accumulatePressureForce(const ConstArrayAccessor1<Vector2D>& positions,
  ...
  parallelFor(kZeroSize, numberOfParticles, [&](size_t i) {
    ...
    for (size_t j : neighbors) {
      ...
      if (dist > 0.0) {
        ...
        pressureForces[i] -=
          massSquared * (pressures[i] / (densities[i] * densities[i]) + pressures[j] / (densities[j] * densities[j])) * kernel.gradient(dist, dir);
      }
    }
  });
}
...
void
SphSolver2::accumulateViscosityForce()
{
  auto particles = sphSystemData();
  size_t numberOfParticles = particles->numberOfParticles();
  auto x = particles->positions();
...
void
SphSolver2::computePseudoViscosity(double timeStepInSeconds)
{
  auto particles = sphSystemData();
  size_t numberOfParticles = particles->numberOfParticles();
  auto x = particles->positions();
  ...
  parallelFor(kZeroSize, numberOfParticles, [&](size_t i) { v[i] = lerp(v[i], smoothedVelocities[i], factor); });
}
...
SphSolver2
SphSolver2::Builder::build() const
{
  return SphSolver2(_targetDensity, _targetSpacing, _relativeKernelRadius);
}

SphSolver2Ptr
SphSolver2::Builder::makeShared() const
{
  return std::shared_ptr<SphSolver2>(new SphSolver2(_targetDensity, _targetSpacing, _relativeKernelRadius), [](SphSolver2* obj) { delete obj; });
}
