...
void SphSolver3::setEosExponent(double newEosExponent) { _eosExponent = std::max(newEosExponent, 1.0); }

double SphSolver3::negativePressureScale() const { return _negativePressureScale; }

void SphSolver3::setNegativePressureScale(double newNegativePressureScale) { _negativePressureScale = clamp(newNegativePressureScale, 0.0, 1.0); }

double SphSolver3::viscosityCoefficient() const { return _viscosityCoefficient; }

void SphSolver3::setViscosityCoefficient(double newViscosityCoefficient) { _viscosityCoefficient = std::max(newViscosityCoefficient, 0.0); }

double SphSolver3::pseudoViscosityCoefficient() const { return _pseudoViscosityCoefficient; }

void SphSolver3::setPseudoViscosityCoefficient(double newPseudoViscosityCoefficient) { _pseudoViscosityCoefficient = std::max(newPseudoViscosityCoefficient, 0.0); }

double SphSolver3::speedOfSound() const { return _speedOfSound; }

void SphSolver3::setSpeedOfSound(double newSpeedOfSound) { _speedOfSound = std::max(newSpeedOfSound, kEpsilonD); }

double SphSolver3::timeStepLimitScale() const { return _timeStepLimitScale; }

void SphSolver3::setTimeStepLimitScale(double newScale) { _timeStepLimitScale = std::max(newScale, 0.0); }

SphSystemData3Ptr SphSolver3::sphSystemData() const { return std::dynamic_pointer_cast<SphSystemData3>(particleSystemData()); }
...
void SphSolver3::computePressure() {
    ...
    parallelFor(kZeroSize, numberOfParticles, [&](size_t i) { p[i] = computePressureFromEos(d[i], targetDensity, eosScale, eosExponent(), negativePressureScale()); });
}

void SphSolver3::accumulatePressureForce(const ConstArrayAccessor1<Vector3D>& positions, const ConstArrayAccessor1<double>& densities, const ConstArrayAccessor1<double>& pressures,
    ...
    parallelFor(kZeroSize, numberOfParticles, [&](size_t i) {
        ...
        for (size_t j : neighbors) {
            ...
            if (dist > 0.0) {
                ...
                pressureForces[i] -= massSquared * (pressures[i] / (densities[i] * densities[i]) + pressures[j] / (densities[j] * densities[j])) * kernel.gradient(dist, dir);
            }
        }
    });
}
...
SphSolver3 SphSolver3::Builder::build() const { return SphSolver3(_targetDensity, _targetSpacing, _relativeKernelRadius); }

SphSolver3Ptr SphSolver3::Builder::makeShared() const {
    return std::shared_ptr<SphSolver3>(new SphSolver3(_targetDensity, _targetSpacing, _relativeKernelRadius), [](SphSolver3* obj) { delete obj; });
}
