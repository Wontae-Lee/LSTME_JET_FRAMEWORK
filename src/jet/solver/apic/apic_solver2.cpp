...
ApicSolver2::ApicSolver2()
  : ApicSolver2({ 1, 1 }, { 1, 1 }, { 0, 0 })
{
}

ApicSolver2::ApicSolver2(const Size2& resolution, const Vector2D& gridSpacing, const Vector2D& gridOrigin)
...
void
ApicSolver2::transferFromParticlesToGrids()
{
...
void
ApicSolver2::transferFromGridsToParticles()
{
  ...
  auto u = flow->uAccessor();
  auto v = flow->vAccessor();
  LinearArraySampler2<double, double> uSampler(static_cast<ConstArrayAccessor<double,2>>(u), flow->gridSpacing(), flow->uOrigin());
  LinearArraySampler2<double, double> vSampler(static_cast<ConstArrayAccessor<double,2>>(v), flow->gridSpacing(), flow->vOrigin());
...
ApicSolver2
ApicSolver2::Builder::build() const
{
  return ApicSolver2(_resolution, getGridSpacing(), _gridOrigin);
}

ApicSolver2Ptr
ApicSolver2::Builder::makeShared() const
{
  return std::shared_ptr<ApicSolver2>(new ApicSolver2(_resolution, getGridSpacing(), _gridOrigin), [](ApicSolver2* obj) { delete obj; });
}
