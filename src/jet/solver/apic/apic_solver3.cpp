...
ApicSolver3::ApicSolver3(const Size3& resolution, const Vector3D& gridSpacing, const Vector3D& gridOrigin) : PicSolver3(resolution, gridSpacing, gridOrigin) {}
...
void ApicSolver3::transferFromParticlesToGrids() {
    ...
    for (size_t i = 0; i < numberOfParticles; ++i) {
        ...
        uPosClamped.y = clamp(uPosClamped.y, bbox.lowerCorner.y + hh.y, bbox.upperCorner.y - hh.y);
        uPosClamped.z = clamp(uPosClamped.z, bbox.lowerCorner.z + hh.z, bbox.upperCorner.z - hh.z);
        ...
        vPosClamped.x = clamp(vPosClamped.x, bbox.lowerCorner.x + hh.x, bbox.upperCorner.x - hh.x);
        vPosClamped.z = clamp(vPosClamped.z, bbox.lowerCorner.z + hh.z, bbox.upperCorner.z - hh.z);
        ...
        wPosClamped.x = clamp(wPosClamped.x, bbox.lowerCorner.x + hh.x, bbox.upperCorner.x - hh.x);
        wPosClamped.y = clamp(wPosClamped.y, bbox.lowerCorner.y + hh.y, bbox.upperCorner.y - hh.y);
...
ApicSolver3 ApicSolver3::Builder::build() const { return ApicSolver3(_resolution, getGridSpacing(), _gridOrigin); }

ApicSolver3Ptr ApicSolver3::Builder::makeShared() const {
    return std::shared_ptr<ApicSolver3>(new ApicSolver3(_resolution, getGridSpacing(), _gridOrigin), [](ApicSolver3* obj) { delete obj; });
}
