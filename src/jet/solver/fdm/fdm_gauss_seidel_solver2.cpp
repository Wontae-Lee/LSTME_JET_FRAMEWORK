...
bool
FdmGaussSeidelSolver2::solve(FdmLinearSystem2* system)
{
...
bool
FdmGaussSeidelSolver2::solveCompressed(FdmCompressedLinearSystem2* system)
{
...
unsigned int
FdmGaussSeidelSolver2::maxNumberOfIterations() const
{
  return _maxNumberOfIterations;
}

unsigned int
FdmGaussSeidelSolver2::lastNumberOfIterations() const
{
  return _lastNumberOfIterations;
}

double
FdmGaussSeidelSolver2::tolerance() const
{
  return _tolerance;
}

double
FdmGaussSeidelSolver2::lastResidual() const
{
  return _lastResidual;
}

double
FdmGaussSeidelSolver2::sorFactor() const
{
  return _sorFactor;
}

bool
FdmGaussSeidelSolver2::useRedBlackOrdering() const
{
  return _useRedBlackOrdering;
}

void
FdmGaussSeidelSolver2::relax(const FdmMatrix2& A, const FdmVector2& b, double sorFactor, FdmVector2* x_)
{
  Size2 size = A.size();
  FdmVector2& x = *x_;

  A.forEachIndex([&](size_t i, size_t j) {
    double r = ((i > 0) ? A(i - 1, j).right * x(i - 1, j) : 0.0) + ((i + 1 < size.x) ? A(i, j).right * x(i + 1, j) : 0.0) +
...
void
FdmGaussSeidelSolver2::relax(const MatrixCsrD& A, const VectorND& b, double sorFactor, VectorND* x_)
{
...
void
FdmGaussSeidelSolver2::relaxRedBlack(const FdmMatrix2& A, const FdmVector2& b, double sorFactor, FdmVector2* x_)
{
  Size2 size = A.size();
  FdmVector2& x = *x_;
...
void
FdmGaussSeidelSolver2::clearUncompressedVectors()
{
  _residual.clear();
}

void
FdmGaussSeidelSolver2::clearCompressedVectors()
{
  _residualComp.clear();
}
