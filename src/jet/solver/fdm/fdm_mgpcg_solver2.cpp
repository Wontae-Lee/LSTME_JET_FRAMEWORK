...
FdmMgpcgSolver2::FdmMgpcgSolver2(unsigned int numberOfCgIter, size_t maxNumberOfLevels, unsigned int numberOfRestrictionIter, unsigned int numberOfCorrectionIter, unsigned int numberOfCoarsestIter,
                                 unsigned int numberOfFinalIter, double maxTolerance, double sorFactor, bool useRedBlackOrdering)
    : FdmMgSolver2(maxNumberOfLevels, numberOfRestrictionIter, numberOfCorrectionIter, numberOfCoarsestIter, numberOfFinalIter, maxTolerance, sorFactor, useRedBlackOrdering),
...
bool FdmMgpcgSolver2::solve(FdmMgLinearSystem2* system) {
    ...
    pcg<FdmBlas2, Preconditioner>(system->A.levels.front(), system->b.levels.front(), _maxNumberOfIterations, _tolerance, &_precond, &system->x.levels.front(), &_r, &_d, &_q, &_s,
