...
void FdmIccgSolver2::Preconditioner::build(const FdmMatrix2& matrix) {
    ...
    matrix.forEachIndex([&](size_t i, size_t j) {
        double denom = matrix(i, j).center - ((i > 0) ? square(matrix(i - 1, j).right) * d(i - 1, j) : 0.0) - ((j > 0) ? square(matrix(i, j - 1).up) * d(i, j - 1) : 0.0);
...
void FdmIccgSolver2::Preconditioner::solve(const FdmVector2& b, FdmVector2* x) {
    ...
    b.forEachIndex([&](size_t i, size_t j) { y(i, j) = (b(i, j) - ((i > 0) ? A(i - 1, j).right * y(i - 1, j) : 0.0) - ((j > 0) ? A(i, j - 1).up * y(i, j - 1) : 0.0)) * d(i, j); });

    for (ssize_t j = sy - 1; j >= 0; --j) {
        for (ssize_t i = sx - 1; i >= 0; --i) {
            (*x)(i, j) = (y(i, j) - ((i + 1 < sx) ? A(i, j).right * (*x)(i + 1, j) : 0.0) - ((j + 1 < sy) ? A(i, j).up * (*x)(i, j + 1) : 0.0)) * d(i, j);
        }
    }
}
...
bool FdmIccgSolver2::solve(FdmLinearSystem2* system) {
    ...
    pcg<FdmBlas2, Preconditioner>(matrix, rhs, _maxNumberOfIterations, _tolerance, &_precond, &solution, &_r, &_d, &_q, &_s, &_lastNumberOfIterations, &_lastResidualNorm);
...
bool FdmIccgSolver2::solveCompressed(FdmCompressedLinearSystem2* system) {
    ...
    pcg<FdmCompressedBlas2, PreconditionerCompressed>(matrix, rhs, _maxNumberOfIterations, _tolerance, &_precondComp, &solution, &_rComp, &_dComp, &_qComp, &_sComp, &_lastNumberOfIterations,
                                                      &_lastResidualNorm);
