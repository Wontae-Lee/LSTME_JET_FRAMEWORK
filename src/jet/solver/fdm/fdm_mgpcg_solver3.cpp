...
FdmMgpcgSolver3::FdmMgpcgSolver3(unsigned int numberOfCgIter, size_t maxNumberOfLevels, unsigned int numberOfRestrictionIter, unsigned int numberOfCorrectionIter, unsigned int numberOfCoarsestIter,
                                 unsigned int numberOfFinalIter, double maxTolerance, double sorFactor, bool useRedBlackOrdering)
    : FdmMgSolver3(maxNumberOfLevels, numberOfRestrictionIter, numberOfCorrectionIter, numberOfCoarsestIter, numberOfFinalIter, maxTolerance, sorFactor, useRedBlackOrdering),
...
bool FdmMgpcgSolver3::solve(FdmMgLinearSystem3* system) {
    ...
    pcg<FdmBlas3, Preconditioner>(system->A.levels.front(), system->b.levels.front(), _maxNumberOfIterations, _tolerance, &_precond, &system->x.levels.front(), &_r, &_d, &_q, &_s,
                                  &_lastNumberOfIterations, &_lastResidualNorm);

    JET_INFO << "Residual after solving MGPCG: " << _lastResidualNorm << " Number of MGPCG iterations: " << _lastNumberOfIterations;
