...
bool
FdmGaussSeidelSolver3::solve(FdmLinearSystem3* system)
{
...
bool
FdmGaussSeidelSolver3::solveCompressed(FdmCompressedLinearSystem3* system)
{
...
unsigned int
FdmGaussSeidelSolver3::maxNumberOfIterations() const
{
  return _maxNumberOfIterations;
}

unsigned int
FdmGaussSeidelSolver3::lastNumberOfIterations() const
{
  return _lastNumberOfIterations;
}

double
FdmGaussSeidelSolver3::tolerance() const
{
  return _tolerance;
}

double
FdmGaussSeidelSolver3::lastResidual() const
{
  return _lastResidual;
}

double
FdmGaussSeidelSolver3::sorFactor() const
{
  return _sorFactor;
}

bool
FdmGaussSeidelSolver3::useRedBlackOrdering() const
{
  return _useRedBlackOrdering;
}

void
FdmGaussSeidelSolver3::relax(const FdmMatrix3& A, const FdmVector3& b, double sorFactor, FdmVector3* x_)
{
  Size3 size = A.size();
  FdmVector3& x = *x_;

  A.forEachIndex([&](size_t i, size_t j, size_t k) {
    double r = ((i > 0) ? A(i - 1, j, k).right * x(i - 1, j, k) : 0.0) + ((i + 1 < size.x) ? A(i, j, k).right * x(i + 1, j, k) : 0.0) +
               ((j > 0) ? A(i, j - 1, k).up * x(i, j - 1, k) : 0.0) + ((j + 1 < size.y) ? A(i, j, k).up * x(i, j + 1, k) : 0.0) +
               ((k > 0) ? A(i, j, k - 1).front * x(i, j, k - 1) : 0.0) + ((k + 1 < size.z) ? A(i, j, k).front * x(i, j, k + 1) : 0.0);
...
void
FdmGaussSeidelSolver3::relax(const MatrixCsrD& A, const VectorND& b, double sorFactor, VectorND* x_)
{
...
void
FdmGaussSeidelSolver3::relaxRedBlack(const FdmMatrix3& A, const FdmVector3& b, double sorFactor, FdmVector3* x_)
{
  Size3 size = A.size();
  FdmVector3& x = *x_;
  ...
  // Red update
  parallelRangeFor(
    kZeroSize, size.x, kZeroSize, size.y, kZeroSize, size.z, [&](size_t iBegin, size_t iEnd, size_t jBegin, size_t jEnd, size_t kBegin, size_t kEnd) {
...
void
FdmGaussSeidelSolver3::clearUncompressedVectors()
{
  _residual.clear();
}

void
FdmGaussSeidelSolver3::clearCompressedVectors()
{
  _residualComp.clear();
}
