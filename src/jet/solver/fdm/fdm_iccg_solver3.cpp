...
void
FdmIccgSolver3::Preconditioner::build(const FdmMatrix3& matrix)
{
...
void
FdmIccgSolver3::Preconditioner::solve(const FdmVector3& b, FdmVector3* x)
{
  Size3 size = b.size();
  ...
  b.forEachIndex([&](size_t i, size_t j, size_t k) {
    y(i, j, k) = (b(i, j, k) - ((i > 0) ? A(i - 1, j, k).right * y(i - 1, j, k) : 0.0) - ((j > 0) ? A(i, j - 1, k).up * y(i, j - 1, k) : 0.0) -
    ...
    for (ssize_t j = sy - 1; j >= 0; --j) {
      for (ssize_t i = sx - 1; i >= 0; --i) {
        (*x)(i, j, k) = (y(i, j, k) - ((i + 1 < sx) ? A(i, j, k).right * (*x)(i + 1, j, k) : 0.0) -
                         ((j + 1 < sy) ? A(i, j, k).up * (*x)(i, j + 1, k) : 0.0) - ((k + 1 < sz) ? A(i, j, k).front * (*x)(i, j, k + 1) : 0.0)) *
                        d(i, j, k);
      }
    }
  }
...
void
FdmIccgSolver3::PreconditionerCompressed::build(const MatrixCsrD& matrix)
{
...
void
FdmIccgSolver3::PreconditionerCompressed::solve(const VectorND& b, VectorND* x)
{
...
bool
FdmIccgSolver3::solve(FdmLinearSystem3* system)
{
  ...
  pcg<FdmBlas3, Preconditioner>(
    matrix, rhs, _maxNumberOfIterations, _tolerance, &_precond, &solution, &_r, &_d, &_q, &_s, &_lastNumberOfIterations, &_lastResidualNorm);

  JET_INFO << "Residual norm after solving ICCG: " << _lastResidualNorm << " Number of ICCG iterations: " << _lastNumberOfIterations;

  return _lastResidualNorm <= _tolerance || _lastNumberOfIterations < _maxNumberOfIterations;
}

bool
FdmIccgSolver3::solveCompressed(FdmCompressedLinearSystem3* system)
{
  ...
  pcg<FdmCompressedBlas3, PreconditionerCompressed>(matrix,
                                                    rhs,
  ...
  JET_INFO << "Residual after solving ICCG: " << _lastResidualNorm << " Number of ICCG iterations: " << _lastNumberOfIterations;

  return _lastResidualNorm <= _tolerance || _lastNumberOfIterations < _maxNumberOfIterations;
}

unsigned int
FdmIccgSolver3::maxNumberOfIterations() const
{
  return _maxNumberOfIterations;
}

unsigned int
FdmIccgSolver3::lastNumberOfIterations() const
{
  return _lastNumberOfIterations;
}

double
FdmIccgSolver3::tolerance() const
{
  return _tolerance;
}

double
FdmIccgSolver3::lastResidual() const
{
  return _lastResidualNorm;
}

void
FdmIccgSolver3::clearUncompressedVectors()
{
  _r.clear();
  _d.clear();
  _q.clear();
...
void
FdmIccgSolver3::clearCompressedVectors()
{
  _r.clear();
  _d.clear();
  _q.clear();
