...
void FdmIccgSolver3::Preconditioner::build(const FdmMatrix3& matrix) {
    ...
    matrix.forEachIndex([&](size_t i, size_t j, size_t k) {
        double denom = matrix(i, j, k).center - ((i > 0) ? square(matrix(i - 1, j, k).right) * d(i - 1, j, k) : 0.0) - ((j > 0) ? square(matrix(i, j - 1, k).up) * d(i, j - 1, k) : 0.0) -
...
void FdmIccgSolver3::Preconditioner::solve(const FdmVector3& b, FdmVector3* x) {
    ...
    b.forEachIndex([&](size_t i, size_t j, size_t k) {
        y(i, j, k) =
            (b(i, j, k) - ((i > 0) ? A(i - 1, j, k).right * y(i - 1, j, k) : 0.0) - ((j > 0) ? A(i, j - 1, k).up * y(i, j - 1, k) : 0.0) - ((k > 0) ? A(i, j, k - 1).front * y(i, j, k - 1) : 0.0)) *
            d(i, j, k);
    });
    ...
    for (ssize_t k = sz - 1; k >= 0; --k) {
        for (ssize_t j = sy - 1; j >= 0; --j) {
            for (ssize_t i = sx - 1; i >= 0; --i) {
                (*x)(i, j, k) = (y(i, j, k) - ((i + 1 < sx) ? A(i, j, k).right * (*x)(i + 1, j, k) : 0.0) - ((j + 1 < sy) ? A(i, j, k).up * (*x)(i, j + 1, k) : 0.0) -
...
FdmIccgSolver3::FdmIccgSolver3(unsigned int maxNumberOfIterations, double tolerance)
    : _maxNumberOfIterations(maxNumberOfIterations), _lastNumberOfIterations(0), _tolerance(tolerance), _lastResidualNorm(kMaxD) {}
...
bool FdmIccgSolver3::solve(FdmLinearSystem3* system) {
    ...
    pcg<FdmBlas3, Preconditioner>(matrix, rhs, _maxNumberOfIterations, _tolerance, &_precond, &solution, &_r, &_d, &_q, &_s, &_lastNumberOfIterations, &_lastResidualNorm);
...
bool FdmIccgSolver3::solveCompressed(FdmCompressedLinearSystem3* system) {
    ...
    pcg<FdmCompressedBlas3, PreconditionerCompressed>(matrix, rhs, _maxNumberOfIterations, _tolerance, &_precondComp, &solution, &_rComp, &_dComp, &_qComp, &_sComp, &_lastNumberOfIterations,
                                                      &_lastResidualNorm);
