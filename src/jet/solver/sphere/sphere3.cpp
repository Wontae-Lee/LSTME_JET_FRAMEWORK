...
Vector3D
Sphere3::closestPointLocal(const Vector3D& otherPoint) const
{
  return radius * closestNormalLocal(otherPoint) + center;
}

double
Sphere3::closestDistanceLocal(const Vector3D& otherPoint) const
{
  return std::fabs(center.distanceTo(otherPoint) - radius);
}

Vector3D
Sphere3::closestNormalLocal(const Vector3D& otherPoint) const
{
...
bool
Sphere3::intersectsLocal(const Ray3D& ray) const
{
...
Sphere3::closestIntersectionLocal(const Ray3D& ray) const
{
...
BoundingBox3D
Sphere3::boundingBoxLocal() const
{
...
Sphere3::Builder&
Sphere3::Builder::withCenter(const Vector3D& center)
{
  _center = center;
  return *this;
}
...
Sphere3::Builder&
Sphere3::Builder::withRadius(double radius)
{
  _radius = radius;
  return *this;
}
...
Sphere3
Sphere3::Builder::build() const
{
  return Sphere3(_center, _radius, _transform, _isNormalFlipped);
}

Sphere3Ptr
Sphere3::Builder::makeShared() const
{
