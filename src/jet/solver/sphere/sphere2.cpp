...
#include "jet/primitive/sphere/sphere2.h"

using namespace jet;

Sphere2::Sphere2(const Transform2& transform_, bool isNormalFlipped_) : Surface2(transform_, isNormalFlipped_) {}

Sphere2::Sphere2(const Vector2D& center_, double radius_, const Transform2& transform_, bool isNormalFlipped_) : Surface2(transform_, isNormalFlipped_), center(center_), radius(radius_) {}

Sphere2::Sphere2(const Sphere2& other) : Surface2(other), center(other.center), radius(other.radius) {}

Vector2D Sphere2::closestPointLocal(const Vector2D& otherPoint) const { return radius * closestNormalLocal(otherPoint) + center; }

double Sphere2::closestDistanceLocal(const Vector2D& otherPoint) const { return std::fabs(center.distanceTo(otherPoint) - radius); }
...
Sphere2Ptr Sphere2::Builder::makeShared() const {
    return std::shared_ptr<Sphere2>(new Sphere2(_center, _radius, _transform, _isNormalFlipped), [](Sphere2* obj) { delete obj; });
}
