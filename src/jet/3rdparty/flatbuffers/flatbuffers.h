...
public:
  explicit vector_downward(size_t initial_size = 1024,
      ...
      : allocator_(allocator ? allocator : &DefaultAllocator::instance()),
        own_allocator_(own_allocator), initial_size_(initial_size),
        reserved_(0), buf_(nullptr), cur_(nullptr) {
    assert(allocator_);
  }
  ~vector_downward() {
    if (buf_) {
...
public:
  ...
  CreateVectorOfStructs(size_t vector_size,
                        const std::function<void(size_t i, T *)> &filler) {
    StartVector(vector_size * sizeof(T) / AlignOf<T>(), AlignOf<T>());
    T *structs =
        reinterpret_cast<T *>(buf_.make_space(vector_size * sizeof(T)));
    for (size_t i = 0; i < vector_size; i++) {
      filler(i, structs);
      structs++;
    }
