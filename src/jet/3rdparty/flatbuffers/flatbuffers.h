...
#define FLATBUFFERS_BYTESWAP16(x) static_cast<uint16_t>(__builtin_bswap32(static_cast<uint32_t>(x) << 16))
#else
...
struct IndirectHelper {
    ...
    static return_type Read(const uint8_t *p, uoffset_t i) { return EndianScalar((reinterpret_cast<const T *>(p))[i]); }
};
template <typename T>
...
struct IndirectHelper<const T *> {
    ...
    typedef T *mutable_return_type;
    static const size_t element_stride = sizeof(T);
    static return_type Read(const uint8_t *p, uoffset_t i) { return reinterpret_cast<const T *>(p + i * sizeof(T)); }
};
...
struct VectorIterator {
    ...
    ptrdiff_t operator-(const VectorIterator &other) const { return (data_ - other.data_) / IndirectHelper<T>::element_stride; }
    ...
    VectorIterator operator+(const uoffset_t &offset) const { return VectorIterator(data_ + offset * IndirectHelper<T>::element_stride, 0); }
    ...
    VectorIterator operator-(const uoffset_t &offset) { return VectorIterator(data_ - offset * IndirectHelper<T>::element_stride, 0); }
...
class DefaultAllocator : public Allocator {
 public:
    virtual uint8_t *allocate(size_t size) FLATBUFFERS_OVERRIDE { return new uint8_t[size]; }

    virtual void deallocate(uint8_t *p, size_t) FLATBUFFERS_OVERRIDE { delete[] p; }
...
class DetachedBuffer {
 public:
    ...
    DetachedBuffer(DetachedBuffer &&other) : allocator_(other.allocator_), own_allocator_(other.own_allocator_), buf_(other.buf_), reserved_(other.reserved_), cur_(other.cur_), size_(other.size_) {
...
class vector_downward {
 public:
    explicit vector_downward(size_t initial_size = 1024, Allocator *allocator = nullptr, bool own_allocator = false)
        : allocator_(allocator ? allocator : &DefaultAllocator::instance()), own_allocator_(own_allocator), initial_size_(initial_size), reserved_(0), buf_(nullptr), cur_(nullptr) {
        assert(allocator_);
    }
    ...
    size_t growth_policy(size_t bytes) { return (bytes == 0) ? initial_size_ : ((bytes / 2) & ~(AlignOf<largest_scalar_t>() - 1)); }
    ...
    uoffset_t size() const { return static_cast<uoffset_t>(reserved_ - (cur_ - buf_)); }
...
class FlatBufferBuilder {
 public:
    ...
    explicit FlatBufferBuilder(size_t initial_size = 1024, Allocator *allocator = nullptr, bool own_allocator = false)
        : buf_(initial_size, allocator, own_allocator), nested(false), finished(false), minalign_(1), force_defaults_(false), dedup_vtables_(true), string_pool(nullptr) {
    ...
    uoffset_t EndTable(uoffset_t start, voffset_t numfields) {
        ...
        for (auto field_location = offsetbuf_.begin(); field_location != offsetbuf_.end(); ++field_location) {
        ...
        WriteScalar(buf_.data_at(vtableoffsetloc), static_cast<soffset_t>(vt_use) - static_cast<soffset_t>(vtableoffsetloc));
    ...
    void PreAlign(size_t len, size_t alignment) { buf_.fill(PaddingBytes(GetSize() + len, alignment)); }
    template <typename T>
    ...
    /// @return Returns the offset in the buffer where the string starts
    Offset<String> CreateString(const String *str) { return str ? CreateString(str->c_str(), str->Length()) : 0; }
    ...
    /// @param[in] str A const char pointer to a C-string to add to the buffer.
    /// @return Returns the offset in the buffer where the string starts.
    Offset<String> CreateSharedString(const char *str) { return CreateSharedString(str, strlen(str)); }
    ...
    /// @param[in] str A const reference to a std::string to store in the buffer.
    /// @return Returns the offset in the buffer where the string starts.
    Offset<String> CreateSharedString(const std::string &str) { return CreateSharedString(str.c_str(), str.length()); }
    ...
    /// @param[in] str A const pointer to a `String` struct to add to the buffer.
    /// @return Returns the offset in the buffer where the string starts
    Offset<String> CreateSharedString(const String *str) { return CreateSharedString(str->c_str(), str->Length()); }
    ...
    void ForceVectorAlignment(size_t len, size_t elemsize, size_t alignment) { PreAlign(len * elemsize, alignment); }

    uint8_t *ReserveElements(size_t len, size_t elemsize) { return buf_.make_space(len * elemsize); }
    /// @endcond
 ...
 protected:
    ...
    void Finish(uoffset_t root, const char *file_identifier, bool size_prefix) {
        ...
        PreAlign((size_prefix ? sizeof(uoffset_t) : 0) + sizeof(uoffset_t) + (file_identifier ? kFileIdentifierLength : 0), minalign_);
        if (file_identifier) {
...
/// vtable).
inline const char *GetBufferIdentifier(const void *buf) { return reinterpret_cast<const char *>(buf) + sizeof(uoffset_t); }

// Helper to see if the identifier in a buffer has the expected value.
inline bool BufferHasIdentifier(const void *buf, const char *identifier) { return strncmp(GetBufferIdentifier(buf), identifier, FlatBufferBuilder::kFileIdentifierLength) == 0; }
...
class Table {
 public:
    ...
    const uint8_t *GetAddressOf(voffset_t field) const { return const_cast<Table *>(this)->GetAddressOf(field); }
    ...
    bool VerifyTableStart(Verifier &verifier) const {
        ...
        return verifier.VerifyComplexity() && verifier.Verify<voffset_t>(vtable) && (ReadScalar<voffset_t>(vtable) & (sizeof(voffset_t) - 1)) == 0 &&
...
inline const uint8_t *GetBufferStartFromRootPointer(const void *root) {
    ...
    static_assert(FlatBufferBuilder::kFileIdentifierLength == sizeof(uoffset_t), "file_identifier is assumed to be the same size as uoffset_t");
...
#define MANUALLY_ALIGNED_STRUCT(alignment) _Pragma("pack(1)") struct __attribute__((aligned(alignment)))
#define STRUCT_END(name, size) _Pragma("pack()") static_assert(sizeof(name) == size, "compiler breaks packing rules")
#else
...
volatile __attribute__((weak)) const char *flatbuffer_version_string =
    "FlatBuffers " FLATBUFFERS_STRING(FLATBUFFERS_VERSION_MAJOR) "." FLATBUFFERS_STRING(FLATBUFFERS_VERSION_MINOR) "." FLATBUFFERS_STRING(FLATBUFFERS_VERSION_REVISION);
