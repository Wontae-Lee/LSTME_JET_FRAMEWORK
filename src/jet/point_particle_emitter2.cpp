...
PointParticleEmitter2::PointParticleEmitter2(const Vector2D& origin, const Vector2D& direction, double speed, double spreadAngleInDegrees, size_t maxNumOfNewParticlesPerSec, size_t maxNumOfParticles,
                                             uint32_t seed)
    : _rng(seed),
...
void PointParticleEmitter2::setMaxNumberOfNewParticlesPerSecond(size_t rate) { _maxNumberOfNewParticlesPerSecond = rate; }

size_t PointParticleEmitter2::maxNumberOfParticles() const { return _maxNumberOfParticles; }

void PointParticleEmitter2::setMaxNumberOfParticles(size_t maxNumberOfParticles) { _maxNumberOfParticles = maxNumberOfParticles; }
...
void PointParticleEmitter2::onUpdate(double currentTimeInSeconds, double timeIntervalInSeconds) {
    ...
    size_t newMaxTotalNumberOfEmittedParticles = static_cast<size_t>(std::ceil((elapsedTimeInSeconds + timeIntervalInSeconds) * _maxNumberOfNewParticlesPerSecond));
...
void PointParticleEmitter2::emit(Array1<Vector2D>* newPositions, Array1<Vector2D>* newVelocities, size_t maxNewNumberOfParticles) {
...
PointParticleEmitter2Ptr PointParticleEmitter2::Builder::makeShared() const {
    return std::shared_ptr<PointParticleEmitter2>(new PointParticleEmitter2(_origin, _direction, _speed, _spreadAngleInDegrees, _maxNumberOfNewParticlesPerSecond, _maxNumberOfParticles, _seed),
