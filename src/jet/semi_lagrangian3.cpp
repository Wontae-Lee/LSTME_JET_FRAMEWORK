...
#include "jet/solver/lagrangian/semi_lagrangian3.h"
#include <pch.h>
#include <algorithm>
...
void SemiLagrangian3::advect(const ScalarGrid3& input, const VectorField3& flow, double dt, ScalarGrid3* output, const ScalarField3& boundarySdf) {
    ...
    double h = min3(output->gridSpacing().x, output->gridSpacing().y, output->gridSpacing().z);
...
void SemiLagrangian3::advect(const CollocatedVectorGrid3& input, const VectorField3& flow, double dt, CollocatedVectorGrid3* output, const ScalarField3& boundarySdf) {
...
void SemiLagrangian3::advect(const FaceCenteredGrid3& input, const VectorField3& flow, double dt, FaceCenteredGrid3* output, const ScalarField3& boundarySdf) {
    auto inputSamplerFunc = getVectorSamplerFunc(input);
...
Vector3D SemiLagrangian3::backTrace(const VectorField3& flow, double dt, double h, const Vector3D& startPt, const ScalarField3& boundarySdf) {
...
std::function<double(const Vector3D&)> SemiLagrangian3::getScalarSamplerFunc(const ScalarGrid3& input) const { return input.sampler(); }

std::function<Vector3D(const Vector3D&)> SemiLagrangian3::getVectorSamplerFunc(const CollocatedVectorGrid3& input) const { return input.sampler(); }

std::function<Vector3D(const Vector3D&)> SemiLagrangian3::getVectorSamplerFunc(const FaceCenteredGrid3& input) const { return input.sampler(); }
