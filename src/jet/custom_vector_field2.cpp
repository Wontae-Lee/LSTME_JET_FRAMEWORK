...
CustomVectorField2::CustomVectorField2(const std::function<Vector2D(const Vector2D&)>& customFunction, double derivativeResolution)
    : _customFunction(customFunction), _resolution(derivativeResolution) {}

CustomVectorField2::CustomVectorField2(const std::function<Vector2D(const Vector2D&)>& customFunction, const std::function<double(const Vector2D&)>& customDivergenceFunction,
...
Vector2D CustomVectorField2::sample(const Vector2D& x) const { return _customFunction(x); }

std::function<Vector2D(const Vector2D&)> CustomVectorField2::sampler() const { return _customFunction; }
...
CustomVectorField2Ptr CustomVectorField2::Builder::makeShared() const {
    if (_customCurlFunction) {
        return std::shared_ptr<CustomVectorField2>(new CustomVectorField2(_customFunction, _customDivergenceFunction, _customCurlFunction), [](CustomVectorField2* obj) { delete obj; });
    } else {
        return std::shared_ptr<CustomVectorField2>(new CustomVectorField2(_customFunction, _customDivergenceFunction, _resolution), [](CustomVectorField2* obj) { delete obj; });
    }
}
