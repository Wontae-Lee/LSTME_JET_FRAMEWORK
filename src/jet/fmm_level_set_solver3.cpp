...
void FmmLevelSetSolver3::reinitialize(const ScalarGrid3 &inputSdf,
  ...
  markers.forEachIndex([&](size_t i, size_t j, size_t k) {
    if (isInsideSdf(output(i, j, k)) &&
      ...
      output(i, j, k) =
          solveQuadNearBoundary(markers, inputSdf.constDataAccessor(),
                                gridSpacing, invGridSpacingSqr, -1.0, i, j, k);
    }
  });
  markers.forEachIndex([&](size_t i, size_t j, size_t k) {
    if (!isInsideSdf(output(i, j, k)) &&
    ...
    while (!trial.empty()) {
      ...
      if (i > 0) {
        if (markers(i - 1, j, k) == kUnknown) {
          ...
          output(i - 1, j, k) = solveQuad(markers, output, gridSpacing,
      ...
      if (j > 0) {
        if (markers(i, j - 1, k) == kUnknown) {
          ...
          output(i, j - 1, k) = solveQuad(markers, output, gridSpacing,
      ...
      if (k > 0) {
        if (markers(i, j, k - 1) == kUnknown) {
          ...
          output(i, j, k - 1) = solveQuad(markers, output, gridSpacing,
