...
void restrict(const Array3<float> &finer, Array3<float> *coarser) {
  ...
  parallelRangeFor(
      ...
      [&](size_t iBegin, size_t iEnd, size_t jBegin, size_t jEnd, size_t kBegin,
          size_t kEnd) {
...
void GridFractionalSinglePhasePressureSolver3::buildSystem(
  ...
  if (_mgSystemSolver == nullptr) {
    if (useCompressed) {
      buildSingleSystem(&_compSystem.A, &_compSystem.x, &_compSystem.b,
                        _fluidSdf[0], _uWeights[0], _vWeights[0], _wWeights[0],
                        _boundaryVel, *finer);
    } else {
      buildSingleSystem(&_system.A, &_system.b, _fluidSdf[0], _uWeights[0],
                        _vWeights[0], _wWeights[0], _boundaryVel, *finer);
    }
  } else {
    buildSingleSystem(&_mgSystem.A.levels.front(), &_mgSystem.b.levels.front(),
                      _fluidSdf[0], _uWeights[0], _vWeights[0], _wWeights[0],
                      _boundaryVel, *finer);
  }
  // Build sub-levels
  ...
  for (size_t l = 1; l < numLevels; ++l) {
    ...
    buildSingleSystem(&_mgSystem.A.levels[l], &_mgSystem.b.levels[l],
                      _fluidSdf[l], _uWeights[l], _vWeights[l], _wWeights[l],
