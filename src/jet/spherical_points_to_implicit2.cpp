...
#include "jet/transfer/spherical_points_to_implicit2.h"

using namespace jet;

SphericalPointsToImplicit2::SphericalPointsToImplicit2(double radius, bool isOutputSdf) : _radius(radius), _isOutputSdf(isOutputSdf) {}
...
void SphericalPointsToImplicit2::convert(const ConstArrayAccessor1<Vector2D>& points, ScalarGrid2* output) const {
    ...
    temp->fill([&](const Vector2D& x) {
        ...
        neighborSearcher->forEachNearbyPoint(x, 2.0 * _radius, [&](size_t, const Vector2D& xj) { minDist = std::min(minDist, (x - xj).length()); });
