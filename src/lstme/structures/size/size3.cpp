...
// Basic setters
inline void
Size3::set(size_t newX, size_t newY, size_t newZ)
{
  x = newX;
  y = newY;
  z = newZ;
...
inline void
Size3::set(const Size2& sz, size_t newZ)
{
  x = sz.x;
  y = sz.y;
  z = newZ;
...
template<typename U>
inline void
Size3::set(const std::initializer_list<U>& lst)
{
  assert(lst.size() >= 3);
...
inline size_t
Size3::dominantAxis() const
{
  return (x > y) ? ((x > z) ? 0 : 2) : ((y > z) ? 1 : 2);
}

inline size_t
Size3::subminantAxis() const
{
  return (x < y) ? ((x < z) ? 0 : 2) : ((y < z) ? 1 : 2);
}

inline bool
Size3::isEqual(const Size3& other) const
{
...
inline bool
Size3::operator==(const Size3& v) const
{
  return isEqual(v);
}
...
inline Size3
operator+(const Size3& a)
{
  return a;
}

inline Size3
operator+(const Size3& a, size_t b)
{
  return a.add(b);
}

inline Size3
operator+(size_t a, const Size3& b)
{
  return b.add(a);
}

inline Size3
operator+(const Size3& a, const Size3& b)
{
  return a.add(b);
}
...
inline Size3
min(const Size3& a, const Size3& b)
{
  return Size3(std::min(a.x, b.x), std::min(a.y, b.y), std::min(a.z, b.z));
}

inline Size3
max(const Size3& a, const Size3& b)
{
  return Size3(std::max(a.x, b.x), std::max(a.y, b.y), std::max(a.z, b.z));
}

inline Size3
clamp(const Size3& v, const Size3& low, const Size3& high)
{
  return Size3(clamp(v.x, low.x, high.x),
