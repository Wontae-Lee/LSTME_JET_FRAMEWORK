//
// Created by LSTME on 2022-09-19.
//
...
template<typename U>
void
Point<T, N>::set(const std::initializer_list<U>& lst)
{
  assert(lst.size() >= N);
...
template<typename T, size_t N>
void
Point<T, N>::set(const Point& other)
{
  _elements = other._elements;
}
...
template<typename U>
Point<T, N>&
Point<T, N>::operator=(const std::initializer_list<U>& lst)
{
  set(lst);
  return *this;
}
...
template<typename T, size_t N>
const T&
Point<T, N>::operator[](size_t i) const
{
  return _elements[i];
}
...
template<typename... Params>
void
Point<T, N>::setAt(size_t i, T v, Params... params)
{
  _elements[i] = v;
