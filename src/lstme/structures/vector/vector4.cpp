...
{
  return lstme::absmin(lstme::absmin(x, y), lstme::absmin(z, w));
}
...
{
  return lstme::absmax(lstme::absmax(x, y), lstme::absmax(z, w));
}
...
{
  return Vector<T, 4>(clamp(v.x, low.x, high.x),
...
template<typename T>
Vector<T, 4>
monotonicCatmullRom(const Vector<T, 4>& v0,
                    const Vector<T, 4>& v1,
                    const Vector<T, 4>& v2,
                    const Vector<T, 4>& v3,
                    T f)
