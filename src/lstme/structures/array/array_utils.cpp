...
template<typename ArrayType1, typename ArrayType2>
void
copyRange1(const ArrayType1& input,
           size_t begin,
           size_t end,
           ArrayType2* output)
{
  parallelFor(
    begin, end, [&input, &output](size_t i) { (*output)[i] = input[i]; });
}

template<typename ArrayType1, typename ArrayType2>
void
copyRange2(const ArrayType1& input,
           size_t sizeX,
           size_t sizeY,
           ArrayType2* output)
{
...
{
  parallelFor(
    beginX, endX, beginY, endY, [&input, &output](size_t i, size_t j) {
...
template<typename ArrayType1, typename ArrayType2>
void
copyRange3(const ArrayType1& input,
           size_t sizeX,
           size_t sizeY,
           size_t sizeZ,
           ArrayType2* output)
                    ...
                    ArrayAccessor2<T> output)
{
  ...
  LSTME_ASSERT(size == valid.size());
  LSTME_ASSERT(size == output.size());
