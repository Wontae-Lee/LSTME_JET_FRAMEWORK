...
inline std::ostream*
levelToStream(LoggingLevel level)
{
  switch (level) {
...
inline std::string
levelToString(LoggingLevel level)
{
  switch (level) {
    case LoggingLevel::Info:
      return "INFO";
...
inline bool
isLeq(LoggingLevel a, LoggingLevel b)
{
...
void
Logging::setInfoStream(std::ostream* strm)
{
  std::lock_guard<std::mutex> lock(critical);
  infoOutStream = strm;
}
...
void
Logging::setWarnStream(std::ostream* strm)
{
  std::lock_guard<std::mutex> lock(critical);
  warnOutStream = strm;
}
...
void
Logging::setErrorStream(std::ostream* strm)
{
  std::lock_guard<std::mutex> lock(critical);
  errorOutStream = strm;
}
...
void
Logging::setDebugStream(std::ostream* strm)
{
  std::lock_guard<std::mutex> lock(critical);
  debugOutStream = strm;
}
...
void
Logging::setAllStream(std::ostream* strm)
{
  setInfoStream(strm);
  setWarnStream(strm);
  setErrorStream(strm);
...
std::string
Logging::getHeader(LoggingLevel level)
{
  auto now =
  ...
  char header[256];
  snprintf(
    header, sizeof(header), "[%s] %s ", levelToString(level).c_str(), timeStr);
  return header;
}

void
Logging::setLevel(LoggingLevel level)
{
  std::lock_guard<std::mutex> lock(critical);
  sLoggingLevel = level;
}
...
Logging::mute()
{
  setLevel(LoggingLevel::Off);
}

void
Logging::unmute()
{
  setLevel(LoggingLevel::All);
}
