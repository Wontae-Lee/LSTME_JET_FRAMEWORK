...
#include <generated/grid_system_data2_generated.hpp>
...
GridSystemData2::GridSystemData2(const Size2& resolution,
...
GridSystemData2::GridSystemData2(const GridSystemData2& other)
{
  ...
  LSTME_ASSERT(_advectableVectorDataList.size() > 0);

  _velocity =
    std::dynamic_pointer_cast<FaceCenteredGrid2>(_advectableVectorDataList[0]);

  LSTME_ASSERT(_velocity != nullptr);
...
void
GridSystemData2::resize(const Size2& resolution,
                        const Vector2D& gridSpacing,
                        const Vector2D& origin)
{
...
Size2
GridSystemData2::resolution() const
{
  return _resolution;
}

Vector2D
GridSystemData2::gridSpacing() const
{
  return _gridSpacing;
}

Vector2D
GridSystemData2::origin() const
{
  return _origin;
}

BoundingBox2D
GridSystemData2::boundingBox() const
{
  return _velocity->boundingBox();
}

size_t
GridSystemData2::addScalarData(const ScalarGridBuilder2Ptr& builder,
                               double initialVal)
{
...
size_t
GridSystemData2::addVectorData(const VectorGridBuilder2Ptr& builder,
...
size_t
GridSystemData2::addAdvectableScalarData(const ScalarGridBuilder2Ptr& builder,
                                         double initialVal)
{
...
size_t
GridSystemData2::addAdvectableVectorData(const VectorGridBuilder2Ptr& builder,
                                         const Vector2D& initialVal)
{
...
GridSystemData2::velocity() const
{
  return _velocity;
}

size_t
GridSystemData2::velocityIndex() const
{
  return _velocityIdx;
}

const ScalarGrid2Ptr&
GridSystemData2::scalarDataAt(size_t idx) const
{
  return _scalarDataList[idx];
}

const VectorGrid2Ptr&
GridSystemData2::vectorDataAt(size_t idx) const
{
...
size_t
GridSystemData2::numberOfScalarData() const
{
  return _scalarDataList.size();
}

size_t
GridSystemData2::numberOfVectorData() const
{
  return _vectorDataList.size();
}

size_t
GridSystemData2::numberOfAdvectableScalarData() const
{
  return _advectableScalarDataList.size();
}

size_t
GridSystemData2::numberOfAdvectableVectorData() const
{
  return _advectableVectorDataList.size();
}

void
GridSystemData2::serialize(std::vector<uint8_t>* buffer) const
{
  flatbuffers::FlatBufferBuilder builder(1024);

  auto resolution = lstmeToFbs(_resolution);
  auto gridSpacing = lstmeToFbs(_gridSpacing);
  auto origin = lstmeToFbs(_origin);
  ...
  auto gsd =
    fbs::CreateGridSystemData2(builder,
                               &resolution,
                               &gridSpacing,
                               &origin,
                               _velocityIdx,
...
void
GridSystemData2::deserialize(const std::vector<uint8_t>& buffer)
{
  auto gsd = fbs::GetGridSystemData2(buffer.data());

  resize(fbsToJet(*gsd->resolution()),
  ...
  deserializeGrid(
    gsd->scalarData(), Factory::buildScalarGrid2, &_scalarDataList);
  deserializeGrid(
    gsd->vectorData(), Factory::buildVectorGrid2, &_vectorDataList);
  deserializeGrid(gsd->advectableScalarData(),
                  Factory::buildScalarGrid2,
                  &_advectableScalarDataList);
  deserializeGrid(gsd->advectableVectorData(),
