...
#include <generated/particle_system_data2_generated.hpp>
...
void
ParticleSystemData2::addParticle(const Vector2D& newPosition,
                                 ...
                                 const Vector2D& newForce)
{
  ...
  addParticles(newPositions.constAccessor(),
               newVelocities.constAccessor(),
               newForces.constAccessor());
}

void
ParticleSystemData2::addParticles(
...
{
  LSTME_THROW_INVALID_ARG_IF(newVelocities.size() > 0 &&
                             newVelocities.size() != newPositions.size());
  LSTME_THROW_INVALID_ARG_IF(newForces.size() > 0 &&
...
ParticleSystemData2::buildNeighborSearcher(double maxSearchRadius)
{
  ...
  // Use PointParallelHashGridSearcher2 by default
  _neighborSearcher =
    std::make_shared<PointParallelHashGridSearcher2>(kDefaultHashGridResolution,
                                                     kDefaultHashGridResolution,
                                                     2.0 * maxSearchRadius);

  _neighborSearcher->build(positions());

  LSTME_INFO << "Building neighbor searcher took: " << timer.durationInSeconds()
             << " seconds";
}
...
ParticleSystemData2::buildNeighborLists(double maxSearchRadius)
{
  ...
  LSTME_INFO << "Building neighbor list took: " << timer.durationInSeconds()
             << " seconds";
}
...
void
ParticleSystemData2::serializeParticleSystemData(
  ...
  flatbuffers::Offset<fbs::ParticleSystemData2>* fbsParticleSystemData) const
{
  ...
  for (const auto& vectorData : _vectorDataList) {
    ...
    for (const auto& v : vectorData) {
      newVectorData.push_back(lstmeToFbs(v));
    }

    auto fbsVectorData = fbs::CreateVectorParticleData2(
      *builder,
      builder->CreateVectorOfStructs(newVectorData.data(),
                                     newVectorData.size()));
  ...
  // Copy the searcher
  *fbsParticleSystemData = fbs::CreateParticleSystemData2(*builder,
                                                          _radius,
                                                          _mass,
                                                          _positionIdx,
                                                          _velocityIdx,
...
void
ParticleSystemData2::deserializeParticleSystemData(
  const fbs::ParticleSystemData2* fbsParticleSystemData)
{
  ...
  _neighborSearcher =
    Factory::buildPointNeighborSearcher2(fbsNeighborSearcher->type()->c_str());
  std::vector<uint8_t> neighborSearcherSerialized(
