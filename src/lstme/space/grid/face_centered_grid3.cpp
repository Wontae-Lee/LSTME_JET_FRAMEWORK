...
FaceCenteredGrid3::FaceCenteredGrid3()
  ...
  , _uLinearSampler(LinearArraySampler3<double, double>(_dataU.constAccessor(),
                                                        Vector3D(1, 1, 1),
                                                        _dataOriginU))
  , _vLinearSampler(LinearArraySampler3<double, double>(_dataV.constAccessor(),
                                                        Vector3D(1, 1, 1),
                                                        _dataOriginV))
  , _wLinearSampler(LinearArraySampler3<double, double>(_dataW.constAccessor(),
                                                        Vector3D(1, 1, 1),
                                                        _dataOriginW))
{
}
...
{
  LSTME_ASSERT(i < resolution().x && j < resolution().y && k < resolution().z);
...
FaceCenteredGrid3::curlAtCellCenter(size_t i, size_t j, size_t k) const
{
  ...
  LSTME_ASSERT(i < res.x && j < res.y && k < res.z);
...
void
FaceCenteredGrid3::fill(const std::function<Vector3D(const Vector3D&)>& func,
...
void
FaceCenteredGrid3::forEachUIndex(
...
void
FaceCenteredGrid3::parallelForEachUIndex(
  const std::function<void(size_t, size_t, size_t)>& func) const
{
...
void
FaceCenteredGrid3::forEachVIndex(
  const std::function<void(size_t, size_t, size_t)>& func) const
{
...
void
FaceCenteredGrid3::parallelForEachVIndex(
  const std::function<void(size_t, size_t, size_t)>& func) const
{
...
void
FaceCenteredGrid3::forEachWIndex(
  const std::function<void(size_t, size_t, size_t)>& func) const
{
...
void
FaceCenteredGrid3::parallelForEachWIndex(
  const std::function<void(size_t, size_t, size_t)>& func) const
{
...
FaceCenteredGrid3::divergence(const Vector3D& x) const
{
  ...
  for (int n = 0; n < 8; ++n) {
    result += weights[n] *
              divergenceAtCellCenter(indices[n].x, indices[n].y, indices[n].z);
  }
...
FaceCenteredGrid3::curl(const Vector3D& x) const
{
  ...
  for (int n = 0; n < 8; ++n) {
    result +=
      weights[n] * curlAtCellCenter(indices[n].x, indices[n].y, indices[n].z);
  }
...
void
FaceCenteredGrid3::onResize(const Size3& resolution,
                            const Vector3D& gridSpacing,
                            const Vector3D& origin,
                            const Vector3D& initialValue)
{
...
FaceCenteredGrid3::resetSampler()
{
  LinearArraySampler3<double, double> uSampler(
    _dataU.constAccessor(), gridSpacing(), _dataOriginU);
  LinearArraySampler3<double, double> vSampler(
    _dataV.constAccessor(), gridSpacing(), _dataOriginV);
  LinearArraySampler3<double, double> wSampler(
    _dataW.constAccessor(), gridSpacing(), _dataOriginW);
...
{
  LSTME_ASSERT(uSize().x * uSize().y * uSize().z +
                 vSize().x * vSize().y * vSize().z +
                 wSize().x * wSize().y * wSize().z ==
               data.size());
...
FaceCenteredGrid3::Builder&
FaceCenteredGrid3::Builder::withResolution(size_t resolutionX,
                                           size_t resolutionY,
                                           size_t resolutionZ)
{
...
FaceCenteredGrid3::Builder&
FaceCenteredGrid3::Builder::withGridSpacing(double gridSpacingX,
                                            double gridSpacingY,
                                            double gridSpacingZ)
{
...
FaceCenteredGrid3::Builder&
FaceCenteredGrid3::Builder::withOrigin(double gridOriginX,
                                       double gridOriginY,
                                       double gridOriginZ)
{
...
FaceCenteredGrid3::Builder&
FaceCenteredGrid3::Builder::withInitialValue(double initialValX,
                                             double initialValY,
                                             double initialValZ)
{
...
VectorGrid3Ptr
FaceCenteredGrid3::Builder::build(const Size3& resolution,
                                  const Vector3D& gridSpacing,
