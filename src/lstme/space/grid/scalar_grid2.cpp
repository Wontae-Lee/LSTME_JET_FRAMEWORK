...
#include <generated/scalar_grid2_generated.hpp>
...
ScalarGrid2::ScalarGrid2()
  : _linearSampler(LinearArraySampler2<double, double>(_data.constAccessor(),
                                                       Vector2D(1, 1),
                                                       Vector2D()))
{
}
...
void
ScalarGrid2::resize(const Size2& resolution,
...
void
ScalarGrid2::resize(double gridSpacingX,
                    double gridSpacingY,
                    double originX,
                    double originY)
{
...
void
ScalarGrid2::fill(const std::function<double(const Vector2D&)>& func,
                  ExecutionPolicy policy)
{
  ...
  parallelFor(
    ...
    kZeroSize,
    _data.height(),
    [this, &func, &pos](size_t i, size_t j) { _data(i, j) = func(pos(i, j)); },
    policy);
}

void
ScalarGrid2::forEachDataPointIndex(
...
void
ScalarGrid2::parallelForEachDataPointIndex(
  const std::function<void(size_t, size_t)>& func) const
{
...
{
  LSTME_ASSERT(dataSize().x * dataSize().y == data.size());
