...
CollocatedVectorGrid3::divergenceAtDataPoint(size_t i, size_t j, size_t k) const
{
  ...
  LSTME_ASSERT(i < ds.x && j < ds.y && k < ds.z);
...
CollocatedVectorGrid3::divergence(const Vector3D& x) const
{
  ...
  for (int i = 0; i < 8; ++i) {
    result += weights[i] *
              divergenceAtDataPoint(indices[i].x, indices[i].y, indices[i].z);
  }
...
CollocatedVectorGrid3::curl(const Vector3D& x) const
{
  ...
  for (int i = 0; i < 8; ++i) {
    result +=
      weights[i] * curlAtDataPoint(indices[i].x, indices[i].y, indices[i].z);
  }
...
void
CollocatedVectorGrid3::forEachDataPointIndex(
...
void
CollocatedVectorGrid3::parallelForEachDataPointIndex(
  const std::function<void(size_t, size_t, size_t)>& func) const
{
...
void
CollocatedVectorGrid3::setCollocatedVectorGrid(
...
void
CollocatedVectorGrid3::onResize(const Size3& resolution,
...
{
  LSTME_ASSERT(3 * dataSize().x * dataSize().y * dataSize().z == data.size());
