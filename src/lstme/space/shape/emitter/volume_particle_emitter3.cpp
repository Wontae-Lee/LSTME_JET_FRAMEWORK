...
void
VolumeParticleEmitter3::onUpdate(double currentTimeInSeconds,
...
void
VolumeParticleEmitter3::emit(const ParticleSystemData3Ptr& particles,
                             Array1<Vector3D>* newPositions,
                             Array1<Vector3D>* newVelocities)
{
  if (!_implicitSurface) {
  ...
  if (_allowOverlapping || _isOneShot) {
    ...
    PointHashGridSearcher3 neighborSearcher(Size3(kDefaultHashGridResolution,
  ...
  LSTME_INFO << "Number of newly generated particles: " << numNewParticles;
  LSTME_INFO << "Number of total generated particles: "
             << _numberOfEmittedParticles;

  newVelocities->resize(newPositions->size());
  newVelocities->parallelForEachIndex(
    [&](size_t i) { (*newVelocities)[i] = velocityAt((*newPositions)[i]); });
}

void
VolumeParticleEmitter3::setPointGenerator(
...
{
  return VolumeParticleEmitter3(_implicitSurface,
                                _bounds,
                                _spacing,
                                _initialVel,
                                _linearVel,
...
VolumeParticleEmitter3::Builder::makeShared() const
{
  return std::shared_ptr<VolumeParticleEmitter3>(
    new VolumeParticleEmitter3(_implicitSurface,
                               _bounds,
                               _spacing,
                               _initialVel,
                               _linearVel,
