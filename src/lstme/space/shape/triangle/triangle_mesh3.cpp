                   ...
                   const WindingNumberGatherData& initGatherData)
{
  ...
  if (bvh.isLeaf(nodeIndex)) {
    ...
    data = data + postOrderTraversal(
                    bvh, children.first, visitorFunc, leafFunc, initGatherData);
    data =
      data + postOrderTraversal(
               bvh, children.second, visitorFunc, leafFunc, initGatherData);
  }
...
void
TriangleMesh3::addPointNormalTriangle(const Point3UI& newPointIndices,
...
void
TriangleMesh3::addPointUvNormalTriangle(const Point3UI& newPointIndices,
...
void
TriangleMesh3::addPointUvTriangle(const Point3UI& newPointIndices,
                                  const Point3UI& newUvIndices)
{
  _pointIndices.append(newPointIndices);
  _uvIndices.append(newUvIndices);
...
{
  parallelFor(kZeroSize, numberOfPoints(), [this, factor](size_t i) {
    _points[i] *= factor;
  });
  invalidateCache();
}
...
{
  parallelFor(
    kZeroSize, numberOfPoints(), [this, t](size_t i) { _points[i] += t; });
  invalidateCache();
}
...
{
  parallelFor(kZeroSize, numberOfPoints(), [this, q](size_t i) {
    _points[i] = q * _points[i];
  });

  parallelFor(kZeroSize, numberOfNormals(), [this, q](size_t i) {
...
TriangleMesh3::buildWindingNumbers() const
{
  ...
  if (_wnInvalidated) {
    ...
    postOrderTraversal(
      _bvh, 0, visitorFunc, leafFunc, WindingNumberGatherData());
...
double
TriangleMesh3::fastWindingNumber(const Vector3D& queryPoint,
...
double
TriangleMesh3::fastWindingNumber(const Vector3D& q,
                                 ...
                                 double accuracy) const
{
  ...
  const Vector3D treeRVec =
    lstme::max(treeP - treeBound.lowerCorner, treeBound.upperCorner - treeP);
...
{
  return std::shared_ptr<TriangleMesh3>(new TriangleMesh3(_points,
                                                          _normals,
                                                          _uvs,
                                                          _pointIndices,
                                                          _normalIndices,
