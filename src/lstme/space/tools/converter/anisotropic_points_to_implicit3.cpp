  ...
  ScalarGrid3* output) const
{
  ...
  LSTME_INFO << "Start converting points to implicit surface.";
  ...
  LSTME_INFO << "Built neighbor searcher.";
  ...
  LSTME_INFO << "Computed G and means.";
  ...
  LSTME_INFO << "Computed SDF.";

  if (_isOutputSdf) {
    FmmLevelSetSolver3 solver;
    solver.reinitialize(*temp, kMaxD, output);

    LSTME_INFO << "Completed einitialization.";
  } else {
    temp->swap(output);
  }

  LSTME_INFO << "Done converting points to implicit surface.";
}
