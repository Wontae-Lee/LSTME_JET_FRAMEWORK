...
NearestArraySampler2<T, R>::operator()(const Vector2<R>& x) const
{
  ...
  LSTME_ASSERT(_gridSpacing.x > std::numeric_limits<R>::epsilon() &&
...
template<typename T, typename R>
void
NearestArraySampler2<T, R>::getCoordinate(const Vector2<R>& x,
...
template<typename T, typename R>
void
LinearArraySampler2<T, R>::getCoordinatesAndWeights(
  const Vector2<R>& x,
...
template<typename T, typename R>
void
LinearArraySampler2<T, R>::getCoordinatesAndGradientWeights(
  const Vector2<R>& x,
  std::array<Point2UI, 4>* indices,
  std::array<Vector2<R>, 4>* weights) const
{
  ...
  (*weights)[1] = Vector2<R>(-fy * _invGridSpacing.x + _invGridSpacing.x,
                             -fx * _invGridSpacing.y);
  (*weights)[2] = Vector2<R>(-fy * _invGridSpacing.x,
                             -fx * _invGridSpacing.y + _invGridSpacing.y);
...
CubicArraySampler2<T, R>::operator()(const Vector2<R>& x) const
{
  ...
  T values[4];
  for (int n = 0; n < 4; ++n) {
    values[n] = monotonicCatmullRom(_accessor(is[0], js[n]),
                                    _accessor(is[1], js[n]),
                                    _accessor(is[2], js[n]),
                                    _accessor(is[3], js[n]),
                                    fx);
