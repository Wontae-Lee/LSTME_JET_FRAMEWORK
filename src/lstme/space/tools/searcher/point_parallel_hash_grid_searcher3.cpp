...
#include <generated/point_parallel_hash_grid_searcher3_generated.hpp>
#include <pch.hpp>
...
PointParallelHashGridSearcher3::PointParallelHashGridSearcher3(
  ...
  double gridSpacing)
  : PointParallelHashGridSearcher3(resolution.x,
                                   resolution.y,
                                   resolution.z,
                                   gridSpacing)
{
...
void
PointParallelHashGridSearcher3::build(
  const ConstArrayAccessor1<Vector3D>& points)
{
  ...
  LSTME_INFO << "Average number of points per non-empty bucket: "
             << static_cast<float>(sumNumberOfPointsPerBucket) /
                  static_cast<float>(numberOfNonEmptyBucket);
  LSTME_INFO << "Max number of points per bucket: "
             << maxNumberOfPointsPerBucket;
}

void
PointParallelHashGridSearcher3::forEachNearbyPoint(
...
bool
PointParallelHashGridSearcher3::hasNearbyPoint(const Vector3D& origin,
                                               double radius) const
{
  size_t nearbyKeys[8];
  getNearbyKeys(origin, nearbyKeys);
...
size_t
PointParallelHashGridSearcher3::getHashKeyFromPosition(
...
size_t
PointParallelHashGridSearcher3::getHashKeyFromBucketIndex(
  const Point3I& bucketIndex) const
{
  ...
  return static_cast<size_t>((wrappedIndex.z * _resolution.y + wrappedIndex.y) *
                               _resolution.x +
                             wrappedIndex.x);
}

void
PointParallelHashGridSearcher3::getNearbyKeys(const Vector3D& position,
...
PointParallelHashGridSearcher3::serialize(std::vector<uint8_t>* buffer) const
{
  ...
  for (const auto& pt : _points) {
    points.push_back(lstmeToFbs(pt));
  }
  ...
  // Copy the searcher
  auto fbsSearcher =
    fbs::CreatePointParallelHashGridSearcher3(builder,
                                              _gridSpacing,
                                              &fbsResolution,
                                              fbsPoints,
                                              fbsKeys,
