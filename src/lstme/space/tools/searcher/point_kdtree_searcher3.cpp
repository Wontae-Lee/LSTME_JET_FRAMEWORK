...
#include <generated/point_kdtree_searcher3_generated.hpp>
...
void
PointKdTreeSearcher3::forEachNearbyPoint(
...
bool
PointKdTreeSearcher3::hasNearbyPoint(const Vector3D& origin,
                                     double radius) const
{
  return _tree.hppasNearbyPoint(origin, radius);
}
...
PointKdTreeSearcher3::serialize(std::vector<uint8_t>* buffer) const
{
  ...
  for (const auto& iter : _tree) {
    points.push_back(lstmeToFbs(iter));
  }
...
PointKdTreeSearcher3::Builder::makeShared() const
{
  return std::shared_ptr<PointKdTreeSearcher3>(
    new PointKdTreeSearcher3, [](PointKdTreeSearcher3* obj) { delete obj; });
}
