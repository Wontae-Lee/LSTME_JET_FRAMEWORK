...
public:
  ...
  // Scalars only. See reflection.hpp, and the documentation.
...
// don't know what the element types are (used with reflection.hpp).
class VectorOfAny
{
public:
  uoffset_t size() const { return EndianScalar(length_); }
...
Vector<Offset<T>>*
VectorCast(Vector<Offset<U>>* ptr)
{
...
VectorCast(const Vector<Offset<U>>* ptr)
{
  static_assert(std::is_base_of<T, U>::value, "Unrelated types");
...
inline voffset_t
FieldIndexToOffset(voffset_t field_id)
{
...
public:
  ...
  /// in the buffer.
  uoffset_t CreateUninitializedVector(size_t len,
                                      size_t elemsize,
                                      uint8_t** buf)
  {
    NotNested();
  ...
  Offset<Vector<T>> CreateUninitializedVector(size_t len, T** buf)
  {
    return CreateUninitializedVector(
      len, sizeof(T), reinterpret_cast<uint8_t**>(buf));
  }
...
template<typename T>
const T*
GetSizePrefixedRoot(const void* buf)
{
...
template<typename T>
T*
GetMutableTemporaryPointer(FlatBufferBuilder& fbb, Offset<T> offset)
{
...
template<typename T>
const T*
GetTemporaryPointer(FlatBufferBuilder& fbb, Offset<T> offset)
{
...
/// vtable).
inline const char*
GetBufferIdentifier(const void* buf)
{
...
inline bool
BufferHasIdentifier(const void* buf, const char* identifier)
{
...
{
public:
  Verifier(const uint8_t* buf,
           size_t buf_len,
           uoffset_t _max_depth = 64,
  ...
  bool Verify(const Vector<T>* vec) const
  {
    const uint8_t* end;
    return !vec ||
           VerifyVector(reinterpret_cast<const uint8_t*>(vec), sizeof(T), &end);
  }
  ...
  bool VerifyVector(const uint8_t* vec,
                    size_t elem_size,
...
GetBufferStartFromRootPointer(const void* root)
{
...
template<typename T>
bool
IsFieldPresent(const T* table, voffset_t field)
{
...
inline int
LookupEnum(const char** names, const char* name)
{
...
volatile __attribute__((weak)) const char* flatbuffer_version_string =
  "FlatBuffers " FLATBUFFERS_STRING(FLATBUFFERS_VERSION_MAJOR) "." FLATBUFFERS_STRING(
