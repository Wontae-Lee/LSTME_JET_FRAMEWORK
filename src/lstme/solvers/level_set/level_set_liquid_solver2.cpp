...
LevelSetLiquidSolver2::onBeginAdvanceTimeStep(double timeIntervalInSeconds)
{
  ...
  LSTME_INFO << "Current volume: " << _lastKnownVolume;
}
...
LevelSetLiquidSolver2::onEndAdvanceTimeStep(double timeIntervalInSeconds)
{
  ...
  LSTME_INFO << "reinitializing level set field took "
  ...
  LSTME_INFO << "Current volume: " << currentVol << " "
             << "Volume diff: " << volDiff;
  ...
  if (_isGlobalCompensationEnabled) {
    ...
    LSTME_INFO << "Volume after global compensation: " << currentVol;
  }
}
...
LevelSetLiquidSolver2::computeAdvection(double timeIntervalInSeconds)
{
  ...
  LSTME_INFO << "velocity extrapolation took " << timer.durationInSeconds()
             << " seconds";
...
LevelSetLiquidSolver2::reinitialize(double currentCfl)
{
  if (_levelSetSolver != nullptr) {
    ...
    LSTME_INFO << "Max reinitialize distance: " << maxReinitDist;
...
LevelSetLiquidSolver2::extrapolateVelocityToAir(double currentCfl)
{
  ...
  LSTME_INFO << "Max velocity extrapolation distance: " << maxDist;
...
LevelSetLiquidSolver2::Builder::makeShared() const
{
  return std::shared_ptr<LevelSetLiquidSolver2>(
    new LevelSetLiquidSolver2(_resolution, getGridSpacing(), _gridOrigin),
