...
void
FmmLevelSetSolver3::reinitialize(const ScalarGrid3& inputSdf,
                                 double maxDistance,
                                 ScalarGrid3* outputSdf)
{
  LSTME_THROW_INVALID_ARG_IF(!inputSdf.hasSameShape(*outputSdf));
  ...
  markers.parallelForEachIndex(
    [&](size_t i, size_t j, size_t k) { output(i, j, k) = inputSdf(i, j, k); });
  ...
  markers.forEachIndex([&](size_t i, size_t j, size_t k) {
    if (isInsideSdf(output(i, j, k)) &&
      ...
      output(i, j, k) = solveQuadNearBoundary(markers,
                                              inputSdf.constDataAccessor(),
                                              gridSpacing,
                                              invGridSpacingSqr,
                                              -1.0,
                                              i,
  ...
  for (int sign = 0; sign < 2; ++sign) {
    ...
    while (!trial.empty()) {
      ...
      markers(i, j, k) = kKnown;
      output(i, j, k) =
        solveQuad(markers, output, gridSpacing, invGridSpacingSqr, i, j, k);
      ...
      if (i > 0) {
        if (markers(i - 1, j, k) == kUnknown) {
          ...
          output(i - 1, j, k) = solveQuad(
            markers, output, gridSpacing, invGridSpacingSqr, i - 1, j, k);
      ...
      if (j > 0) {
        if (markers(i, j - 1, k) == kUnknown) {
          ...
          output(i, j - 1, k) = solveQuad(
            markers, output, gridSpacing, invGridSpacingSqr, i, j - 1, k);
      ...
      if (k > 0) {
        if (markers(i, j, k - 1) == kUnknown) {
          ...
          output(i, j, k - 1) = solveQuad(
            markers, output, gridSpacing, invGridSpacingSqr, i, j, k - 1);
...
void
FmmLevelSetSolver3::extrapolate(const ScalarGrid3& input,
                                const ScalarField3& sdf,
                                double maxDistance,
                                ScalarGrid3* output)
{
  LSTME_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
  ...
  extrapolate(input.constDataAccessor(),
...
void
FmmLevelSetSolver3::extrapolate(const CollocatedVectorGrid3& input,
                                const ScalarField3& sdf,
                                double maxDistance,
...
void
FmmLevelSetSolver3::extrapolate(const FaceCenteredGrid3& input,
                                const ScalarField3& sdf,
                                double maxDistance,
                                ...
                                ArrayAccessor3<double> output)
{
  ...
  // Enqueue initial candidates
  std::priority_queue<Point3UI, std::vector<Point3UI>, decltype(compare)> trial(
    compare);
  markers.forEachIndex([&](size_t i, size_t j, size_t k) {
    if (markers(i, j, k) == kKnown) {
      return;
