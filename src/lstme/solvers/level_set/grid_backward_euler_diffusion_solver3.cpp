...
void
GridBackwardEulerDiffusionSolver3::setLinearSystemSolver(
...
void
GridBackwardEulerDiffusionSolver3::buildMarkers(
  const Size3& size,
...
void
GridBackwardEulerDiffusionSolver3::buildMatrix(const Size3& size,
                                               const Vector3D& c)
{
  ...
  _system.A.parallelForEachIndex([&](size_t i, size_t j, size_t k) {
    ...
    if (_markers(i, j, k) == kFluid) {
      ...
      if (i > 0 && ((isDirichlet && _markers(i - 1, j, k) != kAir) ||
                    _markers(i - 1, j, k) == kFluid)) {
        row.center += c.x;
      }
      ...
      if (j > 0 && ((isDirichlet && _markers(i, j - 1, k) != kAir) ||
                    _markers(i, j - 1, k) == kFluid)) {
        row.center += c.y;
      }
      ...
      if (k > 0 && ((isDirichlet && _markers(i, j, k - 1) != kAir) ||
                    _markers(i, j, k - 1) == kFluid)) {
        row.center += c.z;
      }
    }
...
void
GridBackwardEulerDiffusionSolver3::buildVectors(
