...
void
FmmLevelSetSolver2::reinitialize(const ScalarGrid2& inputSdf,
                                 double maxDistance,
                                 ScalarGrid2* outputSdf)
{
  ...
  markers.parallelForEachIndex(
    [&](size_t i, size_t j) { output(i, j) = inputSdf(i, j); });
  ...
  for (int sign = 0; sign < 2; ++sign) {
    ...
    std::priority_queue<Point2UI, std::vector<Point2UI>, decltype(compare)>
      trial(compare);
    markers.forEachIndex([&](size_t i, size_t j) {
    ...
    while (!trial.empty()) {
      ...
      markers(i, j) = kKnown;
      output(i, j) =
        solveQuad(markers, output, gridSpacing, invGridSpacingSqr, i, j);
      ...
      if (i > 0) {
        if (markers(i - 1, j) == kUnknown) {
          ...
          output(i - 1, j) = solveQuad(
            markers, output, gridSpacing, invGridSpacingSqr, i - 1, j);
      ...
      if (j > 0) {
        if (markers(i, j - 1) == kUnknown) {
          ...
          output(i, j - 1) = solveQuad(
            markers, output, gridSpacing, invGridSpacingSqr, i, j - 1);
    ...
    // Flip the sign
    markers.parallelForEachIndex(
      [&](size_t i, size_t j) { output(i, j) = -output(i, j); });
  }
}

void
FmmLevelSetSolver2::extrapolate(const ScalarGrid2& input,
...
void
FmmLevelSetSolver2::extrapolate(const CollocatedVectorGrid2& input,
                                const ScalarField2& sdf,
                                double maxDistance,
...
void
FmmLevelSetSolver2::extrapolate(const FaceCenteredGrid2& input,
                                const ScalarField2& sdf,
                                double maxDistance,
                                FaceCenteredGrid2* output)
{
  LSTME_THROW_INVALID_ARG_IF(!input.hppasSameShape(*output));
                                ...
                                ArrayAccessor2<double> output)
{
  ...
  // Enqueue initial candidates
  std::priority_queue<Point2UI, std::vector<Point2UI>, decltype(compare)> trial(
    compare);
  markers.forEachIndex([&](size_t i, size_t j) {
    if (markers(i, j) == kKnown) {
      return;
