...
FdmGaussSeidelSolver2::solveCompressed(FdmCompressedLinearSystem2* system)
{
  ...
  for (unsigned int iter = 0; iter < _maxNumberOfIterations; ++iter) {
    ...
    if (iter != 0 && iter % _residualCheckInterval == 0) {
      FdmCompressedBlas2::residual(
        system->A, system->x, system->b, &_residualComp);
...
void
FdmGaussSeidelSolver2::relax(const FdmMatrix2& A,
                             const FdmVector2& b,
                             double sorFactor,
                             FdmVector2* x_)
{
  ...
  A.forEachIndex([&](size_t i, size_t j) {
    ...
    x(i, j) =
      (1.0 - sorFactor) * x(i, j) + sorFactor * (b(i, j) - r) / A(i, j).center;
  });
}

void
FdmGaussSeidelSolver2::relax(const MatrixCsrD& A,
                             const VectorND& b,
                             double sorFactor,
                             VectorND* x_)
{
...
void
FdmGaussSeidelSolver2::relaxRedBlack(const FdmMatrix2& A,
                                     const FdmVector2& b,
                                     double sorFactor,
                                     FdmVector2* x_)
{
