...
FdmIccgSolver3::Preconditioner::solve(const FdmVector3& b, FdmVector3* x)
{
  ...
  b.forEachIndex([&](size_t i, size_t j, size_t k) {
    y(i, j, k) =
      (b(i, j, k) - ((i > 0) ? A(i - 1, j, k).right * y(i - 1, j, k) : 0.0) -
...
FdmIccgSolver3::solve(FdmLinearSystem3* system)
{
  ...
  LSTME_ASSERT(matrix.size() == rhs.size());
  LSTME_ASSERT(matrix.size() == solution.size());
  ...
  pcg<FdmBlas3, Preconditioner>(matrix,
                                rhs,
  ...
  LSTME_INFO << "Residual norm after solving ICCG: " << _lastResidualNorm
