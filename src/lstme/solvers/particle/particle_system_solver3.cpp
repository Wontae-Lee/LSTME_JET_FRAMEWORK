...
double
ParticleSystemSolver3::dragCoefficient() const
{
  return _dragCoefficient;
}

void
ParticleSystemSolver3::setDragCoefficient(double newDragCoefficient)
{
  _dragCoefficient = std::max(newDragCoefficient, 0.0);
}

double
ParticleSystemSolver3::restitutionCoefficient() const
{
...
const Vector3D&
ParticleSystemSolver3::gravity() const
{
  return _gravity;
}

void
ParticleSystemSolver3::setGravity(const Vector3D& newGravity)
{
  _gravity = newGravity;
}
...
const Collider3Ptr&
ParticleSystemSolver3::collider() const
{
  return _collider;
}

void
ParticleSystemSolver3::setCollider(const Collider3Ptr& newCollider)
{
  _collider = newCollider;
}

const ParticleEmitter3Ptr&
ParticleSystemSolver3::emitter() const
{
  return _emitter;
}

void
ParticleSystemSolver3::setEmitter(const ParticleEmitter3Ptr& newEmitter)
{
  _emitter = newEmitter;
  newEmitter->setTarget(_particleSystemData);
}

const VectorField3Ptr&
ParticleSystemSolver3::wind() const
{
  return _wind;
}

void
ParticleSystemSolver3::setWind(const VectorField3Ptr& newWind)
{
  _wind = newWind;
}

void
ParticleSystemSolver3::onInitialize()
{
...
void
ParticleSystemSolver3::onAdvanceTimeStep(double timeStepInSeconds)
{
  beginAdvanceTimeStep(timeStepInSeconds);

  Timer timer;
  accumulateForces(timeStepInSeconds);
  LSTME_INFO << "Accumulating forces took " << timer.durationInSeconds()
             << " seconds";

  timer.reset();
  timeIntegration(timeStepInSeconds);
  LSTME_INFO << "Time integration took " << timer.durationInSeconds()
             << " seconds";

  timer.reset();
  resolveCollision();
  LSTME_INFO << "Resolving collision took " << timer.durationInSeconds()
             << " seconds";
...
void
ParticleSystemSolver3::accumulateForces(double timeStepInSeconds)
{
...
void
ParticleSystemSolver3::beginAdvanceTimeStep(double timeStepInSeconds)
{
  // Clear forces
...
void
ParticleSystemSolver3::endAdvanceTimeStep(double timeStepInSeconds)
{
  // Update data
  size_t n = _particleSystemData->numberOfParticles();
...
void
ParticleSystemSolver3::onBeginAdvanceTimeStep(double timeStepInSeconds)
{
  UNUSED_VARIABLE(timeStepInSeconds);
}

void
ParticleSystemSolver3::onEndAdvanceTimeStep(double timeStepInSeconds)
{
  UNUSED_VARIABLE(timeStepInSeconds);
}

void
ParticleSystemSolver3::resolveCollision()
{
  resolveCollision(_newPositions.accessor(), _newVelocities.accessor());
}

void
ParticleSystemSolver3::resolveCollision(ArrayAccessor1<Vector3D> newPositions,
  ...
  if (_collider != nullptr) {
    ...
    parallelFor(kZeroSize, numberOfParticles, [&](size_t i) {
      _collider->resolveCollision(
        radius, _restitutionCoefficient, &newPositions[i], &newVelocities[i]);
    });
  }
}

void
ParticleSystemSolver3::setParticleSystemData(
...
void
ParticleSystemSolver3::accumulateExternalForces()
{
  size_t n = _particleSystemData->numberOfParticles();
  auto forces = _particleSystemData->forces();
  auto velocities = _particleSystemData->velocities();
...
void
ParticleSystemSolver3::timeIntegration(double timeStepInSeconds)
{
  size_t n = _particleSystemData->numberOfParticles();
  auto forces = _particleSystemData->forces();
  auto velocities = _particleSystemData->velocities();
...
void
ParticleSystemSolver3::updateCollider(double timeStepInSeconds)
{
  if (_collider != nullptr) {
...
void
ParticleSystemSolver3::updateEmitter(double timeStepInSeconds)
{
  if (_emitter != nullptr) {
...
ParticleSystemSolver3::Builder::build() const
{
  return ParticleSystemSolver3(_radius, _mass);
}

ParticleSystemSolver3Ptr
ParticleSystemSolver3::Builder::makeShared() const
{
