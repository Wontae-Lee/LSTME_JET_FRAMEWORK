...
double
SphSystemData3::interpolate(const Vector3D& origin,
...
Vector3D
SphSystemData3::gradientAt(size_t i,
                           const ConstArrayAccessor1<double>& values) const
{
  Vector3D sum;
  auto p = positions();
  ...
  for (size_t j : neighbors) {
    ...
    if (dist > 0.0) {
      ...
      sum += d[i] * m * (values[i] / square(d[i]) + values[j] / square(d[j])) *
...
double
SphSystemData3::laplacianAt(size_t i,
                            const ConstArrayAccessor1<double>& values) const
{
  double sum = 0.0;
  auto p = positions();
...
SphSystemData3::computeMass()
{
  ...
  LSTME_ASSERT(maxNumberDensity > 0);
...
SphSystemData3::serialize(std::vector<uint8_t>* buffer) const
{
  ...
  auto fbsSphSystemData =
    fbs::CreateSphSystemData3(builder,
