...
template<typename T>
T&
ParticleSystemSolverBuilderBase3<T>::withRadius(double radius)
{
  _radius = radius;
...
template<typename T>
T&
ParticleSystemSolverBuilderBase3<T>::withMass(double mass)
{
  _mass = mass;
  return static_cast<T&>(*this);
}
...
#endif // INCLUDE_LSTME_PARTICLE_SYSTEM_SOLVER3_H_
