...
void ParticleSystemSolver2::onInitialize() {
    ...
    Timer timer;
    updateCollider(0.0);
    LSTME_INFO << "Update collider took "
             << timer.durationInSeconds() << " seconds";

    timer.reset();
    updateEmitter(0.0);
    LSTME_INFO << "Update emitter took "
             << timer.durationInSeconds() << " seconds";
}
...
void ParticleSystemSolver2::onAdvanceTimeStep(double timeStepInSeconds) {
    ...
    LSTME_INFO << "Accumulating forces took "
             << timer.durationInSeconds() << " seconds";

    timer.reset();
    timeIntegration(timeStepInSeconds);
    LSTME_INFO << "Time integration took "
             << timer.durationInSeconds() << " seconds";

    timer.reset();
    resolveCollision();
    LSTME_INFO << "Resolving collision took "
             << timer.durationInSeconds() << " seconds";
