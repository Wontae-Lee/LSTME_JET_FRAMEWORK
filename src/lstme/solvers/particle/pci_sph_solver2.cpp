...
PciSphSolver2::PciSphSolver2(double targetDensity,
...
double
PciSphSolver2::maxDensityErrorRatio() const
{
  return _maxDensityErrorRatio;
}

void
PciSphSolver2::setMaxDensityErrorRatio(double ratio)
{
  _maxDensityErrorRatio = std::max(ratio, 0.0);
}

unsigned int
PciSphSolver2::maxNumberOfIterations() const
{
  return _maxNumberOfIterations;
}

void
PciSphSolver2::setMaxNumberOfIterations(unsigned int n)
{
  _maxNumberOfIterations = n;
}

void
PciSphSolver2::accumulatePressureForce(double timeIntervalInSeconds)
{
  ...
  for (unsigned int k = 0; k < _maxNumberOfIterations; ++k) {
    ...
    parallelFor(kZeroSize, numberOfParticles, [&](size_t i) {
      _tempVelocities[i] =
        v[i] + timeIntervalInSeconds / mass * (f[i] + _pressureForces[i]);
  ...
  LSTME_INFO << "Number of PCI iterations: " << maxNumIter;
  LSTME_INFO << "Max density error after PCI iteration: " << maxDensityError;
  if (std::fabs(densityErrorRatio) > _maxDensityErrorRatio) {
    LSTME_WARN << "Max density error ratio is greater than the threshold!";
    LSTME_WARN << "Ratio: " << densityErrorRatio
               << " Threshold: " << _maxDensityErrorRatio;
  }

  // Accumulate pressure force
  parallelFor(kZeroSize, numberOfParticles, [this, &f](size_t i) {
    f[i] += _pressureForces[i];
  });
}

void
PciSphSolver2::onBeginAdvanceTimeStep(double timeStepInSeconds)
{
...
double
PciSphSolver2::computeDelta(double timeStepInSeconds)
{
  auto particles = sphSystemData();
...
double
PciSphSolver2::computeBeta(double timeStepInSeconds)
{
  auto particles = sphSystemData();
