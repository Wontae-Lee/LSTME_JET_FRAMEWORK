...
SphSolver2::SphSolver2(double targetDensity,
...
SphSolver2::onBeginAdvanceTimeStep(double timeStepInSeconds)
{
  ...
  LSTME_INFO << "Building neighbor lists and updating densities took "
...
SphSolver2::onEndAdvanceTimeStep(double timeStepInSeconds)
{
  ...
  LSTME_INFO << "Max density: " << maxDensity << " "
...
SphSolver2::computePressure()
{
  ...
  parallelFor(kZeroSize, numberOfParticles, [&](size_t i) {
    p[i] = computePressureFromEos(
      d[i], targetDensity, eosScale, eosExponent(), negativePressureScale());
  });
}
