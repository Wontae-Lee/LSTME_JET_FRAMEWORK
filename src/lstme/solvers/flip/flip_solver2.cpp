...
FlipSolver2::FlipSolver2(const Size2& resolution,
...
FlipSolver2::transferFromParticlesToGrids()
{
  ...
  vel->parallelForEachUIndex(
    [&](size_t i, size_t j) { _uDelta(i, j) = static_cast<float>(u(i, j)); });
  vel->parallelForEachVIndex(
    [&](size_t i, size_t j) { _vDelta(i, j) = static_cast<float>(v(i, j)); });
}
...
FlipSolver2::Builder::makeShared() const
{
  return std::shared_ptr<FlipSolver2>(
    new FlipSolver2(_resolution, getGridSpacing(), _gridOrigin),
