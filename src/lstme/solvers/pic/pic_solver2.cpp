...
PicSolver2::PicSolver2(const Size2& resolution,
...
PicSolver2::onInitialize()
{
  ...
  LSTME_INFO << "Update particle emitter took " << timer.durationInSeconds()
             << " seconds";
}
...
PicSolver2::onBeginAdvanceTimeStep(double timeIntervalInSeconds)
{
  ...
  LSTME_INFO << "Number of PIC-type particles: "
             << _particles->numberOfParticles();

  timer.reset();
  transferFromParticlesToGrids();
  LSTME_INFO << "transferFromParticlesToGrids took "
             << timer.durationInSeconds() << " seconds";

  timer.reset();
  buildSignedDistanceField();
  LSTME_INFO << "buildSignedDistanceField took " << timer.durationInSeconds()
             << " seconds";

  timer.reset();
  extrapolateVelocityToAir();
  LSTME_INFO << "extrapolateVelocityToAir took " << timer.durationInSeconds()
             << " seconds";
...
PicSolver2::computeAdvection(double timeIntervalInSeconds)
{
  ...
  LSTME_INFO << "transferFromGridsToParticles took "
             << timer.durationInSeconds() << " seconds";

  timer.reset();
  moveParticles(timeIntervalInSeconds);
  LSTME_INFO << "moveParticles took " << timer.durationInSeconds()
             << " seconds";
}
...
PicSolver2::Builder::makeShared() const
{
  return std::shared_ptr<PicSolver2>(
    new PicSolver2(_resolution, getGridSpacing(), _gridOrigin),
