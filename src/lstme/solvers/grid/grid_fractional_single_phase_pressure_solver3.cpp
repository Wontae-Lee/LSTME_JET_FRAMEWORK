...
void restrict(const Array3<float>& finer, Array3<float>* coarser)
{
  ...
  //           to
  // -|---------|---------|-
  static const std::array<float, 4> staggeredKernel = {
    { 0.f, 1.f, 0.f, 0.f }
  };
...
void
GridFractionalSinglePhasePressureSolver3::solve(
  const FaceCenteredGrid3& input,
...
void
GridFractionalSinglePhasePressureSolver3::buildWeights(
  const FaceCenteredGrid3& input,
  const ScalarField3& boundarySdf,
  const VectorField3& boundaryVelocity,
  const ScalarField3& fluidSdf)
...
void
GridFractionalSinglePhasePressureSolver3::buildSystem(
  const FaceCenteredGrid3& input,
  bool useCompressed)
{
  ...
  if (_mgSystemSolver == nullptr) {
    if (useCompressed) {
      buildSingleSystem(&_compSystem.A,
                        &_compSystem.x,
                        &_compSystem.b,
                        _fluidSdf[0],
                        _uWeights[0],
  ...
  for (size_t l = 1; l < numLevels; ++l) {
    ...
    buildSingleSystem(&_mgSystem.A.levels[l],
                      &_mgSystem.b.levels[l],
                      _fluidSdf[l],
                      _uWeights[l],
                      _vWeights[l],
                      _wWeights[l],
...
void
GridFractionalSinglePhasePressureSolver3::applyPressureGradient(
  const FaceCenteredGrid3& input,
  FaceCenteredGrid3* output)
{
  Size3 size = input.resolution();
  ...
  x.parallelForEachIndex([&](size_t i, size_t j, size_t k) {
    ...
    if (i + 1 < size.x && _uWeights[0](i + 1, j, k) > 0.0 &&
        (isInsideSdf(centerPhi) || isInsideSdf(_fluidSdf[0](i + 1, j, k)))) {
    ...
    if (j + 1 < size.y && _vWeights[0](i, j + 1, k) > 0.0 &&
        (isInsideSdf(centerPhi) || isInsideSdf(_fluidSdf[0](i, j + 1, k)))) {
    ...
    if (k + 1 < size.z && _wWeights[0](i, j, k + 1) > 0.0 &&
        (isInsideSdf(centerPhi) || isInsideSdf(_fluidSdf[0](i, j, k + 1)))) {
