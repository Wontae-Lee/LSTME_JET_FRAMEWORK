...
void
GridFluidSolver3::resizeGrid(const Size3& newSize,
...
GridFluidSolver3::onInitialize()
{
  ...
  Timer timer;
  updateCollider(0.0);
  LSTME_INFO << "Update collider took " << timer.durationInSeconds()
             << " seconds";

  timer.reset();
  updateEmitter(0.0);
  LSTME_INFO << "Update emitter took " << timer.durationInSeconds()
             << " seconds";
}
...
GridFluidSolver3::onAdvanceTimeStep(double timeIntervalInSeconds)
{
  ...
  if (_grids->resolution().x == 0 || _grids->resolution().y == 0 ||
    ...
    LSTME_WARN << "Empty grid. Skipping the simulation.";
    return;
  }
  ...
  LSTME_INFO << "Computing external force took " << timer.durationInSeconds()
             << " seconds";

  timer.reset();
  computeViscosity(timeIntervalInSeconds);
  LSTME_INFO << "Computing viscosity force took " << timer.durationInSeconds()
             << " seconds";

  timer.reset();
  computePressure(timeIntervalInSeconds);
  LSTME_INFO << "Computing pressure force took " << timer.durationInSeconds()
             << " seconds";

  timer.reset();
  computeAdvection(timeIntervalInSeconds);
  LSTME_INFO << "Computing advection force took " << timer.durationInSeconds()
             << " seconds";
...
GridFluidSolver3::computePressure(double timeIntervalInSeconds)
{
  if (_pressureSolver != nullptr) {
    ...
    _pressureSolver->solve(*vel0,
                           timeIntervalInSeconds,
                           vel.get(),
                           *colliderSdf(),
                           *colliderVelocityField(),
                           *fluidSdf(),
                           _useCompressedLinearSys);
    applyBoundaryCondition();
  }
}
  ...
  if (_advectionSolver != nullptr) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      _advectionSolver->advect(
        *grid0, *vel, timeIntervalInSeconds, grid.get(), *colliderSdf());
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (collocated != nullptr) {
        _advectionSolver->advect(*collocated0,
                                 *vel,
                                 timeIntervalInSeconds,
                                 collocated.get(),
                                 *colliderSdf());
      ...
      if (faceCentered != nullptr && faceCentered0 != nullptr) {
        _advectionSolver->advect(*faceCentered0,
                                 *vel,
                                 timeIntervalInSeconds,
                                 faceCentered.get(),
                                 *colliderSdf());
    ...
    auto vel0 = std::dynamic_pointer_cast<FaceCenteredGrid3>(vel->clone());
    _advectionSolver->advect(
      *vel0, *vel0, timeIntervalInSeconds, vel.get(), *colliderSdf());
    applyBoundaryCondition();
  }
}
...
GridFluidSolver3::beginAdvanceTimeStep(double timeIntervalInSeconds)
{
  ...
  if (_boundaryConditionSolver != nullptr) {
    _boundaryConditionSolver->updateCollider(
      _collider, _grids->resolution(), _grids->gridSpacing(), _grids->origin());
  }
