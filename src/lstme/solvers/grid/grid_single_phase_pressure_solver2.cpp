...
void
GridSinglePhasePressureSolver2::buildMarkers(
  const Size2& size,
...
void
GridSinglePhasePressureSolver2::buildSystem(const FaceCenteredGrid2& input,
                                            bool useCompressed)
{
  Size2 size = input.resolution();
  size_t numLevels = 1;
  ...
  if (_mgSystemSolver == nullptr) {
    if (useCompressed) {
      buildSingleSystem(
        &_compSystem.A, &_compSystem.x, &_compSystem.b, _markers[0], *finer);
    } else {
  ...
  for (size_t l = 1; l < numLevels; ++l) {
    ...
    buildSingleSystem(
      &_mgSystem.A.levels[l], &_mgSystem.b.levels[l], _markers[l], coarser);
...
void
GridSinglePhasePressureSolver2::applyPressureGradient(
  const FaceCenteredGrid2& input,
  FaceCenteredGrid2* output)
{
  Size2 size = input.resolution();
