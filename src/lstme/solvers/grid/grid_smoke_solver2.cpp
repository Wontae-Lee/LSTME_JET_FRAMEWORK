...
GridSmokeSolver2::computeDiffusion(double timeIntervalInSeconds)
{
  if (diffusionSolver() != nullptr) {
    if (_smokeDiffusionCoefficient > kEpsilonD) {
      ...
      auto den0 =
        std::dynamic_pointer_cast<CellCenteredScalarGrid2>(den->clone());

      diffusionSolver()->solve(*den0,
    ...
    if (_temperatureDiffusionCoefficient > kEpsilonD) {
      ...
      auto temp0 =
        std::dynamic_pointer_cast<CellCenteredScalarGrid2>(temp->clone());

      diffusionSolver()->solve(*temp0,
  ...
  if (std::abs(_buoyancySmokeDensityFactor) > kEpsilonD ||
    ...
    if (std::abs(up.x) > kEpsilonD) {
      vel->parallelForEachUIndex([&](size_t i, size_t j) {
        ...
        double fBuoy = _buoyancySmokeDensityFactor * den->sample(pt) +
