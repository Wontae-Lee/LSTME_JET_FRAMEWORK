...
template<typename T>
inline T
laplacian(const ConstArrayAccessor2<T>& data,
                                        ...
                                        const ScalarField2& fluidSdf)
{
  ...
  source.parallelForEachDataPointIndex([&](size_t i, size_t j) {
    if (_markers(i, j) == kFluid) {
      (*dest)(i, j) = source(i, j) + diffusionCoefficient *
                                        ...
                                        const ScalarField2& fluidSdf)
{
  ...
  source.parallelForEachDataPointIndex([&](size_t i, size_t j) {
    if (_markers(i, j) == kFluid) {
      (*dest)(i, j) = src(i, j) + diffusionCoefficient * timeIntervalInSeconds *
                                    laplacian(src, _markers, h, i, j);
    } else {
      (*dest)(i, j) = src(i, j);
    }
                                        ...
                                        const ScalarField2& fluidSdf)
{
  ...
  source.parallelForEachUIndex([&](size_t i, size_t j) {
    if (_markers(i, j) == kFluid) {
      u(i, j) = uSrc(i, j) + diffusionCoefficient * timeIntervalInSeconds *
  ...
  source.parallelForEachVIndex([&](size_t i, size_t j) {
    if (_markers(i, j) == kFluid) {
      v(i, j) = vSrc(i, j) + diffusionCoefficient * timeIntervalInSeconds *
...
void
GridForwardEulerDiffusionSolver2::buildMarkers(
  const Size2& size,
