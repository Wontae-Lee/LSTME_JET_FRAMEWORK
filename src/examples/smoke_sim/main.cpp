...
void
saveVolumeAsVol(const ScalarGrid3Ptr& density, const std::string& rootDir, int frameCnt)
{
  char basename[256];
  ...
  if (file) {
    ...
    data.parallelForEachIndex([&](size_t i, size_t j, size_t k) {
      ...
      if (i > data.size().x - 1 - kEdgeBlur) {
        d *= smoothStep(0.f, kEdgeBlurF, static_cast<float>((data.size().x - 1) - i));
      }
      if (j < kEdgeBlur) {
        d *= smoothStep(0.f, kEdgeBlurF, static_cast<float>(j));
      }
      if (j > data.size().y - 1 - kEdgeBlur) {
        d *= smoothStep(0.f, kEdgeBlurF, static_cast<float>((data.size().y - 1) - j));
      }
      if (k < kEdgeBlur) {
        d *= smoothStep(0.f, kEdgeBlurF, static_cast<float>(k));
      }
      if (k > data.size().z - 1 - kEdgeBlur) {
        d *= smoothStep(0.f, kEdgeBlurF, static_cast<float>((data.size().z - 1) - k));
      }

      data(i, j, k) = d;
    });
    file.write(reinterpret_cast<const char*>(data.data()), sizeof(float) * data.size().x * data.size().y * data.size().z);
...
void
saveVolumeAsTga(const ScalarGrid3Ptr& density, const std::string& rootDir, int frameCnt)
{
  char basename[256];
...
void
printInfo(const GridSmokeSolver3Ptr& solver)
{
...
void
runSimulation(const std::string& rootDir, const GridSmokeSolver3Ptr& solver, int numberOfFrames, const std::string& format, double fps)
{
...
{
  // Build solver
  auto solver = GridSmokeSolver3::builder().withResolution({ resolutionX, 2 * resolutionX, resolutionX }).withDomainSizeX(1.0).makeShared();
  ...
  // Build emitter
  auto box = Box3::builder().withLowerCorner({ 0.45, -1, 0.45 }).withUpperCorner({ 0.55, 0.05, 0.55 }).makeShared();

  auto emitter = VolumeGridEmitter3::builder().withSourceRegion(box).withIsOneShot(false).makeShared();
  ...
  // Build collider
  auto sphere = Sphere3::builder().withCenter({ 0.5, 0.3, 0.5 }).withRadius(0.075 * domain.width()).makeShared();
...
runExample3(const std::string& rootDir, size_t resolutionX, int numberOfFrames, const std::string& format, double fps)
{
  ...
  auto dragon = ImplicitTriangleMesh3::builder().withTriangleMesh(dragonMesh).withResolutionX(resolutionX).makeShared();

  auto emitter = VolumeGridEmitter3::builder().withSourceRegion(dragon).withIsOneShot(false).makeShared();
...
{
  // Build solver
  auto solver = GridSmokeSolver3::builder().withResolution({ resolutionX, 6 * resolutionX / 5, resolutionX / 2 }).withDomainSizeX(1.0).makeShared();

  solver->setBuoyancyTemperatureFactor(2.0);

  // Build emitter
  auto box = Box3::builder().withLowerCorner({ 0.05, 0.1, 0.225 }).withUpperCorner({ 0.1, 0.15, 0.275 }).makeShared();
  ...
  emitter->addStepFunctionTarget(solver->smokeDensity(), 0, 1);
  emitter->addStepFunctionTarget(solver->temperature(), 0, 1);
  emitter->addTarget(solver->velocity(), [](double sdf, const Vector3D&, const Vector3D& oldVal) {
    if (sdf < 0.05) {
...
main(int argc, char* argv[])
{
  ...
  auto parser = clara::Help(showHelp) | clara::Opt(resolutionX, "resolutionX")["-r"]["--resx"]("grid resolution in x-axis (default is 50)") |
                ...
                clara::Opt(exampleNum, "exampleNum")["-e"]["--example"]("example number (between 1 and 5, default is 1)") |
                ...
                clara::Opt(format, "format")["-m"]["--format"]("particle output format (tag or vol. default is tag)");
