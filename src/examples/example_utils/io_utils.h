...
inline bool readPositions(const std::string& filename, jet::Array1<jet::Vector3D>& positions) {
    if (pystring::endswith(filename, ".xyz")) {
        ...
        if (posFile) {
            std::vector<uint8_t> buffer((std::istreambuf_iterator<char>(posFile)), (std::istreambuf_iterator<char>()));
