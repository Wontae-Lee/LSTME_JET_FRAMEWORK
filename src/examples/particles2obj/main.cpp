...
void
printInfo(const Size3& resolution, const BoundingBox3D& domain, const Vector3D& gridSpacing, size_t numberOfParticles, const std::string& method)
{
...
void
triangulateAndSave(const ScalarGrid3& sdf, const std::string& objFilename)
{
  TriangleMesh3 mesh;
...
void
particlesToObj(const Array1<Vector3D>& positions,
               ...
               const std::string& objFilename)
{
  ...
  if (method == kSpherical) {
    ...
    converter = std::make_shared<ZhuBridsonPointsToImplicit3>(kernelRadius, sZhuBridsonCutOffThreshold, false);
  } else {
    converter =
      std::make_shared<AnisotropicPointsToImplicit3>(kernelRadius, sAnisoCutOffDensity, sAnisoPositionSmoothingFactor, sAnisoMinNumNeighbors, false);
  }
...
main(int argc, char* argv[])
{
  ...
  // Parsing
  auto parser = clara::Help(showHelp) | clara::Opt(inputFilename, "inputFilename")["-i"]["--input"]("input pos/xyz file name") |
                clara::Opt(outputFilename, "outputFilename")["-o"]["--output"]("output obj file name") |
                clara::Opt(strResolution, "resolution")["-r"]["--resolution"]("grid resolution in CSV format (default is 100,100,100)") |
                clara::Opt(strGridSpacing, "gridSpacing")["-g"]["--grid_spacing"]("grid spacing in CSV format (default is 0.01,0.01,0.01)") |
                clara::Opt(strOrigin, "origin")["-n"]["--origin"]("domain origin in CSV format (default is 0,0,0)") |
                clara::Opt(method, "method")["-m"]["--method"]("spherical, sph, zhu_bridson, and anisotropic "
                                                               "followed by optional method-dependent parameters (default is "
                                                               "anisotropic)") |
                clara::Opt(kernelRadius, "kernelRadius")["-k"]["--kernel"]("interpolation kernel radius (default is 0.2)");
  ...
  if (!strMethod.empty()) {
    ...
    if (method == kSpherical) {
      ...
      if (tokens.size() > 3) {
        sAnisoMinNumNeighbors = static_cast<size_t>(atoi(tokens[3].c_str()));
      }
    } else {
