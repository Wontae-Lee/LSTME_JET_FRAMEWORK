...
void
saveTriangleMeshData(const TriangleMesh3& data, const std::string& filename)
{
...
main(int argc, char* argv[])
{
  ...
  // Parsing
  auto parser = clara::Help(showHelp) | clara::Opt(inputFilename, "inputFilename")["-i"]["--input"]("input obj file name") |
                clara::Opt(outputFilename, "outputFilename")["-o"]["--output"]("output sdf file name") |
                clara::Opt(resolutionX, "resolutionX")["-r"]["--resx"]("grid resolution in x-axis (default is 100)") |
                clara::Opt(marginScale, "marginScale")["-m"]["--margin"]("margin scale around the sdf (default is 0.2)");
  ...
  VertexCenteredScalarGrid3 grid(resolutionX, resolutionY, resolutionZ, dx, dx, dx, box.lowerCorner.x, box.lowerCorner.y, box.lowerCorner.z);
  ...
  marchingCubes(grid.constDataAccessor(), grid.gridSpacing(), grid.origin(), &triMesh2, 0, kDirectionAll);
