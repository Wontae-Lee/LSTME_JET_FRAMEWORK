...
void
printInfo(const PicSolver3Ptr& solver)
{
  ...
  printf("Domain: [%f, %f, %f] x [%f, %f, %f]\n",
         ...
         domain.lowerCorner.y,
         domain.lowerCorner.z,
         domain.upperCorner.x,
         domain.upperCorner.y,
         domain.upperCorner.z);
  printf("Grid spacing: [%f, %f, %f]\n", gridSpacing.x, gridSpacing.y, gridSpacing.z);
}

void
runSimulation(const std::string& rootDir, const PicSolver3Ptr& solver, int numberOfFrames, const std::string& format, double fps)
{
...
void
runExample1(const std::string& rootDir, size_t resolutionX, int numberOfFrames, const std::string& format, double fps)
{
  // Build solver
  auto solver = FlipSolver3::builder().withResolution({ resolutionX, 2 * resolutionX, resolutionX }).withDomainSizeX(1.0).makeShared();
  ...
  auto emitter1 = VolumeParticleEmitter3::builder().withSurface(plane).withSpacing(0.5 * dx).withMaxRegion(domain).withIsOneShot(true).makeShared();
  emitter1->setPointGenerator(std::make_shared<GridPointGenerator3>());

  auto emitter2 = VolumeParticleEmitter3::builder().withSurface(sphere).withSpacing(0.5 * dx).withMaxRegion(domain).withIsOneShot(true).makeShared();
  emitter2->setPointGenerator(std::make_shared<GridPointGenerator3>());

  auto emitterSet = ParticleEmitterSet3::builder().withEmitters({ emitter1, emitter2 }).makeShared();
...
void
runExample2(const std::string& rootDir, size_t resolutionX, int numberOfFrames, const std::string& format, double fps)
{
  // Build solver
  auto solver = PicSolver3::builder().withResolution({ resolutionX, 2 * resolutionX, resolutionX }).withDomainSizeX(1.0).makeShared();
...
void
runExample3(const std::string& rootDir, size_t resolutionX, int numberOfFrames, const std::string& format, double fps)
{
  // Build solver
  Size3 resolution{ 3 * resolutionX, 2 * resolutionX, (3 * resolutionX) / 2 };
  auto solver = FlipSolver3::builder().withResolution(resolution).withDomainSizeX(3.0).makeShared();
  ...
  auto emitter = VolumeParticleEmitter3::builder().withSurface(boxSet).withMaxRegion(domain).withSpacing(0.5 * dx).makeShared();
...
void
runExample4(const std::string& rootDir, size_t resolutionX, int numberOfFrames, const std::string& format, double fps)
{
  // Build solver
  Size3 resolution{ 3 * resolutionX, 2 * resolutionX, (3 * resolutionX) / 2 };
  auto solver = PicSolver3::builder().withResolution(resolution).withDomainSizeX(3.0).makeShared();
  solver->setUseCompressedLinearSystem(true);
...
void
runExample5(const std::string& rootDir, size_t resolutionX, int numberOfFrames, const std::string& format, double fps)
{
  // Build solver
  Size3 resolution{ 3 * resolutionX, 2 * resolutionX, (3 * resolutionX) / 2 };
  auto solver = ApicSolver3::builder().withResolution(resolution).withDomainSizeX(3.0).makeShared();
  solver->setUseCompressedLinearSystem(true);
...
void
runExample6(const std::string& rootDir, size_t resolutionX, int numberOfFrames, const std::string& format, double fps)
{
  // Build solver
  auto solver = ApicSolver3::builder().withResolution({ resolutionX, resolutionX, resolutionX }).withDomainSizeX(1.0).makeShared();
  solver->setUseCompressedLinearSystem(true);

  // Build collider
  auto sphere = Sphere3::builder().withCenter({ 0.5, 0.5, 0.5 }).withRadius(0.4).withIsNormalFlipped(true).makeShared();

  auto collider = RigidBodyCollider3::builder().withSurface(sphere).makeShared();
  ...
  std::mt19937 rng;
  std::uniform_real_distribution<> dist(-0.1 * solver->gridSpacing().x, 0.1 * solver->gridSpacing().x);
  BccLatticePointGenerator pointGenerator;
  pointGenerator.forEachPoint(BoundingBox3D({ 0.75, 0, 0 }, { 1, 1, 1 }), 0.5 * solver->gridSpacing().x, [&](const Vector3D& pt) -> bool {
...
main(int argc, char* argv[])
{
  ...
  // Parsing
  auto parser = clara::Help(showHelp) | clara::Opt(resolutionX, "resolutionX")["-r"]["--resx"]("grid resolution in x-axis (default is 50)") |
                ...
                clara::Opt(exampleNum, "exampleNum")["-e"]["--example"]("example number (between 1 and 6, default is 1)") |
