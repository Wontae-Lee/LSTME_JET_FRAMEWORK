...
void
saveTriangleMesh(const TriangleMesh3& mesh, const std::string& rootDir, int frameCnt)
{
  char basename[256];
...
void
triangulateAndSave(const ScalarGrid3Ptr& sdf, const std::string& rootDir, int frameCnt)
{
  TriangleMesh3 mesh;
...
void
printInfo(const LevelSetLiquidSolver3Ptr& solver)
{
...
void
runSimulation(const std::string& rootDir, const LevelSetLiquidSolver3Ptr& solver, int numberOfFrames, double fps)
{
...
// Water-drop example
void
runExample1(const std::string& rootDir, size_t resX, int numberOfFrames, double fps)
{
  // Build solver
...
void
runExample2(const std::string& rootDir, size_t resX, int numberOfFrames, double fps)
{
  // Build solver
  auto solver = LevelSetLiquidSolver3::builder().withResolution({ 3 * resX, 2 * resX, (3 * resX) / 2 }).withDomainSizeX(3.0).makeShared();
...
void
runExample3(const std::string& rootDir, size_t resX, int numberOfFrames, double fps)
{
  // Build solver
  auto solver = LevelSetLiquidSolver3::builder().withResolution({ resX, resX, resX }).withDomainSizeX(1.0).makeShared();
  solver->setUseCompressedLinearSystem(true);
...
void
runExample4(const std::string& rootDir, size_t resX, int numberOfFrames, double fps)
{
  // Build solver
  auto solver = LevelSetLiquidSolver3::builder().withResolution({ resX, resX, resX }).withDomainSizeX(1.0).makeShared();
  solver->setUseCompressedLinearSystem(true);
...
main(int argc, char* argv[])
{
  ...
  // Parsing
  auto parser = clara::Help(showHelp) | clara::Opt(resX, "resX")["-r"]["--resx"]("grid resolution in x-axis (default is 50)") |
                ...
                clara::Opt(exampleNum, "exampleNum")["-e"]["--example"]("example number (between 1 and 4, default is 1)") |
