...
JET_BEGIN_TEST_F(GridForwardEulerDiffusionSolver3, Solve) {
    ...
    src.fill([&](const Vector3D& x) { return (x.distanceTo(src.boundingBox().midPoint()) < 0.2) ? 1.0 : 0.0; });
...
JET_BEGIN_TEST_F(GridBackwardEulerDiffusionSolver2, Solve) {
    ...
    src.fill([&](const Vector2D& x) { return (x.distanceTo(src.boundingBox().midPoint()) < 0.2) ? 1.0 : 0.0; });
    ...
    diffusionSolver.solve(src, 100.0 * diffusionCoeff, timeStep, &dst, ConstantScalarField2(kMaxD), CustomScalarField2([&](const Vector2D& pt) {
...
JET_BEGIN_TEST_F(GridBackwardEulerDiffusionSolver3, SolveWithBoundaryDirichlet) {
    Size3 size(80, 60, 75);
    Vector3D gridSpacing(1.0 / size.x, 1.0 / size.x, 1.0 / size.x);
    ...
    CustomScalarField3 boundarySdf([&](const Vector3D& x) { return boundaryCenter.x - x.x; });

    Array2<double> data(size.x, size.y);

    src.fill([&](const Vector3D& x) { return (x.distanceTo(src.boundingBox().midPoint()) < 0.2) ? 1.0 : 0.0; });

    data.forEachIndex([&](size_t i, size_t j) { data(i, j) = src(i, j, size.z / 2); });

    double timeStep = 0.01;
    double diffusionCoeff = 100 * square(gridSpacing.x) / timeStep / 12.0;

    GridBackwardEulerDiffusionSolver3 diffusionSolver(GridBackwardEulerDiffusionSolver3::Dirichlet);
...
JET_BEGIN_TEST_F(GridBackwardEulerDiffusionSolver3, SolveWithBoundaryNeumann) {
    ...
    GridBackwardEulerDiffusionSolver3 diffusionSolver(GridBackwardEulerDiffusionSolver3::Neumann);
