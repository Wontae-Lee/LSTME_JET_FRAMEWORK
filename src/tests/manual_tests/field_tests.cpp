...
#include "jet/primitive/field/scalar_field3.h"
...
class MyCustomScalarField3 final : public ScalarField3 {
 public:
    double sample(const Vector3D& x) const override { return std::sin(x.x) * std::sin(x.y) * std::sin(x.z); }

    Vector3D gradient(const Vector3D& x) const override {
        return Vector3D(std::cos(x.x) * std::sin(x.y) * std::sin(x.z), std::sin(x.x) * std::cos(x.y) * std::sin(x.z), std::sin(x.x) * std::sin(x.y) * std::cos(x.z));
    }

    double laplacian(const Vector3D& x) const override {
        return -std::sin(x.x) * std::sin(x.y) * std::sin(x.z) - std::sin(x.x) * std::sin(x.y) * std::sin(x.z) - std::sin(x.x) * std::sin(x.y) * std::sin(x.z);
    }
};
...
class MyCustomVectorField3 final : public VectorField3 {
 public:
    Vector3D sample(const Vector3D& x) const override { return Vector3D(std::sin(x.x) * std::sin(x.y), std::sin(x.y) * std::sin(x.z), std::sin(x.z) * std::sin(x.x)); }

    double divergence(const Vector3D& x) const override { return std::cos(x.x) * std::sin(x.y) + std::cos(x.y) * std::sin(x.z) + std::cos(x.z) * std::sin(x.x); }

    Vector3D curl(const Vector3D& x) const override { return Vector3D(-std::sin(x.y) * std::cos(x.z), -std::sin(x.z) * std::cos(x.x), -std::sin(x.x) * std::cos(x.y)); }
};

class MyCustomVectorField3_2 final : public VectorField3 {
 public:
    Vector3D sample(const Vector3D& x) const override { return Vector3D(-x.y, x.x, 0.0); }
};
