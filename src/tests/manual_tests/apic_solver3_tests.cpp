...
JET_BEGIN_TEST_F(ApicSolver3, WaterDrop) {
    ...
    // Build solver
    auto solver = ApicSolver3::builder().withResolution({resolutionX, 2 * resolutionX, resolutionX}).withDomainSizeX(1.0).makeShared();
...
JET_BEGIN_TEST_F(ApicSolver3, Spherical) {
    // Build solver
    auto solver = ApicSolver3::builder().withResolution({30, 30, 30}).withDomainSizeX(1.0).makeShared();
    solver->setUseCompressedLinearSystem(true);
