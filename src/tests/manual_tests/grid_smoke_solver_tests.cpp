...
#include <algorithm>
#include "jet/emitter/volume_grid_emitter2.h"
#include "jet/emitter/volume_grid_emitter3.h"
...
#include "jet/solver/grid/grid_single_phase_pressure_solver2.h"
...
JET_BEGIN_TEST_F(GridSmokeSolver2, Rising) {
    // Build solver
    auto solver = GridSmokeSolver2::builder().withResolution({32, 64}).withGridSpacing(1.0 / 32.0).makeShared();

    // Build emitter
    auto box = Box2::builder().withLowerCorner({0.3, 0.0}).withUpperCorner({0.7, 0.4}).makeShared();

    auto emitter = VolumeGridEmitter2::builder().withSourceRegion(box).makeShared();
...
JET_BEGIN_TEST_F(GridSmokeSolver2, RisingWithCollider) {
    ...
    // Build collider
    auto sphere = Sphere2::builder().withCenter({0.5, 1.0}).withRadius(0.1).makeShared();
...
JET_BEGIN_TEST_F(GridSmokeSolver2, MovingEmitterWithCollider) {
    ...
    // Build emitter
    auto box = Box2::builder().withLowerCorner({0.3, 0.0}).withUpperCorner({0.7, 0.1}).makeShared();

    auto emitter = VolumeGridEmitter2::builder().withSourceRegion(box).withIsOneShot(false).makeShared();

    solver->setEmitter(emitter);
    emitter->addStepFunctionTarget(solver->smokeDensity(), 0.0, 1.0);
    emitter->addStepFunctionTarget(solver->temperature(), 0.0, 1.0);
    emitter->setOnBeginUpdateCallback([&box](double t, double dt) {
...
JET_BEGIN_TEST_F(GridSmokeSolver3, Rising) {
    ...
    emitter->addTarget(solver->velocity(), [](double sdf, const Vector3D& pt, const Vector3D& oldVal) {
        if (sdf < 0.05) {
    ...
    for (Frame frame(0, 1.0 / 60.0); frame.index < 240; ++frame) {
        ...
        output.set(0.0);
        density->forEachDataPointIndex([&](size_t i, size_t j, size_t k) { output(i, j) += (*density)(i, j, k); });
        snprintf(filename, sizeof(filename), "data.#grid2,%04d.npy", frame.index);
