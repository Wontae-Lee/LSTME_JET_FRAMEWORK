...
JET_BEGIN_TEST_F(GridBlockedBoundaryConditionSolver2, ConstrainVelocity) {
    ...
    solver.setClosedDomainBoundaryFlag(kDirectionRight | kDirectionDown | kDirectionUp);
...
JET_BEGIN_TEST_F(GridBlockedBoundaryConditionSolver2, ConstrainVelocityWithFriction) {
    double dx = 1.0 / 32.0;
    FaceCenteredGrid2 velocity(Size2(64, 32), Vector2D(dx, dx), Vector2D());
    velocity.fill(Vector2D(1.0, 0.0));
    BoundingBox2D domain = velocity.boundingBox();
...
JET_BEGIN_TEST_F(GridFractionalBoundaryConditionSolver2, ConstrainVelocity) {
    double dx = 1.0 / 32.0;
    FaceCenteredGrid2 velocity(Size2(64, 32), Vector2D(dx, dx), Vector2D());
    velocity.fill(Vector2D(1.0, 0.0));
    BoundingBox2D domain = velocity.boundingBox();
