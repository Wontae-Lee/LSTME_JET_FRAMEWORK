...
JET_BEGIN_TEST_F(SemiLagrangian2, Boundary) {
    ...
    src.fill([&](const Vector2D& pt) -> Vector2D { return {0.5 * (std::sin(15 * pt.x) + 1.0), 0.5 * (std::sin(15 * pt.y) + 1.0)}; });

    ConstantVectorField2 flow(Vector2D(1.0, 1.0));
    CustomScalarField2 boundarySdf([](const Vector2D& pt) { return Vector2D(0.5, 0.5).distanceTo(pt) - 0.25; });
...
JET_BEGIN_TEST_F(SemiLagrangian2, Zalesak) {
    ...
    CustomVectorField2 flow([](const Vector2D& pt) { return Vector2D(kPiD / 3.14 * (0.5 - pt.y), kPiD / 3.14 * (pt.x - 0.5)); });
