...
JET_BEGIN_TEST_F(SphSolver3, SteadyState) {
    ...
    auto emitter = std::make_shared<VolumeParticleEmitter3>(std::make_shared<SurfaceToImplicit3>(std::make_shared<Sphere3>(Vector3D(), 10.0)), initialBound, particles->targetSpacing(), Vector3D());
...
JET_BEGIN_TEST_F(SphSolver3, WaterDrop) {
    ...
    auto emitter = std::make_shared<VolumeParticleEmitter3>(surfaceSet, sourceBound, targetSpacing, Vector3D());
    solver.setEmitter(emitter);
