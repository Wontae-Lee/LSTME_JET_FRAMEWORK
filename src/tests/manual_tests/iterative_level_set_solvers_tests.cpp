...
#include "jet/solver/eno_level_set/eno_level_set_solver2.h"
#include "jet/solver/eno_level_set/eno_level_set_solver3.h"
#include "jet/solver/level_set/upwind_level_set_solver2.h"
#include "jet/solver/level_set/upwind_level_set_solver3.h"
...
JET_BEGIN_TEST_F(LevelSetSolver2, Reinitialize) {
    ...
    data0.fill([&](const Vector2D& pt) { return pt.distanceTo(Vector2D(0.5, 0.75)) - 0.15; });

    auto flowFunc = [&](const Vector2D& pt) {
        Vector2D ret;
        ret.x = 2.0 * square(std::sin(kPiD * pt.x)) * std::sin(kPiD * pt.y) * std::cos(kPiD * pt.y);
        ret.y = -2.0 * square(std::sin(kPiD * pt.y)) * std::sin(kPiD * pt.x) * std::cos(kPiD * pt.x);
        return ret;
    };
...
JET_BEGIN_TEST_F(UpwindLevelSetSolver2, ReinitializeSmall) {
    ...
    sdf.fill([](const Vector2D& x) { return (x - Vector2D(20, 20)).length() - 8.0; });
...
JET_BEGIN_TEST_F(UpwindLevelSetSolver2, Reinitialize) {
    ...
    // Starting from SDF field
    sdf.fill([](const Vector2D& x) { return (x - Vector2D(80, 80)).length() - 32.0; });
    saveData(sdf.constDataAccessor(), "sdf0_#grid2,iso.npy");
...
JET_BEGIN_TEST_F(UpwindLevelSetSolver2, Extrapolate) {
    ...
    sdf.fill([&](const Vector2D& x) { return (x - Vector2D(0.75, 0.5)).length() - 0.3; });
...
JET_BEGIN_TEST_F(UpwindLevelSetSolver3, ReinitializeSmall) {
    ...
    sdf.fill([](const Vector3D& x) { return (x - Vector3D(20, 20, 20)).length() - 8.0; });
