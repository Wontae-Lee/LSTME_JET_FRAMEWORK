...
JET_BEGIN_TEST_F(PicSolver2, Rotation) {
    // Build solver
    auto solver = PicSolver2::builder().withResolution({10, 10}).withDomainSizeX(1.0).makeShared();
...
JET_BEGIN_TEST_F(PicSolver2, DamBreaking) {
    // Build solver
    auto solver = PicSolver2::builder().withResolution({100, 100}).withDomainSizeX(1.0).makeShared();
