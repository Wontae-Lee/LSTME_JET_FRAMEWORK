...
JET_BEGIN_TEST_F(FlipSolver2, SteadyState) {
    // Build solver
    auto solver = FlipSolver2::builder().withResolution({32, 32}).withDomainSizeX(1.0).makeShared();
...
JET_BEGIN_TEST_F(FlipSolver2, Rotation) {
    // Build solver
    auto solver = FlipSolver2::builder().withResolution({10, 10}).withDomainSizeX(1.0).makeShared();
...
JET_BEGIN_TEST_F(FlipSolver2, DamBreaking) {
    // Build solver
    auto solver = FlipSolver2::builder().withResolution({100, 100}).withDomainSizeX(1.0).makeShared();
...
JET_BEGIN_TEST_F(FlipSolver2, RotatingTank) {
    ...
    // Build emitter
    auto box = Box2::builder().withLowerCorner({0.25, 0.25}).withUpperCorner({0.75, 0.50}).makeShared();
    ...
    // Build collider
    auto tank = Box2::builder().withLowerCorner({-0.25, -0.25}).withUpperCorner({0.25, 0.25}).withTranslation({0.5, 0.5}).withOrientation(0.0).withIsNormalFlipped(true).makeShared();

    auto collider = RigidBodyCollider2::builder().withSurface(tank).withAngularVelocity(2.0).makeShared();
