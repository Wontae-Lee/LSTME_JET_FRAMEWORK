...
JET_BEGIN_TEST_F(PciSphSolver2, SteadyState) {
    ...
    auto emitter = std::make_shared<VolumeParticleEmitter2>(std::make_shared<SurfaceToImplicit2>(std::make_shared<Sphere2>(Vector2D(), 10.0)), initialBound, targetSpacing, Vector2D());
...
JET_BEGIN_TEST_F(PciSphSolver2, WaterDrop) {
    ...
    auto emitter = std::make_shared<VolumeParticleEmitter2>(surfaceSet, sourceBound, targetSpacing, Vector2D());
    solver.setEmitter(emitter);
...
JET_BEGIN_TEST_F(PciSphSolver2, RotatingTank) {
    ...
    // Build solver
    auto solver = PciSphSolver2::builder().withTargetSpacing(targetSpacing).makeShared();

    solver->setViscosityCoefficient(0.01);

    // Build emitter
    auto box = Box2::builder().withLowerCorner({0.25 + targetSpacing, 0.25 + targetSpacing}).withUpperCorner({0.75 - targetSpacing, 0.50}).makeShared();

    auto emitter = VolumeParticleEmitter2::builder().withSurface(box).withSpacing(targetSpacing).withIsOneShot(true).makeShared();
