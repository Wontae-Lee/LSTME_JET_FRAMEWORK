...
namespace {
void
saveTriangleMesh(const TriangleMesh3& mesh, const std::string& filename)
{
...
void
triangulateAndSave(const ScalarGrid3Ptr& sdf, const std::string& filename)
{
    TriangleMesh3 mesh;
...
JET_BEGIN_TEST_F(LevelSetLiquidSolver3, SubtleSloshing)
{
    ...
    for (Frame frame(0, 1.0 / 60.0); frame.index < 120; frame.advance()) {
        ...
        output.forEachIndex(sdfToBinary);
        snprintf(
            filename, sizeof(filename), "data.#grid2,%04d.npy", frame.index);
        saveData(output.constAccessor(), filename);

        snprintf(
            filename, sizeof(filename), "data.#grid2,%04d.obj", frame.index);
        triangulateAndSave(sdf, getFullFilePath(filename));
    }
}
JET_END_TEST_F
