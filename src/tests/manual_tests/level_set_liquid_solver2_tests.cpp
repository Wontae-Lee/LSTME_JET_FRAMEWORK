...
#include "jet/primitive/plane/plane2.h"
...
JET_BEGIN_TEST_F(LevelSetLiquidSolver2, Drop) {
    ...
    output.forEachIndex([&](size_t i, size_t j) { output(i, j) = 1.0 - smearedHeavisideSdf((*sdf)(i, j) / dx); });
...
JET_BEGIN_TEST_F(LevelSetLiquidSolver2, DropStopAndGo) {
    ...
    {
        // Build solver
        auto solver = LevelSetLiquidSolver2::builder().withResolution({32, 64}).withDomainSizeX(1.0).makeShared();
        ...
        // Build emitter
        auto plane = Plane2::builder().withNormal({0, 1}).withPoint({0, 0.5}).makeShared();

        auto sphere = Sphere2::builder().withCenter(domain.midPoint()).withRadius(0.15).makeShared();

        auto surfaceSet = ImplicitSurfaceSet2::builder().withExplicitSurfaces({plane, sphere}).makeShared();

        auto emitter = VolumeGridEmitter2::builder().withSourceRegion(surfaceSet).makeShared();
...
JET_BEGIN_TEST_F(LevelSetLiquidSolver2, DropWithCollider) {
    ...
    output.forEachIndex([&](size_t i, size_t j) { output(i, j) = smearedHeavisideSdf((*sdf)(i, j) / dx); });
...
JET_BEGIN_TEST_F(LevelSetLiquidSolver2, RisingFloor) {
    // Build solver
    auto solver = LevelSetLiquidSolver2::builder().withResolution({5, 10}).withDomainSizeX(1.0).makeShared();
    solver->setGravity({0, 0, 0});

    // Build emitter
    auto box = Box2::builder().withLowerCorner({0.0, 0.0}).withUpperCorner({1.0, 0.8}).makeShared();
    ...
    // Build collider
    auto tank = Box2::builder().withLowerCorner({-1, 0}).withUpperCorner({2, 2}).withIsNormalFlipped(true).makeShared();

    auto collider = RigidBodyCollider2::builder().withSurface(tank).makeShared();
    ...
    for (Frame frame(0, 1 / 100.0); frame.index < 120; ++frame) {
        ...
        output.forEachIndex([&](size_t i, size_t j) { output(i, j) = 1.0 - smearedHeavisideSdf((*sdf)(i, j) * 5.0); });
        snprintf(filename, sizeof(filename), "output.#grid2,%04d.npy", frame.index);
        saveData(output.constAccessor(), filename);
    }
}
JET_END_TEST_F
