...
JET_BEGIN_TEST_F(VolumeParticleEmitter2, EmitContinuousNonOverlapping) {
    ...
    auto emitter = std::make_shared<VolumeParticleEmitter2>(std::make_shared<SurfaceToImplicit2>(std::make_shared<Sphere2>(Vector2D(), 1.0)), BoundingBox2D(Vector2D(-1, -1), Vector2D(1, 1)), 0.2);
...
JET_BEGIN_TEST_F(VolumeParticleEmitter3, EmitContinuousNonOverlapping) {
    ...
    auto emitter =
        std::make_shared<VolumeParticleEmitter3>(std::make_shared<SurfaceToImplicit3>(std::make_shared<Sphere3>(Vector3D(), 1.0)), BoundingBox3D(Vector3D(-1, -1, -1), Vector3D(1, 1, 1)), 0.2);
    emitter->setIsOneShot(false);
    emitter->setAllowOverlapping(false);
    solver.setEmitter(emitter);
