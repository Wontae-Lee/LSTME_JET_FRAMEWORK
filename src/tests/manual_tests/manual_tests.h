    ...
    class testSetName##Tests : public ::testing::Test {                                                                                                         \
     ...
     protected:                                                                                                                                                 \
        ...
        void saveData(const ConstArrayAccessor2<T>& data, const std::string& name) {                                                                            \
            std::string filename = getFullFilePath(name);                                                                                                       \
            unsigned int dim[2] = {static_cast<unsigned int>(data.height()), static_cast<unsigned int>(data.width())};                                          \
        ...
        void saveData(const ConstArrayAccessor3<T>& data, const std::string& name) {                                                                            \
            std::string filename = getFullFilePath(name);                                                                                                       \
            unsigned int dim[3] = {static_cast<unsigned int>(data.depth()), static_cast<unsigned int>(data.height()), static_cast<unsigned int>(data.width())}; \
        ...
        std::string getTestDirectoryName(const std::string& name) { return pystring::os::path::join(_testCollectionDir, name); }                                \
        std::string getResourceFileName(const std::string& name) { return pystring::os::path::join(RESOURCES_DIR, name); }                                      \
    };
