...
JET_BEGIN_TEST_F(ParticleSystemSolver3, Update) {
    auto plane = Plane3::builder().withNormal({0, 1, 0}).withPoint({0, 0, 0}).makeShared();

    auto collider = RigidBodyCollider3::builder().withSurface(plane).makeShared();

    auto wind = ConstantVectorField3::builder().withValue({1, 0, 0}).makeShared();

    auto emitter = PointParticleEmitter3::builder().withOrigin({0, 3, 0}).withDirection({0, 1, 0}).withSpeed(5).withSpreadAngleInDegrees(45.0).withMaxNumberOfNewParticlesPerSecond(300).makeShared();
