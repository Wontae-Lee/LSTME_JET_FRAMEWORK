...
#include "jet/emitter/particle_emitter_set3.h"
#include "jet/emitter/volume_particle_emitter3.h"
#include "jet/figure/box3.h"
#include "jet/figure/cylinder3.h"
#include "jet/figure/sphere3.h"
#include "jet/generator/grid_point_generator3.h"
...
JET_BEGIN_TEST_F(FlipSolver3, RotatingTank) {
    // Build solver
    auto solver = FlipSolver3::builder().withResolution({32, 32, 32}).withDomainSizeX(1.0).makeShared();

    // Build emitter
    auto box = Box3::builder().withLowerCorner({0.25, 0.25, 0.25}).withUpperCorner({0.75, 0.50, 0.75}).makeShared();

    auto emitter = VolumeParticleEmitter3::builder().withSurface(box).withSpacing(1.0 / 64.0).withIsOneShot(true).makeShared();
    ...
    auto collider = RigidBodyCollider3::builder().withSurface(tank).withAngularVelocity({0, 0, 2}).makeShared();
