...
#include "jet/solver/apic/apic_solver2.h"
...
JET_BEGIN_TEST_F(ApicSolver2, SteadyState) {
    // Build solver
    auto solver = ApicSolver2::builder().withResolution({32, 32}).withDomainSizeX(1.0).makeShared();

    // Build emitter
    auto box = Box2::builder().withLowerCorner({0.0, 0.0}).withUpperCorner({1.0, 0.5}).makeShared();

    auto emitter = VolumeParticleEmitter2::builder().withSurface(box).withSpacing(1.0 / 64.0).withIsOneShot(true).makeShared();
...
JET_BEGIN_TEST_F(ApicSolver2, Rotation) {
    // Build solver
    auto solver = ApicSolver2::builder().withResolution({10, 10}).withDomainSizeX(1.0).makeShared();
    ...
    auto emitter = VolumeParticleEmitter2::builder().withSurface(box).withSpacing(1.0 / 20.0).withIsOneShot(true).makeShared();
...
JET_BEGIN_TEST_F(ApicSolver2, DamBreaking) {
    // Build solver
    auto solver = ApicSolver2::builder().withResolution({100, 100}).withDomainSizeX(1.0).makeShared();

    // Build emitter
    auto box = Box2::builder().withLowerCorner({0.0, 0.0}).withUpperCorner({0.2, 0.8}).makeShared();

    auto emitter = VolumeParticleEmitter2::builder().withSurface(box).withSpacing(0.005).withIsOneShot(true).makeShared();
...
JET_BEGIN_TEST_F(ApicSolver2, LeftWall) {
    ...
    // Build emitter
    auto box = Box2::builder().withLowerCorner({0.0, 0.0}).withUpperCorner({0.01, 0.5}).makeShared();
...
JET_BEGIN_TEST_F(ApicSolver2, RightWall) {
    ...
    // Build emitter
    auto box = Box2::builder().withLowerCorner({1.0 - 1.0 / 64.0, 0.0}).withUpperCorner({1.0, 0.5}).makeShared();
...
JET_BEGIN_TEST_F(ApicSolver2, LeftWallPic) {
    // Build solver
    auto solver = PicSolver2::builder().withResolution({32, 32}).withDomainSizeX(1.0).makeShared();
...
JET_BEGIN_TEST_F(ApicSolver2, Circular) {
    // Build solver
    auto solver = ApicSolver2::builder().withResolution({40, 40}).withDomainSizeX(1.0).makeShared();

    // Build collider
    auto sphere = Sphere2::builder().withCenter({0.5, 0.5}).withRadius(0.4).withIsNormalFlipped(true).makeShared();
