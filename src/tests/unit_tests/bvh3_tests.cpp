...
TEST(Bvh3, Nearest) {
    ...
    auto distanceFunc = [](const Vector3D& a, const Vector3D& b) { return a.distanceTo(b); };
...
TEST(Bvh3, ForEachOverlappingItems) {
    ...
    auto overlapsFunc = [](const Vector3D& pt, const BoundingBox3D& bbox) { return bbox.contains(pt); };
