...
TEST(Bvh2, Nearest) {
    ...
    auto distanceFunc = [](const Vector2D& a, const Vector2D& b) { return a.distanceTo(b); };
...
TEST(Bvh2, RayIntersects) {
    ...
    for (i = 0; i < numSamples / 2; ++i) {
        Ray2D ray(getSampleDirs2()[i + numSamples / 2], getSampleDirs2()[i + numSamples / 2]);
        // ad-hoc search
        bool ansInts = false;
...
TEST(Bvh2, ForEachOverlappingItems) {
    ...
    auto overlapsFunc = [](const Vector2D& pt, const BoundingBox2D& bbox) { return bbox.contains(pt); };
