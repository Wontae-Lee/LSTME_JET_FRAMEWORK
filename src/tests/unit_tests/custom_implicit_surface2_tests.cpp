...
TEST(CustomImplicitSurface2, SignedDistance) {
    CustomImplicitSurface2 cis([](const Vector2D& pt) { return (pt - Vector2D(0.5, 0.5)).length() - 0.25; }, BoundingBox2D({0, 0}, {1, 1}), 1e-3);
...
TEST(CustomImplicitSurface2, CloseestPoint) {
    ...
    CustomImplicitSurface2 cis1([&](const Vector2D& pt) { return refSurf.signedDistance(pt); }, BoundingBox2D({0, 0}, {1, 1}), 1e-3);
...
TEST(CustomImplicitSurface2, ClosestIntersection) {
    ...
    CustomImplicitSurface2 cis1([&](const Vector2D& pt) { return refSurf.signedDistance(pt); }, BoundingBox2D({0, 0}, {1, 1}), 1e-3, 1e-3);
