...
TEST(Octree, Nearest)
{
    ...
    octree.build(
        points, BoundingBox3D({ 0, 0, 0 }, { 1, 1, 1 }), overlapsFunc, 5);
...
TEST(Octree, ForEachOverlappingItems)
{
    ...
    octree.forEachIntersectingItem(
        testBox, overlapsFunc, [&](const Vector3D& pt) {
...
TEST(Octree, RayIntersects)
{
    ...
    octree.build(
        items, BoundingBox3D({ 0, 0, 0 }, { 1, 1, 1 }), overlapsFunc, 5);
