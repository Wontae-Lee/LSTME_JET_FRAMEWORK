...
TEST(FaceCenteredGrid2, Constructors)
{
    ...
    grid2.forEachUIndex(
        [&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(5.0, grid2.u(i, j)); });
    grid2.forEachVIndex(
        [&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(6.0, grid2.v(i, j)); });
    // Copy constructor
    ...
    grid3.forEachUIndex(
        [&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(5.0, grid3.u(i, j)); });
    grid3.forEachVIndex(
        [&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(6.0, grid3.v(i, j)); });
}
...
{
    {
        ...
        auto grid = builder.build(Size2(5, 2),
                                  Vector2D(2.0, 4.0),
                                  Vector2D(-1.0, 2.0),
                                  Vector2D(3.0, 5.0));
    ...
    }
    {
        ...
        EXPECT_EQ(Vector2D(-1.0, 2.0), grid.origin());
        grid.forEachUIndex(
            [&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(3.0, grid.u(i, j)); });
        grid.forEachVIndex(
            [&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(5.0, grid.v(i, j)); });
    }
}
