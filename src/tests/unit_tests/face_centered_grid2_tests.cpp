...
TEST(FaceCenteredGrid2, ValueAtCellCenter) {
    FaceCenteredGrid2 grid(5, 8, 2.0, 3.0);
    grid.fill([&](const Vector2D& x) { return Vector2D(3.0 * x.y + 1.0, 5.0 * x.x + 7.0); });
...
TEST(FaceCenteredGrid2, Builder) {
    {
        ...
        auto grid = builder.build(Size2(5, 2), Vector2D(2.0, 4.0), Vector2D(-1.0, 2.0), Vector2D(3.0, 5.0));
        ...
        faceCenteredGrid->forEachUIndex([&faceCenteredGrid](size_t i, size_t j) { EXPECT_DOUBLE_EQ(3.0, faceCenteredGrid->u(i, j)); });
        faceCenteredGrid->forEachVIndex([&faceCenteredGrid](size_t i, size_t j) { EXPECT_DOUBLE_EQ(5.0, faceCenteredGrid->v(i, j)); });
    }

    {
        auto grid = FaceCenteredGrid2::builder().withResolution(5, 2).withGridSpacing(2, 4).withOrigin(-1, 2).withInitialValue(3, 5).build();
...
TEST(FaceCenteredGrid2, Serialization) {
    ...
    grid1.forEachUIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(grid1.u(i, j), grid2.u(i, j)); });

    grid1.forEachVIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(grid1.v(i, j), grid2.v(i, j)); });
