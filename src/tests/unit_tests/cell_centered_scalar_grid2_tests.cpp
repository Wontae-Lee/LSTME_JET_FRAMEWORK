...
TEST(CellCenteredScalarGrid2, Builder) {
    {
        auto grid1 = CellCenteredScalarGrid2::builder().build(Size2(3, 8), Vector2D(2.0, 3.0), Vector2D(1.0, 5.0), 4.0);
    ...
    {
        auto grid1 = CellCenteredScalarGrid2::builder().withResolution(3, 8).withGridSpacing(2, 3).withOrigin(1, 5).withInitialValue(4.0).build();
        EXPECT_EQ(3u, grid1.resolution().x);
        EXPECT_EQ(8u, grid1.resolution().y);
        EXPECT_DOUBLE_EQ(2.0, grid1.gridSpacing().x);
        EXPECT_DOUBLE_EQ(3.0, grid1.gridSpacing().y);
...
TEST(CellCenteredScalarGrid2, LaplacianAtAtDataPoint) {
    ...
    grid.fill([](const Vector2D& x) { return square(x.x) + 2.0 * square(x.y); });
...
TEST(CellCenteredScalarGrid2, Serialization) {
    ...
    grid1.forEachDataPointIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(grid1(i, j), grid2(i, j)); });
