...
TEST(CellCenteredScalarGrid3, Builder) {
    ...
    {
        auto grid1 = CellCenteredScalarGrid3::builder().withResolution(3, 8, 5).withGridSpacing(2, 3, 1).withOrigin(5, 4, 7).withInitialValue(8.0).build();
        EXPECT_EQ(3u, grid1.resolution().x);
        EXPECT_EQ(8u, grid1.resolution().y);
        EXPECT_EQ(5u, grid1.resolution().z);
        EXPECT_DOUBLE_EQ(2.0, grid1.gridSpacing().x);
...
TEST(CellCenteredScalarGrid3, LaplacianAtAtDataPoint) {
    ...
    grid.fill([](const Vector3D& x) { return square(x.x) + 2.0 * square(x.y) - 4.0 * square(x.z); });
...
TEST(CellCenteredScalarGrid3, Serialization) {
    ...
    grid1.forEachDataPointIndex([&](size_t i, size_t j, size_t k) { EXPECT_DOUBLE_EQ(grid1(i, j, k), grid2(i, j, k)); });
