...
class VolumeParticleEmitter3 : public ::benchmark::Fixture {
 protected:
    ...
    void SetUp(const ::benchmark::State&) {
        ...
        // Build emitter
        auto box1 = Box3::builder().withLowerCorner({0, 0, 0}).withUpperCorner({0.5 * lx + pd, 0.75 * ly + pd, 0.75 * lz + pd}).makeShared();

        auto box2 = Box3::builder().withLowerCorner({2.5 * lx - pd, 0, 0.25 * lz - pd}).withUpperCorner({3.5 * lx + pd, 0.75 * ly + pd, 1.5 * lz + pd}).makeShared();

        auto boxSet = ImplicitSurfaceSet3::builder().withExplicitSurfaces({box1, box2}).makeShared();

        emitter = jet::VolumeParticleEmitter3::builder().withSurface(boxSet).withMaxRegion(BoundingBox3D({0, 0, 0}, {lx, ly, lz})).withSpacing(0.5 * dx).withAllowOverlapping(true).makeShared();
