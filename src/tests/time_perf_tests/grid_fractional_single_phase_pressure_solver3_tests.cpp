...
(benchmark::State& state) {
    ...
    while (state.KeepRunning()) {
        solver.solve(vel, 1.0, &vel, ConstantScalarField3(kMaxD), ConstantVectorField3({0, 0, 0}), fluidSdf, compressed);
    }
}

BENCHMARK_REGISTER_F(GridFractionalSinglePhasePressureSolver3, Solve)->Args({128, 128, 0})->Args({128, 128, 1})->Args({128, 64, 0})->Args({128, 64, 1})->Args({128, 32, 0})->Args({128, 32, 1});
