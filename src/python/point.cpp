...
void addPoint2UI(pybind11::module& m) {
    py::class_<Point2UI>(m, "Point2UI")
        // CTOR
        .def(
            "__init__", [](Point2UI& instance, size_t x, size_t y) { new (&instance) Point2UI(x, y); },
            R"pbdoc(
             Constructs Point2UI
...
void addPoint3UI(pybind11::module& m) {
    py::class_<Point3UI>(m, "Point3UI")
        // CTOR
        .def(
            "__init__", [](Point3UI& instance, size_t x, size_t y, size_t z) { new (&instance) Point3UI(x, y, z); },
            R"pbdoc(
             Constructs Point3UI
