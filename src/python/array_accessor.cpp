...
void addArrayAccessor1Scalar(py::module m, const char* name) {
    py::class_<ArrayAccessor1<T>>(m, name, py::buffer_protocol()).def_buffer([](ArrayAccessor1<T>& m) -> py::buffer_info {
        return py::buffer_info(m.data(), sizeof(T), py::format_descriptor<T>::format(), 1, {m.size()}, {sizeof(T)});
    });
};

template <typename T, size_t N>
void addArrayAccessor1Vector(py::module m, const char* name) {
    py::class_<ArrayAccessor1<Vector<T, N>>>(m, name, py::buffer_protocol()).def_buffer([](ArrayAccessor1<Vector<T, N>>& m) -> py::buffer_info {
        return py::buffer_info(m.data(), sizeof(T), py::format_descriptor<T>::format(), 2, {m.size(), N}, {sizeof(T) * N, sizeof(T)});
    });
};

template <typename T>
void addArrayAccessor2Scalar(py::module m, const char* name) {
    py::class_<ArrayAccessor2<T>>(m, name, py::buffer_protocol()).def_buffer([](ArrayAccessor2<T>& m) -> py::buffer_info {
        return py::buffer_info(m.data(), sizeof(T), py::format_descriptor<T>::format(), 2, {m.height(), m.width()}, {sizeof(T) * m.width(), sizeof(T)});
    });
};

template <typename T>
void addArrayAccessor3Scalar(py::module m, const char* name) {
    py::class_<ArrayAccessor3<T>>(m, name, py::buffer_protocol()).def_buffer([](ArrayAccessor3<T>& m) -> py::buffer_info {
        return py::buffer_info(m.data(), sizeof(T), py::format_descriptor<T>::format(), 3, {m.depth(), m.height(), m.width()}, {sizeof(T) * m.width() * m.height(), sizeof(T) * m.width(), sizeof(T)});
    });
};
