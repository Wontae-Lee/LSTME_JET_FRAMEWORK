...
void addFrame(pybind11::module& m) {
    py::class_<Frame>(m, "Frame", R"pbdoc(
        ...
        .def(
            "__init__", [](Frame& instance, int index, double timeIntervalInSeconds) { new (&instance) Frame(index, timeIntervalInSeconds); },
            R"pbdoc(
             Constructs Frame
        ...
        .def_readwrite("index", &Frame::index, R"pbdoc(Index of the frame)pbdoc")
        .def_readwrite("timeIntervalInSeconds", &Frame::timeIntervalInSeconds, R"pbdoc(Time interval of the frame in seconds)pbdoc")
        .def("timeInSeconds", &Frame::timeInSeconds, R"pbdoc(Elapsed time in seconds)pbdoc")
        .def(
            "advance", [](Frame& instance, int delta) { instance.advance(delta); },
            R"pbdoc(
...
class PyAnimation : public Animation {
 public:
    ...
    void onUpdate(const Frame& frame) override { PYBIND11_OVERLOAD_PURE(void, Animation, onUpdate, frame); }
};
