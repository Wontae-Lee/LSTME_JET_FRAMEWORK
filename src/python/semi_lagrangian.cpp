...
using namespace jet;
void
addSemiLagrangian2(py::module& m)
{
    py::class_<SemiLagrangian2, SemiLagrangian2Ptr, AdvectionSolver2>(
        ...
        .def(
            "solve",
            [](SemiLagrangian2& instance,
               const Grid2Ptr& input,
               const VectorField2Ptr& flow,
               double dt,
               Grid2Ptr output,
               const ScalarField2Ptr& boundarySdf) {
                ...
                if (inputSG != nullptr && outputSG != nullptr) {
                    instance.advect(
                        *inputSG, *flow, dt, outputSG.get(), *boundarySdf);
                } else if (inputCG != nullptr && outputCG != nullptr) {
                    instance.advect(
                        *inputCG, *flow, dt, outputCG.get(), *boundarySdf);
                } else if (inputFG != nullptr && outputFG != nullptr) {
                    instance.advect(
                        *inputFG, *flow, dt, outputFG.get(), *boundarySdf);
                } else {
...
void
addSemiLagrangian3(py::module& m)
{
    py::class_<SemiLagrangian3, SemiLagrangian3Ptr, AdvectionSolver3>(
        ...
        .def(
            "solve",
            [](SemiLagrangian3& instance,
               const Grid3Ptr& input,
               const VectorField3Ptr& flow,
               double dt,
               Grid3Ptr output,
