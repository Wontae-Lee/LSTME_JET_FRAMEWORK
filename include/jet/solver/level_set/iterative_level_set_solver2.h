...
class IterativeLevelSetSolver2 : public LevelSetSolver2 {
 ...
 private:
    ...
    void extrapolate(const ConstArrayAccessor2<double>& input, const ConstArrayAccessor2<double>& sdf, const Vector2D& gridSpacing, double maxDistance, ArrayAccessor2<double> output);

    static unsigned int distanceToNumberOfIterations(double distance, double dtau);

    static double sign(const ConstArrayAccessor2<double>& sdf, const Vector2D& gridSpacing, size_t i, size_t j);

    double pseudoTimeStep(ConstArrayAccessor2<double> sdf, const Vector2D& gridSpacing);
};
