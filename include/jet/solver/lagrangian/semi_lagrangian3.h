...
class SemiLagrangian3 : public AdvectionSolver3 {
 public:
    ...
    //!     field.
    //!
    void advect(const CollocatedVectorGrid3& input, const VectorField3& flow, double dt, CollocatedVectorGrid3* output,
    ...
    //!     field.
    //!
    void advect(const FaceCenteredGrid3& input, const VectorField3& flow, double dt, FaceCenteredGrid3* output,
                const ScalarField3& boundarySdf = ConstantScalarField3(std::numeric_limits<double>::max())) final;
 ...
 private:
    Vector3D backTrace(const VectorField3& flow, double dt, double h, const Vector3D& pt0, const ScalarField3& boundarySdf);
};
