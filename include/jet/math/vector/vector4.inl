...
template<typename T>
void
Vector<T, 4>::set(T newX, T newY, T newZ, T newW)
{
  x = newX;
  y = newY;
  z = newZ;
...
template<typename T>
void
Vector<T, 4>::set(const Vector<T, 3>& pt, T newW)
{
  x = pt.x;
  y = pt.y;
  z = pt.z;
...
template<typename U>
void
Vector<T, 4>::set(const std::initializer_list<U>& lst)
{
...
template<typename T>
Vector<T, 4>
Vector<T, 4>::add(const Vector& v) const
{
...
template<typename T>
Vector<T, 4>
Vector<T, 4>::sub(const Vector& v) const
{
  return Vector(x - v.x, y - v.y, z - v.z, w - v.w);
}
...
template<typename T>
Vector<T, 4>
Vector<T, 4>::mul(const Vector& v) const
{
  return Vector(x * v.x, y * v.y, z * v.z, w * v.w);
}
...
template<typename T>
Vector<T, 4>
Vector<T, 4>::div(const Vector& v) const
{
  return Vector(x / v.x, y / v.y, z / v.z, w / v.w);
}
...
template<typename T>
T
Vector<T, 4>::dot(const Vector& v) const
{
...
template<typename T>
Vector<T, 4>
Vector<T, 4>::rsub(const Vector& v) const
{
...
template<typename T>
Vector<T, 4>
Vector<T, 4>::rdiv(const Vector& v) const
{
  return Vector(v.x / x, v.y / y, v.z / z, v.w / w);
}
...
template<typename T>
void
Vector<T, 4>::iadd(const Vector& v)
{
  x += v.x;
  y += v.y;
  z += v.z;
...
template<typename T>
void
Vector<T, 4>::isub(const Vector& v)
{
  x -= v.x;
  y -= v.y;
  z -= v.z;
...
template<typename T>
void
Vector<T, 4>::imul(const Vector& v)
{
  x *= v.x;
  y *= v.y;
  z *= v.z;
...
template<typename T>
void
Vector<T, 4>::idiv(const Vector& v)
{
  x /= v.x;
  y /= v.y;
  z /= v.z;
...
template<typename T>
size_t
Vector<T, 4>::dominantAxis() const
{
  return (std::fabs(x) > std::fabs(y))
           ? ((std::fabs(x) > std::fabs(z)) ? ((std::fabs(x) > std::fabs(w)) ? 0 : 3) : ((std::fabs(z) > std::fabs(w)) ? 2 : 3))
           : ((std::fabs(y) > std::fabs(z)) ? ((std::fabs(y) > std::fabs(w)) ? 1 : 3) : ((std::fabs(z) > std::fabs(w)) ? 2 : 3));
}

template<typename T>
size_t
Vector<T, 4>::subminantAxis() const
{
  return (std::fabs(x) < std::fabs(y))
           ? ((std::fabs(x) < std::fabs(z)) ? ((std::fabs(x) < std::fabs(w)) ? 0 : 3) : ((std::fabs(z) < std::fabs(w)) ? 2 : 3))
           : ((std::fabs(y) < std::fabs(z)) ? ((std::fabs(y) < std::fabs(w)) ? 1 : 3) : ((std::fabs(z) < std::fabs(w)) ? 2 : 3));
...
template<typename T>
T
Vector<T, 4>::lengthSquared() const
{
...
template<typename T>
T
Vector<T, 4>::distanceTo(const Vector<T, 4>& other) const
{
...
template<typename T>
T
Vector<T, 4>::distanceSquaredTo(const Vector<T, 4>& other) const
{
...
template<typename T>
bool
Vector<T, 4>::isEqual(const Vector& other) const
{
...
template<typename T>
bool
Vector<T, 4>::isSimilar(const Vector& other, T epsilon) const
{
...
template<typename U>
Vector<T, 4>&
Vector<T, 4>::operator=(const std::initializer_list<U>& lst)
{
  set(lst);
  return (*this);
}
...
template<typename T>
Vector<T, 4>&
Vector<T, 4>::operator=(const Vector& v)
{
  set(v);
  return (*this);
}
...
template<typename T>
bool
Vector<T, 4>::operator==(const Vector& v) const
{
  return x == v.x && y == v.y && z == v.z && w == v.w;
}
...
template<typename T>
Vector<T, 4>
monotonicCatmullRom(const Vector<T, 4>& v0, const Vector<T, 4>& v1, const Vector<T, 4>& v2, const Vector<T, 4>& v3, T f)
{
  ...
  if (std::fabs(D1.w) < std::numeric_limits<float>::epsilon() || sign(D1.w) != sign(d1.w) || sign(D1.w) != sign(d2.w)) {
    d1.w = d2.w = 0;
  }
