...
#include "jet/engine/list_query_engine3.h"
...
template <typename T>
void ListQueryEngine3<T>::forEachIntersectingItem(const BoundingBox3D& box, const BoxIntersectionTestFunc3<T>& testFunc, const IntersectionVisitorFunc3<T>& visitorFunc) const {
    for (const auto& item : _items) {
        if (testFunc(item, box)) {
            visitorFunc(item);
        }
...
template <typename T>
void ListQueryEngine3<T>::forEachIntersectingItem(const Ray3D& ray, const RayIntersectionTestFunc3<T>& testFunc, const IntersectionVisitorFunc3<T>& visitorFunc) const {
    for (const auto& item : _items) {
        if (testFunc(item, ray)) {
            visitorFunc(item);
        }
...
template <typename T>
ClosestIntersectionQueryResult3<T> ListQueryEngine3<T>::closestIntersection(const Ray3D& ray, const GetRayIntersectionFunc3<T>& testFunc) const {
...
template <typename T>
NearestNeighborQueryResult3<T> ListQueryEngine3<T>::nearest(const Vector3D& pt, const NearestNeighborDistanceFunc3<T>& distanceFunc) const {
