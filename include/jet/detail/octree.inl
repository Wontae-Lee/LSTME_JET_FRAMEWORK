...
namespace jet
{
template <typename T> bool Octree<T>::Node::isLeaf() const
{
...
template <typename T>
NearestNeighborQueryResult3<T> Octree<T>::nearest(const Vector3D &pt,
...
template <typename T> bool Octree<T>::intersects(const Ray3D &ray, const RayIntersectionTestFunc3<T> &testFunc) const
{
...
template <typename T> typename Octree<T>::Iterator Octree<T>::begin()
{
    return _items.begin();
}

template <typename T> typename Octree<T>::Iterator Octree<T>::end()
{
    return _items.end();
}

template <typename T> typename Octree<T>::ConstIterator Octree<T>::begin() const
{
    return _items.begin();
}

template <typename T> typename Octree<T>::ConstIterator Octree<T>::end() const
{
    return _items.end();
}

template <typename T> size_t Octree<T>::numberOfItems() const
{
    return _items.size();
}

template <typename T> const T &Octree<T>::item(size_t i) const
{
    return _items[i];
}

template <typename T> size_t Octree<T>::numberOfNodes() const
{
    return _nodes.size();
}

template <typename T> const std::vector<size_t> &Octree<T>::itemsAtNode(size_t nodeIdx) const
{
    return _nodes[nodeIdx].items;
}

template <typename T> size_t Octree<T>::childIndex(size_t nodeIdx, size_t childIdx) const
{
    return _nodes[nodeIdx].firstChild + childIdx;
}

template <typename T> const BoundingBox3D &Octree<T>::boundingBox() const
{
    return _bbox;
}

template <typename T> size_t Octree<T>::maxDepth() const
{
    return _maxDepth;
}
...
template <typename T>
bool Octree<T>::intersects(const BoundingBox3D &box, const BoxIntersectionTestFunc3<T> &testFunc, size_t nodeIdx,
...
template <typename T>
bool Octree<T>::intersects(const Ray3D &ray, const RayIntersectionTestFunc3<T> &testFunc, size_t nodeIdx,
                           const BoundingBox3D &bound) const
{
