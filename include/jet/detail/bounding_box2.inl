...
namespace jet
{
template <typename T> BoundingBox<T, 2>::BoundingBox()
{
    reset();
}

template <typename T> BoundingBox<T, 2>::BoundingBox(const Vector2<T> &point1, const Vector2<T> &point2)
{
...
template <typename T> T BoundingBox<T, 2>::width() const
{
    return upperCorner.x - lowerCorner.x;
}

template <typename T> T BoundingBox<T, 2>::height() const
{
    return upperCorner.y - lowerCorner.y;
}

template <typename T> T BoundingBox<T, 2>::length(size_t axis)
{
    return upperCorner[axis] - lowerCorner[axis];
}

template <typename T> bool BoundingBox<T, 2>::overlaps(const BoundingBox &other) const
{
...
template <typename T> bool BoundingBox<T, 2>::contains(const Vector2<T> &point) const
{
...
template <typename T> bool BoundingBox<T, 2>::intersects(const Ray2<T> &ray) const
{
    T tMin = 0;
...
template <typename T> BoundingBoxRayIntersection2<T> BoundingBox<T, 2>::closestIntersection(const Ray2<T> &ray) const
{
...
template <typename T> Vector2<T> BoundingBox<T, 2>::midPoint() const
{
    return (upperCorner + lowerCorner) / static_cast<T>(2);
}

template <typename T> T BoundingBox<T, 2>::diagonalLength() const
{
    return (upperCorner - lowerCorner).length();
}

template <typename T> T BoundingBox<T, 2>::diagonalLengthSquared() const
{
    return (upperCorner - lowerCorner).lengthSquared();
}

template <typename T> void BoundingBox<T, 2>::reset()
{
...
template <typename T> void BoundingBox<T, 2>::merge(const Vector2<T> &point)
{
...
template <typename T> void BoundingBox<T, 2>::merge(const BoundingBox &other)
{
...
template <typename T> void BoundingBox<T, 2>::expand(T delta)
{
    lowerCorner -= delta;
    upperCorner += delta;
}

template <typename T> Vector2<T> BoundingBox<T, 2>::corner(size_t idx) const
{
    static const T h = static_cast<T>(1) / 2;
    static const Vector2<T> offset[4] = {{-h, -h}, {+h, -h}, {-h, +h}, {+h, +h}};
    return Vector2<T>(width(), height()) * offset[idx] + midPoint();
}

template <typename T> Vector2<T> BoundingBox<T, 2>::clamp(const Vector2<T> &pt) const
{
    return ::jet::clamp(pt, lowerCorner, upperCorner);
}

template <typename T> bool BoundingBox<T, 2>::isEmpty() const
{
