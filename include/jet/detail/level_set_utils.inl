...
namespace jet
{
template <typename T> bool isInsideSdf(T phi)
{
    return phi < 0;
}

template <typename T> inline T smearedHeavisideSdf(T phi)
{
    if (phi > 1.5)
    {
        return 1;
...
template <typename T> inline T smearedDeltaSdf(T phi)
{
    if (std::fabs(phi) > 1.5)
    {
        return 0;
...
template <typename T> T fractionInsideSdf(T phi0, T phi1)
{
...
template <typename T> void cycleArray(T *arr, int size)
{
    T t = arr[0];
...
template <typename T> T distanceToZeroLevelSet(T phi0, T phi1)
{
