...
namespace jet
{
template <typename T> BoundingBox<T, 3>::BoundingBox()
{
    reset();
}

template <typename T> BoundingBox<T, 3>::BoundingBox(const Vector3<T> &point1, const Vector3<T> &point2)
{
...
template <typename T> T BoundingBox<T, 3>::width() const
{
    return upperCorner.x - lowerCorner.x;
}

template <typename T> T BoundingBox<T, 3>::height() const
{
    return upperCorner.y - lowerCorner.y;
}

template <typename T> T BoundingBox<T, 3>::depth() const
{
    return upperCorner.z - lowerCorner.z;
}

template <typename T> T BoundingBox<T, 3>::length(size_t axis)
{
    return upperCorner[axis] - lowerCorner[axis];
}

template <typename T> bool BoundingBox<T, 3>::overlaps(const BoundingBox &other) const
{
...
template <typename T> bool BoundingBox<T, 3>::contains(const Vector3<T> &point) const
{
...
template <typename T> bool BoundingBox<T, 3>::intersects(const Ray3<T> &ray) const
{
    T tMin = 0;
...
template <typename T> BoundingBoxRayIntersection3<T> BoundingBox<T, 3>::closestIntersection(const Ray3<T> &ray) const
{
...
template <typename T> Vector3<T> BoundingBox<T, 3>::midPoint() const
{
    return (upperCorner + lowerCorner) / static_cast<T>(2);
}

template <typename T> T BoundingBox<T, 3>::diagonalLength() const
{
    return (upperCorner - lowerCorner).length();
}

template <typename T> T BoundingBox<T, 3>::diagonalLengthSquared() const
{
    return (upperCorner - lowerCorner).lengthSquared();
}

template <typename T> void BoundingBox<T, 3>::reset()
{
...
template <typename T> void BoundingBox<T, 3>::merge(const Vector3<T> &point)
{
...
template <typename T> void BoundingBox<T, 3>::merge(const BoundingBox &other)
{
...
template <typename T> void BoundingBox<T, 3>::expand(T delta)
{
    lowerCorner -= delta;
    upperCorner += delta;
}

template <typename T> Vector3<T> BoundingBox<T, 3>::corner(size_t idx) const
{
    static const T h = static_cast<T>(1) / 2;
    static const Vector3<T> offset[8] = {{-h, -h, -h}, {+h, -h, -h}, {-h, +h, -h}, {+h, +h, -h},
                                         {-h, -h, +h}, {+h, -h, +h}, {-h, +h, +h}, {+h, +h, +h}};
    return Vector3<T>(width(), height(), depth()) * offset[idx] + midPoint();
}

template <typename T> Vector3<T> BoundingBox<T, 3>::clamp(const Vector3<T> &pt) const
{
    return ::jet::clamp(pt, lowerCorner, upperCorner);
}

template <typename T> bool BoundingBox<T, 3>::isEmpty() const
{
