...
namespace jet
{
template <typename T> Bvh3<T>::Node::Node() : flags(0)
{
    child = kMaxSize;
}

template <typename T> void Bvh3<T>::Node::initLeaf(size_t it, const BoundingBox3D &b)
{
    flags = 3;
    item = it;
    bound = b;
...
template <typename T> void Bvh3<T>::Node::initInternal(uint8_t axis, size_t c, const BoundingBox3D &b)
{
    flags = axis;
    child = c;
    bound = b;
...
template <typename T> bool Bvh3<T>::Node::isLeaf() const
{
    return flags == 3;
}

template <typename T> Bvh3<T>::Bvh3()
{
}

template <typename T> void Bvh3<T>::build(const std::vector<T> &items, const std::vector<BoundingBox3D> &itemsBounds)
{
    _items = items;
...
template <typename T> void Bvh3<T>::clear()
{
    _bound = BoundingBox3D();
    _items.clear();
    _itemBounds.clear();
...
template <typename T>
inline NearestNeighborQueryResult3<T> Bvh3<T>::nearest(const Vector3D &pt,
...
template <typename T>
inline ClosestIntersectionQueryResult3<T> Bvh3<T>::closestIntersection(const Ray3D &ray,
...
template <typename T> const BoundingBox3D &Bvh3<T>::boundingBox() const
{
    return _bound;
}

template <typename T> typename Bvh3<T>::Iterator Bvh3<T>::begin()
{
    return _items.begin();
}

template <typename T> typename Bvh3<T>::Iterator Bvh3<T>::end()
{
    return _items.end();
}

template <typename T> typename Bvh3<T>::ConstIterator Bvh3<T>::begin() const
{
    return _items.begin();
}

template <typename T> typename Bvh3<T>::ConstIterator Bvh3<T>::end() const
{
    return _items.end();
}

template <typename T> size_t Bvh3<T>::numberOfItems() const
{
    return _items.size();
}

template <typename T> const T &Bvh3<T>::item(size_t i) const
{
    return _items[i];
}

template <typename T> size_t Bvh3<T>::numberOfNodes() const
{
    return _nodes.size();
}

template <typename T> std::pair<size_t, size_t> Bvh3<T>::children(size_t i) const
{
    if (isLeaf(i))
    {
...
template <typename T> bool Bvh3<T>::isLeaf(size_t i) const
{
    return _nodes[i].isLeaf();
}

template <typename T> const BoundingBox3D &Bvh3<T>::nodeBound(size_t i) const
{
    return _nodes[i].bound;
}

template <typename T> typename Bvh3<T>::Iterator Bvh3<T>::itemOfNode(size_t i)
{
    if (isLeaf(i))
    {
...
template <typename T> typename Bvh3<T>::ConstIterator Bvh3<T>::itemOfNode(size_t i) const
{
    if (isLeaf(i))
    {
        return _nodes[i].item + begin();
...
template <typename T> size_t Bvh3<T>::build(size_t nodeIndex, size_t *itemIndices, size_t nItems, size_t currentDepth)
{
    // add a node
...
template <typename T> size_t Bvh3<T>::qsplit(size_t *itemIndices, size_t numItems, double pivot, uint8_t axis)
{
    double centroid;
    size_t ret = 0;
