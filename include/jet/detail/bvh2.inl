...
namespace jet
{
template <typename T> Bvh2<T>::Node::Node() : flags(0)
{
    child = kMaxSize;
}

template <typename T> void Bvh2<T>::Node::initLeaf(size_t it, const BoundingBox2D &b)
{
    flags = 2;
    item = it;
    bound = b;
...
template <typename T> void Bvh2<T>::Node::initInternal(uint8_t axis, size_t c, const BoundingBox2D &b)
{
    flags = axis;
    child = c;
    bound = b;
...
template <typename T> bool Bvh2<T>::Node::isLeaf() const
{
    return flags == 2;
}

template <typename T> Bvh2<T>::Bvh2()
{
}

template <typename T> void Bvh2<T>::build(const std::vector<T> &items, const std::vector<BoundingBox2D> &itemsBounds)
{
    _items = items;
...
template <typename T> void Bvh2<T>::clear()
{
    _bound = BoundingBox2D();
    _items.clear();
    _itemBounds.clear();
...
template <typename T>
inline NearestNeighborQueryResult2<T> Bvh2<T>::nearest(const Vector2D &pt,
...
template <typename T>
inline ClosestIntersectionQueryResult2<T> Bvh2<T>::closestIntersection(const Ray2D &ray,
...
template <typename T> const BoundingBox2D &Bvh2<T>::boundingBox() const
{
    return _bound;
}

template <typename T> typename Bvh2<T>::Iterator Bvh2<T>::begin()
{
    return _items.begin();
}

template <typename T> typename Bvh2<T>::Iterator Bvh2<T>::end()
{
    return _items.end();
}

template <typename T> typename Bvh2<T>::ConstIterator Bvh2<T>::begin() const
{
    return _items.begin();
}

template <typename T> typename Bvh2<T>::ConstIterator Bvh2<T>::end() const
{
    return _items.end();
}

template <typename T> size_t Bvh2<T>::numberOfItems() const
{
    return _items.size();
}

template <typename T> const T &Bvh2<T>::item(size_t i) const
{
    return _items[i];
}

template <typename T> size_t Bvh2<T>::numberOfNodes() const
{
    return _nodes.size();
}

template <typename T> std::pair<size_t, size_t> Bvh2<T>::children(size_t i) const
{
    if (isLeaf(i))
    {
...
template <typename T> bool Bvh2<T>::isLeaf(size_t i) const
{
    return _nodes[i].isLeaf();
}

template <typename T> const BoundingBox2D &Bvh2<T>::nodeBound(size_t i) const
{
    return _nodes[i].bound;
}

template <typename T> typename Bvh2<T>::Iterator Bvh2<T>::itemOfNode(size_t i)
{
    if (isLeaf(i))
    {
...
template <typename T> typename Bvh2<T>::ConstIterator Bvh2<T>::itemOfNode(size_t i) const
{
    if (isLeaf(i))
    {
        return _nodes[i].item + begin();
...
template <typename T> size_t Bvh2<T>::build(size_t nodeIndex, size_t *itemIndices, size_t nItems, size_t currentDepth)
{
    // add a node
...
template <typename T> size_t Bvh2<T>::qsplit(size_t *itemIndices, size_t numItems, double pivot, uint8_t axis)
{
    double centroid;
    size_t ret = 0;
