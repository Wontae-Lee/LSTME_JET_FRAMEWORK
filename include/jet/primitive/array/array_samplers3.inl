...
NearestArraySampler3<T, R>::operator()(const Vector3<R>& x) const
{
  ...
  JET_ASSERT(_gridSpacing.x > std::numeric_limits<R>::epsilon() && _gridSpacing.y > std::numeric_limits<R>::epsilon() &&
...
template<typename T, typename R>
void
NearestArraySampler3<T, R>::getCoordinate(const Vector3<R>& x, Point3UI* index) const
{
  ssize_t i, j, k;
  R fx, fy, fz;
...
template<typename T, typename R>
void
LinearArraySampler3<T, R>::getCoordinatesAndWeights(const Vector3<R>& x, std::array<Point3UI, 8>* indices, std::array<R, 8>* weights) const
{
  ssize_t i, j, k;
  R fx, fy, fz;
...
template<typename T, typename R>
void
LinearArraySampler3<T, R>::getCoordinatesAndGradientWeights(const Vector3<R>& x,
                                                            ...
                                                            std::array<Vector3<R>, 8>* weights) const
{
  ...
  (*weights)[0] =
    Vector3<R>(-_invGridSpacing.x * (1 - fy) * (1 - fz), -_invGridSpacing.y * (1 - fx) * (1 - fz), -_invGridSpacing.z * (1 - fx) * (1 - fy));
  ...
  for (int kk = 0; kk < 4; ++kk) {
    ...
    for (int jj = 0; jj < 4; ++jj) {
      jValues[jj] = monotonicCatmullRom(
        _accessor(is[0], js[jj], ks[kk]), _accessor(is[1], js[jj], ks[kk]), _accessor(is[2], js[jj], ks[kk]), _accessor(is[3], js[jj], ks[kk]), fx);
    }
