...
NearestArraySampler3<T, R>::NearestArraySampler(const ConstArrayAccessor3<T>& accessor, const Vector3<R>& gridSpacing, const Vector3<R>& gridOrigin) {
...
T NearestArraySampler3<T, R>::operator()(const Vector3<R>& x) const {
    ...
    JET_ASSERT(_gridSpacing.x > std::numeric_limits<R>::epsilon() && _gridSpacing.y > std::numeric_limits<R>::epsilon() && _gridSpacing.z > std::numeric_limits<R>::epsilon());
...
template <typename T, typename R>
void NearestArraySampler3<T, R>::getCoordinate(const Vector3<R>& x, Point3UI* index) const {
    ssize_t i, j, k;
    R fx, fy, fz;
...
template <typename T, typename R>
LinearArraySampler3<T, R>::LinearArraySampler(const ConstArrayAccessor3<T>& accessor, const Vector3<R>& gridSpacing, const Vector3<R>& gridOrigin) {
    _gridSpacing = gridSpacing;
...
T LinearArraySampler3<T, R>::operator()(const Vector3<R>& x) const {
    ...
    return trilerp(_accessor(i, j, k), _accessor(ip1, j, k), _accessor(i, jp1, k), _accessor(ip1, jp1, k), _accessor(i, j, kp1), _accessor(ip1, j, kp1), _accessor(i, jp1, kp1),
                   _accessor(ip1, jp1, kp1), fx, fy, fz);
}

template <typename T, typename R>
void LinearArraySampler3<T, R>::getCoordinatesAndWeights(const Vector3<R>& x, std::array<Point3UI, 8>* indices, std::array<R, 8>* weights) const {
    ssize_t i, j, k;
    R fx, fy, fz;

    JET_ASSERT(_gridSpacing.x > 0.0 && _gridSpacing.y > 0.0 && _gridSpacing.z > 0.0);
...
template <typename T, typename R>
void LinearArraySampler3<T, R>::getCoordinatesAndGradientWeights(const Vector3<R>& x, std::array<Point3UI, 8>* indices, std::array<Vector3<R>, 8>* weights) const {
    ssize_t i, j, k;
    R fx, fy, fz;
    ...
    (*weights)[0] = Vector3<R>(-_invGridSpacing.x * (1 - fy) * (1 - fz), -_invGridSpacing.y * (1 - fx) * (1 - fz), -_invGridSpacing.z * (1 - fx) * (1 - fy));
    (*weights)[1] = Vector3<R>(_invGridSpacing.x * (1 - fy) * (1 - fz), fx * (-_invGridSpacing.y) * (1 - fz), fx * (1 - fy) * (-_invGridSpacing.z));
    (*weights)[2] = Vector3<R>((-_invGridSpacing.x) * fy * (1 - fz), (1 - fx) * _invGridSpacing.y * (1 - fz), (1 - fx) * fy * (-_invGridSpacing.z));
    (*weights)[3] = Vector3<R>(_invGridSpacing.x * fy * (1 - fz), fx * _invGridSpacing.y * (1 - fz), fx * fy * (-_invGridSpacing.z));
    (*weights)[4] = Vector3<R>((-_invGridSpacing.x) * (1 - fy) * fz, (1 - fx) * (-_invGridSpacing.y) * fz, (1 - fx) * (1 - fy) * _invGridSpacing.z);
    (*weights)[5] = Vector3<R>(_invGridSpacing.x * (1 - fy) * fz, fx * (-_invGridSpacing.y) * fz, fx * (1 - fy) * _invGridSpacing.z);
    (*weights)[6] = Vector3<R>((-_invGridSpacing.x) * fy * fz, (1 - fx) * _invGridSpacing.y * fz, (1 - fx) * fy * _invGridSpacing.z);
...
template <typename T, typename R>
CubicArraySampler3<T, R>::CubicArraySampler(const ConstArrayAccessor3<T>& accessor, const Vector3<R>& gridSpacing, const Vector3<R>& gridOrigin) {
    _gridSpacing = gridSpacing;
    _origin = gridOrigin;
    _accessor = accessor;
}
...
T CubicArraySampler3<T, R>::operator()(const Vector3<R>& x) const {
    ...
    for (int kk = 0; kk < 4; ++kk) {
        ...
        for (int jj = 0; jj < 4; ++jj) {
            jValues[jj] = monotonicCatmullRom(_accessor(is[0], js[jj], ks[kk]), _accessor(is[1], js[jj], ks[kk]), _accessor(is[2], js[jj], ks[kk]), _accessor(is[3], js[jj], ks[kk]), fx);
        }
