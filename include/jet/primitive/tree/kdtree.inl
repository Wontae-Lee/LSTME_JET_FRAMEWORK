...
#include "jet/primitive/tree/kdtree.h"
...
template<typename T, size_t K>
void
KdTree<T, K>::Node::initInternal(size_t axis, size_t it, size_t c, const Point& pt)
{
  flags = axis;
  item = it;
  child = c;
...
template<typename T, size_t K>
void
KdTree<T, K>::forEachNearbyPoint(const Point& origin, T radius, const std::function<void(size_t, const Point&)>& callback) const
{
...
template<typename T, size_t K>
size_t
KdTree<T, K>::build(size_t nodeIndex, size_t* itemIndices, size_t nItems, size_t currentDepth)
{
  // add a node
  _nodes.emplace_back();
  ...
  // pick mid point
  std::nth_element(
    itemIndices, itemIndices + nItems / 2, itemIndices + nItems, [&](size_t a, size_t b) { return _points[a][axis] < _points[b][axis]; });
