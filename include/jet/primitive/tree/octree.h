...
class Octree final : public IntersectionQueryEngine3<T>, public NearestNeighborQueryEngine3<T> {
 ...
 private:
    ...
    void forEachIntersectingItem(const BoundingBox3D& box, const BoxIntersectionTestFunc3<T>& testFunc, const IntersectionVisitorFunc3<T>& visitorFunc, size_t nodeIdx,
                                 const BoundingBox3D& currentBound) const;

    void forEachIntersectingItem(const Ray3D& ray, const RayIntersectionTestFunc3<T>& testFunc, const IntersectionVisitorFunc3<T>& visitorFunc, size_t nodeIdx,
                                 const BoundingBox3D& currentBound) const;

    ClosestIntersectionQueryResult3<T> closestIntersection(const Ray3D& ray, const GetRayIntersectionFunc3<T>& testFunc, size_t nodeIdx, const BoundingBox3D& currentBound,
