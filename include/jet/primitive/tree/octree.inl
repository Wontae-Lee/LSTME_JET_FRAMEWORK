...
#include "jet/primitive/tree/octree.h"
...
template <typename T>
void Octree<T>::build(const std::vector<T>& items, const BoundingBox3D& bound, const BoxIntersectionTestFunc3<T>& testFunc, size_t maxDepth) {
    // Reset items
    _maxDepth = maxDepth;
    _items = items;
    _nodes.clear();
    ...
    _bbox.upperCorner = _bbox.lowerCorner + Vector3D(maxEdgeLen, maxEdgeLen, maxEdgeLen);
...
template <typename T>
NearestNeighborQueryResult3<T> Octree<T>::nearest(const Vector3D& pt, const NearestNeighborDistanceFunc3<T>& distanceFunc) const {
    ...
    while (node != nullptr) {
        if (node->isLeaf()) {
            ...
            std::sort(childDistSqrPairs.begin(), childDistSqrPairs.end(), [](const NodeDistBox& a, const NodeDistBox& b) { return std::get<1>(a) > std::get<1>(b); });
...
template <typename T>
void Octree<T>::forEachIntersectingItem(const BoundingBox3D& box, const BoxIntersectionTestFunc3<T>& testFunc, const IntersectionVisitorFunc3<T>& visitorFunc) const {
    forEachIntersectingItem(box, testFunc, visitorFunc, 0, _bbox);
}

template <typename T>
void Octree<T>::forEachIntersectingItem(const Ray3D& ray, const RayIntersectionTestFunc3<T>& testFunc, const IntersectionVisitorFunc3<T>& visitorFunc) const {
    forEachIntersectingItem(ray, testFunc, visitorFunc, 0, _bbox);
}

template <typename T>
ClosestIntersectionQueryResult3<T> Octree<T>::closestIntersection(const Ray3D& ray, const GetRayIntersectionFunc3<T>& testFunc) const {
...
template <typename T>
void Octree<T>::build(size_t nodeIdx, size_t depth, const BoundingBox3D& bound, const BoxIntersectionTestFunc3<T>& testFunc) {
...
template <typename T>
bool Octree<T>::intersects(const BoundingBox3D& box, const BoxIntersectionTestFunc3<T>& testFunc, size_t nodeIdx, const BoundingBox3D& bound) const {
...
template <typename T>
bool Octree<T>::intersects(const Ray3D& ray, const RayIntersectionTestFunc3<T>& testFunc, size_t nodeIdx, const BoundingBox3D& bound) const {
...
template <typename T>
void Octree<T>::forEachIntersectingItem(const BoundingBox3D& box, const BoxIntersectionTestFunc3<T>& testFunc, const IntersectionVisitorFunc3<T>& visitorFunc, size_t nodeIdx,
    ...
    if (node.firstChild != kMaxSize) {
        for (int i = 0; i < 8; ++i) {
            forEachIntersectingItem(box, testFunc, visitorFunc, node.firstChild + i, BoundingBox3D(bound.corner(i), bound.midPoint()));
        }
    }
}

template <typename T>
void Octree<T>::forEachIntersectingItem(const Ray3D& ray, const RayIntersectionTestFunc3<T>& testFunc, const IntersectionVisitorFunc3<T>& visitorFunc, size_t nodeIdx,
                                        const BoundingBox3D& bound) const {
    if (!bound.intersects(ray)) {
        return;
    }
    ...
    if (node.firstChild != kMaxSize) {
        for (int i = 0; i < 8; ++i) {
            forEachIntersectingItem(ray, testFunc, visitorFunc, node.firstChild + i, BoundingBox3D(bound.corner(i), bound.midPoint()));
        }
    }
}

template <typename T>
ClosestIntersectionQueryResult3<T> Octree<T>::closestIntersection(const Ray3D& ray, const GetRayIntersectionFunc3<T>& testFunc, size_t nodeIdx, const BoundingBox3D& bound,
    ...
    if (node.firstChild != kMaxSize) {
        for (int i = 0; i < 8; ++i) {
            best = closestIntersection(ray, testFunc, node.firstChild + i, BoundingBox3D(bound.corner(i), bound.midPoint()), best);
        }
    }
