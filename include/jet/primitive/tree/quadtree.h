...
#include <vector>
#include "jet/engine/intersection_query_engine2.h"
#include "jet/engine/nearest_neighbor_query_engine2.h"
...
class Quadtree final : public IntersectionQueryEngine2<T>, public NearestNeighborQueryEngine2<T> {
 ...
 private:
    ...
    void build(size_t nodeIdx, size_t currentDepth, const BoundingBox2D& currentBound, const BoxIntersectionTestFunc2<T>& overlapsFunc);

    bool intersects(const BoundingBox2D& box, const BoxIntersectionTestFunc2<T>& testFunc, size_t nodeIdx, const BoundingBox2D& currentBound) const;

    bool intersects(const Ray2D& ray, const RayIntersectionTestFunc2<T>& testFunc, size_t nodeIdx, const BoundingBox2D& currentBound) const;

    void forEachIntersectingItem(const BoundingBox2D& box, const BoxIntersectionTestFunc2<T>& testFunc, const IntersectionVisitorFunc2<T>& visitorFunc, size_t nodeIdx,
                                 const BoundingBox2D& currentBound) const;

    void forEachIntersectingItem(const Ray2D& ray, const RayIntersectionTestFunc2<T>& testFunc, const IntersectionVisitorFunc2<T>& visitorFunc, size_t nodeIdx,
                                 const BoundingBox2D& currentBound) const;

    ClosestIntersectionQueryResult2<T> closestIntersection(const Ray2D& ray, const GetRayIntersectionFunc2<T>& testFunc, size_t nodeIdx, const BoundingBox2D& currentBound,
