...
#include "jet/primitive/tree/quadtree.h"
...
template <typename T>
void Quadtree<T>::build(const std::vector<T>& items, const BoundingBox2D& bound, const BoxIntersectionTestFunc2<T>& testFunc, size_t maxDepth) {
    // Reset items
    _maxDepth = maxDepth;
    _items = items;
    _nodes.clear();
...
template <typename T>
NearestNeighborQueryResult2<T> Quadtree<T>::nearest(const Vector2D& pt, const NearestNeighborDistanceFunc2<T>& distanceFunc) const {
...
template <typename T>
void Quadtree<T>::forEachIntersectingItem(const BoundingBox2D& box, const BoxIntersectionTestFunc2<T>& testFunc, const IntersectionVisitorFunc2<T>& visitorFunc) const {
    forEachIntersectingItem(box, testFunc, visitorFunc, 0, _bbox);
}

template <typename T>
void Quadtree<T>::forEachIntersectingItem(const Ray2D& ray, const RayIntersectionTestFunc2<T>& testFunc, const IntersectionVisitorFunc2<T>& visitorFunc) const {
    forEachIntersectingItem(ray, testFunc, visitorFunc, 0, _bbox);
}

template <typename T>
ClosestIntersectionQueryResult2<T> Quadtree<T>::closestIntersection(const Ray2D& ray, const GetRayIntersectionFunc2<T>& testFunc) const {
...
template <typename T>
void Quadtree<T>::build(size_t nodeIdx, size_t depth, const BoundingBox2D& bound, const BoxIntersectionTestFunc2<T>& testFunc) {
...
template <typename T>
bool Quadtree<T>::intersects(const BoundingBox2D& box, const BoxIntersectionTestFunc2<T>& testFunc, size_t nodeIdx, const BoundingBox2D& bound) const {
...
template <typename T>
bool Quadtree<T>::intersects(const Ray2D& ray, const RayIntersectionTestFunc2<T>& testFunc, size_t nodeIdx, const BoundingBox2D& bound) const {
...
template <typename T>
void Quadtree<T>::forEachIntersectingItem(const BoundingBox2D& box, const BoxIntersectionTestFunc2<T>& testFunc, const IntersectionVisitorFunc2<T>& visitorFunc, size_t nodeIdx,
    ...
    if (node.firstChild != kMaxSize) {
        for (int i = 0; i < 4; ++i) {
            forEachIntersectingItem(box, testFunc, visitorFunc, node.firstChild + i, BoundingBox2D(bound.corner(i), bound.midPoint()));
        }
    }
}

template <typename T>
void Quadtree<T>::forEachIntersectingItem(const Ray2D& ray, const RayIntersectionTestFunc2<T>& testFunc, const IntersectionVisitorFunc2<T>& visitorFunc, size_t nodeIdx,
                                          const BoundingBox2D& bound) const {
    if (!bound.intersects(ray)) {
        return;
    }
    ...
    if (node.firstChild != kMaxSize) {
        for (int i = 0; i < 4; ++i) {
            forEachIntersectingItem(ray, testFunc, visitorFunc, node.firstChild + i, BoundingBox2D(bound.corner(i), bound.midPoint()));
        }
    }
}

template <typename T>
ClosestIntersectionQueryResult2<T> Quadtree<T>::closestIntersection(const Ray2D& ray, const GetRayIntersectionFunc2<T>& testFunc, size_t nodeIdx, const BoundingBox2D& bound,
    ...
    if (node.firstChild != kMaxSize) {
        for (int i = 0; i < 4; ++i) {
            best = closestIntersection(ray, testFunc, node.firstChild + i, BoundingBox2D(bound.corner(i), bound.midPoint()), best);
        }
    }
