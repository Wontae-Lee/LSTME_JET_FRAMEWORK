...
template<typename T>
void
Bvh2<T>::Node::initLeaf(size_t it, const BoundingBox2D& b)
{
  flags = 2;
  item = it;
  bound = b;
...
template<typename T>
void
Bvh2<T>::Node::initInternal(uint8_t axis, size_t c, const BoundingBox2D& b)
{
  flags = axis;
  child = c;
  bound = b;
...
template<typename T>
void
Bvh2<T>::build(const std::vector<T>& items, const std::vector<BoundingBox2D>& itemsBounds)
{
  _items = items;
...
template<typename T>
inline NearestNeighborQueryResult2<T>
Bvh2<T>::nearest(const Vector2D& pt, const NearestNeighborDistanceFunc2<T>& distanceFunc) const
{
...
template<typename T>
inline bool
Bvh2<T>::intersects(const BoundingBox2D& box, const BoxIntersectionTestFunc2<T>& testFunc) const
{
...
template<typename T>
inline bool
Bvh2<T>::intersects(const Ray2D& ray, const RayIntersectionTestFunc2<T>& testFunc) const
{
...
template<typename T>
inline void
Bvh2<T>::forEachIntersectingItem(const BoundingBox2D& box,
...
template<typename T>
inline void
Bvh2<T>::forEachIntersectingItem(const Ray2D& ray, const RayIntersectionTestFunc2<T>& testFunc, const IntersectionVisitorFunc2<T>& visitorFunc) const
{
  if (!_bound.intersects(ray)) {
    return;
  }
...
template<typename T>
inline ClosestIntersectionQueryResult2<T>
Bvh2<T>::closestIntersection(const Ray2D& ray, const GetRayIntersectionFunc2<T>& testFunc) const
{
...
template<typename T>
const BoundingBox2D&
Bvh2<T>::boundingBox() const
{
  return _bound;
}

template<typename T>
typename Bvh2<T>::Iterator
Bvh2<T>::begin()
{
  return _items.begin();
}
...
Bvh2<T>::children(size_t i) const
{
  if (isLeaf(i)) {
...
template<typename T>
const BoundingBox2D&
Bvh2<T>::nodeBound(size_t i) const
{
...
template<typename T>
typename Bvh2<T>::ConstIterator
Bvh2<T>::itemOfNode(size_t i) const
{
  if (isLeaf(i)) {
    return _nodes[i].item + begin();
  } else {
...
template<typename T>
size_t
Bvh2<T>::build(size_t nodeIndex, size_t* itemIndices, size_t nItems, size_t currentDepth)
{
  // add a node
...
template<typename T>
size_t
Bvh2<T>::qsplit(size_t* itemIndices, size_t numItems, double pivot, uint8_t axis)
{
  double centroid;
  size_t ret = 0;
