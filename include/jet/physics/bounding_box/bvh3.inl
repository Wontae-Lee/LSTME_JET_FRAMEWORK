...
template<typename T>
void
Bvh3<T>::build(const std::vector<T>& items, const std::vector<BoundingBox3D>& itemsBounds)
{
  _items = items;
...
Bvh3<T>::nearest(const Vector3D& pt, const NearestNeighborDistanceFunc3<T>& distanceFunc) const
{
...
template<typename T>
inline bool
Bvh3<T>::intersects(const BoundingBox3D& box, const BoxIntersectionTestFunc3<T>& testFunc) const
{
...
template<typename T>
inline bool
Bvh3<T>::intersects(const Ray3D& ray, const RayIntersectionTestFunc3<T>& testFunc) const
{
...
template<typename T>
inline void
Bvh3<T>::forEachIntersectingItem(const BoundingBox3D& box,
...
template<typename T>
inline void
Bvh3<T>::forEachIntersectingItem(const Ray3D& ray, const RayIntersectionTestFunc3<T>& testFunc, const IntersectionVisitorFunc3<T>& visitorFunc) const
{
  if (!_bound.intersects(ray)) {
    return;
  }
...
template<typename T>
size_t
Bvh3<T>::build(size_t nodeIndex, size_t* itemIndices, size_t nItems, size_t currentDepth)
{
  // add a node
...
template<typename T>
size_t
Bvh3<T>::qsplit(size_t* itemIndices, size_t numItems, double pivot, uint8_t axis)
{
  double centroid;
  size_t ret = 0;
