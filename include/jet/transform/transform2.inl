...
inline Transform2::Transform2(const Vector2D& translation, double orientation) : _translation(translation), _orientation(orientation) {
...
inline Ray2D Transform2::toLocal(const Ray2D& rayInWorld) const { return Ray2D(toLocal(rayInWorld.origin), toLocalDirection(rayInWorld.direction)); }
...
inline Vector2D Transform2::toWorld(const Vector2D& pointInLocal) const {
    ...
    return Vector2D(_cosAngle * pointInLocal.x - _sinAngle * pointInLocal.y + _translation.x, _sinAngle * pointInLocal.x + _cosAngle * pointInLocal.y + _translation.y);
}
...
inline Ray2D Transform2::toWorld(const Ray2D& rayInLocal) const { return Ray2D(toWorld(rayInLocal.origin), toWorldDirection(rayInLocal.direction)); }
